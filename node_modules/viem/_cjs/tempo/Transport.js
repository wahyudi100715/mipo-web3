...
exports.http = http;
exports.withMultisig = withMultisig;
...
function withMultisig(transport, parameters) {
    return ((options) => {
        const value = transport(options);
        return {
            ...value,
            request: Multisig.handleRequest((request, requestOptions) => value.request(request, requestOptions), parameters),
        ...
        const transport = (0, createTransport_js_1.createTransport)({
            ...
            async request({ method, params }, options) {
                ...
                if (method === 'eth_getTransactionByHash' ||
                    ...
                    const result = await transport_default.request({ method, params }, options);
                    if (result !== null && typeof result !== 'undefined')
                        return result;
                    const operation = await (async () => {
                        try {
                            return await transport_relay.request({ method: 'multisig_getOperation', params }, options);
                        }
                        catch (error) {
                            if (error instanceof rpc_js_1.MethodNotFoundRpcError ||
                                error instanceof rpc_js_1.MethodNotSupportedRpcError) {
                                return null;
                            }
                            throw error;
                        }
                ...
                if (method === 'multisig_approveKeyAuthorization' ||
