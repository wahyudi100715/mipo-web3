    ...
    return async (request, requestOptions_) => {
        ...
        const client = (0, createClient_js_1.createClient)({
            transport: (0, custom_js_1.custom)({
                request: ({ method, params }, options) => next({ method, params }, { ...requestOptions, ...options }),
            }),
        });
...
async function resolveRequestChainId(request, store) {
    const value = request.params?.[0];
    const chainId_body = value && typeof value === 'object' && 'chainId' in value
    ...
    const chainId_multisig = await (async () => {
        if (request.method === 'eth_sendRawTransaction' ||
            ...
            request.method === 'multisig_approveRawTransaction' ||
            request.method === 'multisig_approveRawTransactionSync') {
            if (!isSerializedTempoTransaction(value))
                return undefined;
            try {
        ...
        if (request.method === 'multisig_approveKeyAuthorization' &&
            ...
            try {
                return parseChainId(tempo_1.KeyAuthorization.fromRpc(value.keyAuthorization).chainId);
            }
            catch {
                return undefined;
            }
        ...
        if (!operation)
            return undefined;
        if (operation.type === 'transaction')
            return parseChainId(Transaction.deserialize(operation.transaction).chainId);
        return parseChainId(tempo_1.KeyAuthorization.deserialize(operation.keyAuthorization).chainId);
    })();
    if (chainId_body !== undefined &&
