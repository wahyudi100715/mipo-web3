...
const minify = linebreakPattern => {
  ...
  return code => {
    ...
    const eliminatedExpressionIndices = difference(code.match(_placeholderUtils.placeholderRegex), newCode.match(_placeholderUtils.placeholderRegex)).map(x => parseInt(x.match(/\d+/)[0], 10));
    return [compressSymbols(newCode), eliminatedExpressionIndices];
  };
};
const minifyRaw = exports.minifyRaw = minify('(?:\\\\r|\\\\n|\\r|\\n)');
const minifyCooked = exports.minifyCooked = minify('[\\r\\n]');
