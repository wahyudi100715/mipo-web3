...
const hasDisplayNameOrComponentId = (t, properties) => properties.some(prop => {
...
const addConfig = t => (path, displayName, componentId) => {
  ...
  if (existingConfig && existingConfig.arguments.length && Array.isArray(existingConfig.arguments[0].properties) && !hasDisplayNameOrComponentId(t, existingConfig.arguments[0].properties)) {
    existingConfig.arguments[0].properties.push(...withConfigProps);
    return;
  }
  if (path.node.callee && t.isMemberExpression(path.node.callee.callee) && path.node.callee.callee.property && path.node.callee.callee.property.name && path.node.callee.callee.property.name == 'withConfig' && path.node.callee.arguments.length && Array.isArray(path.node.callee.arguments[0].properties) && !hasDisplayNameOrComponentId(t, path.node.callee.arguments[0].properties)) {
