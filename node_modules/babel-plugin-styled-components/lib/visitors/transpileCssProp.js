...
const ISSUE_URL = 'https://github.com/styled-components/babel-plugin-styled-components';
const getName = (node, t, path) => {
  if (typeof node.name === 'string') return node.name;
  if (t.isJSXMemberExpression(node)) {
    return `${getName(node.object, t, path)}.${node.property.name}`;
  }
  throw path.buildCodeFrameError(`Cannot infer name from node with type "${node.type}". Please submit an issue at ${ISSUE_URL} with your code so we can take a look at your use case!`);
};
const getNameExpression = (node, t, path) => {
  if (typeof node.name === 'string') return t.identifier(node.name);
  if (t.isJSXMemberExpression(node)) {
    return t.memberExpression(getNameExpression(node.object, t, path), t.identifier(node.property.name));
  }
  throw path.buildCodeFrameError(`Cannot infer name expression from node with type "${node.type}". Please submit an issue at ${ISSUE_URL} with your code so we can take a look at your use case!`);
};
...
var _default = t => {
  const taggedTemplateVisit = (0, _process.processTaggedTemplate)(t);
  const callExpressionVisit = (0, _process.processCallExpression)(t);
  return (path, state) => {
    ...
    const importBindingIsNamespace = importBinding && importBinding.path && importBinding.path.isImportNamespaceSpecifier();
    ...
    if (!importBinding || importBindingIsNamespace) {
      (0, _helperModuleImports.addDefault)(path, (0, _options.useCssPropImportPath)(state), {
        nameHint: 'styled'
      });
    ...
    if (t.isObjectExpression(css)) {
      ...
      css.properties = css.properties.reduce(function propertiesReducer(acc, property) {
        ...
        t.isIdentifier(property.key) && property.computed && path.scope.hasBinding(property.key.name) && (
        ...
        .every(x => !x(property.value))) {
          ...
          acc.push(t.objectProperty(property.key, t.memberExpression(p, identifier), property.computed,
    ...
    const insertedPaths = injector(t.variableDeclaration('var', [t.variableDeclarator(id, t.isObjectExpression(css) || t.isArrowFunctionExpression(css) ? t.callExpression(styled, [css]) : t.taggedTemplateExpression(styled, css))]));
