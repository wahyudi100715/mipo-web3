...
const CODE_REQUIREMENT = "Must be a string of 1-32 characters, lowercase alphanumeric and underscores only.";
 ...
 */
export function assertBuilderCode(code) {
 ...
 * than four codes.
 */
export function toServiceBuilderCodes(builderCode) {
    const codes = Array.isArray(builderCode) ? builderCode : [builderCode];
    if (codes.length === 0) {
        throw new Error("Invalid builder code: []. Supply at least one code, or omit builderCode to use only the SDK's service code.");
    }
    if (codes.length > MAX_CONFIGURED_SERVICE_BUILDER_CODES) {
        throw new Error(`Invalid builder code: at most ${MAX_CONFIGURED_SERVICE_BUILDER_CODES} configured service codes are allowed.`);
    }
 ...
 */
export function declareServerBuilderCodeExtension(appCode) {
    return {
        info: {
