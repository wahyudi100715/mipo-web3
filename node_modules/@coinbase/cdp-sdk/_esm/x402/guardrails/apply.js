...
import { parseAmount, parseDuration, SpendControlError, } from "./types.js";
/** Default thresholds (as fractions of the cap) for `onApproachingLimit`. */
export const DEFAULT_APPROACHING_LIMIT_THRESHOLDS = [0.8, 0.95];
/**
...
const parseAtomicFromRequirement = (req, x402Version) => {
    ...
    if (hasAmount && hasMaxAmountRequired) {
        throw new SpendControlError("malformed_requirement", "PaymentRequirements must not include both `amount` (v2) and `maxAmountRequired` (v1) " +
    ...
    if (x402Version === 1) {
        ...
        if (!validated.success) {
            throw new SpendControlError("malformed_requirement", `PaymentRequirements failed v1 schema validation: ${validated.error.message}`, { asset: req.asset, network: req.network, payTo: req.payTo });
        }
        ...
        if (!validated.success) {
            throw new SpendControlError("malformed_requirement", `PaymentRequirements failed v2 schema validation: ${validated.error.message}`, { asset: req.asset, network: req.network, payTo: req.payTo });
        }
        raw = validated.data.amount;
        maxTimeoutSeconds = validated.data.maxTimeoutSeconds;
    }
    if (maxTimeoutSeconds > MAX_PAYMENT_TIMEOUT_SECONDS) {
        throw new SpendControlError("max_timeout_exceeded", `PaymentRequirements maxTimeoutSeconds ${maxTimeoutSeconds} exceeds the maximum allowed ${MAX_PAYMENT_TIMEOUT_SECONDS}s`, {
    ...
    try {
        ...
        throw new SpendControlError("amount_unparseable", `PaymentRequirements amount ${JSON.stringify(raw)} is not a valid atomic integer`, {
    ...
    if (parsed < 0n) {
        throw new SpendControlError("amount_unparseable", `PaymentRequirements amount ${JSON.stringify(raw)} must be a non-negative atomic integer`, {
 ...
 */
export function applySpendControls(client, controls) {
    const taggedClient = client;
    if (taggedClient[APPLIED_MARKER]) {
        throw new SpendControlError("already_applied", "applySpendControls() was called more than once on the same x402Client. " +
    ...
    if (maxCumulativeSpend && !maxCumulativeSpend.asset) {
        throw new SpendControlError("amount_unparseable", "maxCumulativeSpend requires an `asset` — cross-asset atomic units cannot be summed.", { input: controls.maxCumulativeSpend });
    }
        ...
        },
        {
            ...
            check: req => {
                ...
                if (!normalizedAllowList) {
                    normalizedAllowList = new Set(rawAllowedPayees.map(payee => normalizePayee(canonicalReqNetwork, payee)));
    ...
    const beforeHook = async (context) => {
        ...
        if (maxAmountPerPayment &&
            ...
            throw new SpendControlError("per_payment_cap", `Payment amount ${atomic} exceeds per-payment cap ${maxAmountPerPayment.atomic}` +
        ...
        if (maxCumulativeSpend && assetMatches(maxCumulativeSpend.asset, req.asset)) {
            ...
            await withAssetLock(lockKey, async () => {
                ...
                if (total + atomic > maxCumulativeSpend.atomic) {
                    throw new SpendControlError("cumulative_cap", `Payment of ${atomic} would push cumulative spend to ${total + atomic}, exceeding cap ${maxCumulativeSpend.atomic}` +
    ...
    const failureHook = async (context) => {
        ...
        if (!recordedEntry)
            return undefined;
        clearRecordedEntry(context);
        const notifyKey = normalizeAsset(context.selectedRequirements.asset);
        await withAssetLock(notifyKey, async () => {
            const pMap = getProvisionalMap();
    ...
    taggedClient[SPEND_CONTROLS_REGISTRY] = registry;
    /*
 ...
 */
export function getSpendControlsRegistry(client) {
    const tagged = client;
