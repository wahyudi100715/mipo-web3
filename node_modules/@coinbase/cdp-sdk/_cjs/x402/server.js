...
const CDP_EXACT_PAYMENT_FLOWS = new Set(["authorization", "upfront"]);
/**
 ...
 */
function getDefaultNetworks(environment) {
 ...
 */
function assertPaymentFlowCompatible(route, scheme) {
    if (route.paymentFlow === undefined)
        return;
    if (!CDP_EXACT_PAYMENT_FLOWS.has(route.paymentFlow)) {
        throw new Error(`Unsupported paymentFlow "${String(route.paymentFlow)}". ` +
            `The simplified route format only supports "authorization" (default) and "upfront".`);
    }
    if (scheme !== "exact") {
        throw new Error(`paymentFlow is only supported on the "exact" scheme. ` +
 ...
 */
const CDP_SIMPLIFIED_ROUTE_SCHEMES = new Set([
    "exact",
    "upto",
]);
/**
 ...
 */
function assertValidSimplifiedScheme(scheme) {
    if (typeof scheme !== "string" || !CDP_SIMPLIFIED_ROUTE_SCHEMES.has(scheme)) {
        throw new Error(`Unsupported simplified route scheme "${String(scheme)}". ` +
 ...
 * @param environment - Deployment environment controlling default network selection.
 */
function validateRouteSemantics(routes, environment) {
    for (const route of Object.values(routes)) {
        if ("accepts" in route)
            continue;
        ...
        for (const network of networks) {
            if (networkFamily(network) === "other") {
                throw new Error(`Unsupported network family for simplified route: "${network}". ` +
...
function convertCdpRoute(route, evmAddress, svmAddress, environment, available) {
    ...
    const accepts = networks.map(network => {
        ...
        return {
            ...
            payTo,
            maxTimeoutSeconds,
            ...(route.paymentFlow === "upfront" ? { extra: { paymentFlow: "upfront" } } : {}),
        };
    });
    return {
