...
exports.SpendControlError = exports.SpendControlErrorCodes = void 0;
exports.parseDuration = parseDuration;
exports.parseAmount = parseAmount;
/**
 * Typed string constants for every {@link SpendControlErrorCode} value.
 */
exports.SpendControlErrorCodes = {
 ...
 */
class SpendControlError extends Error {
...
exports.SpendControlError = SpendControlError;
const MS_UNIT_FACTORS = {
    ms: 1,
    s: 1_000,
    m: 60 * 1_000,
    h: 60 * 60 * 1_000,
...
function parseDuration(input) {
    if (typeof input === "number") {
        if (!Number.isFinite(input) || input < 0) {
            throw new SpendControlError("amount_unparseable", `Duration must be a non-negative finite number, received ${input}`, { input });
        }
        return Math.floor(input);
    }
    const match = DURATION_PATTERN.exec(input);
    if (!match) {
        throw new SpendControlError("amount_unparseable", `Cannot parse duration ${JSON.stringify(input)}; expected e.g. "500ms", "30s", "5m", "1h", "24h", "7d"`, { input });
    }
...
function parseAmount(value, asset) {
    if (typeof value === "bigint") {
        if (value < 0n) {
            throw new SpendControlError("amount_unparseable", `Amount must be non-negative, got ${value}`, { input: value.toString() });
        }
    ...
    if (typeof value === "string") {
        ...
        if (!/^\d+$/.test(trimmed)) {
            throw new SpendControlError("amount_unparseable", `Cannot parse amount ${JSON.stringify(value)}; expected a non-negative integer or an "<asset>:<atomic>" shorthand`, { input: value });
        }
        return { atomic: BigInt(trimmed), asset };
    }
    if (value && typeof value === "object" && "atomic" in value) {
        const innerAsset = asset ?? value.asset;
        return parseAmount(value.atomic, innerAsset);
    }
    throw new SpendControlError("amount_unparseable", `Cannot parse amount: unsupported value of type ${typeof value}`, { input: value });
}
