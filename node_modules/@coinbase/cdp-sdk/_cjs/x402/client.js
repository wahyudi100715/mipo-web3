...
const client_2 = require("@x402/evm/auth-capture/client");
const client_3 = require("@x402/evm/batch-settlement/client");
const client_4 = require("@x402/evm/exact/client");
const client_5 = require("@x402/evm/exact/v1/client");
const client_6 = require("@x402/evm/upto/client");
const builder_code_1 = require("@x402/extensions/builder-code");
const client_7 = require("@x402/svm/exact/client");
const client_8 = require("@x402/svm/exact/v1/client");
const client_9 = require("@x402/svm/upto/client");
...
const setupCdpSigners = async (client, config) => {
    ...
    for (const override of config?.networkSchemes ?? []) {
         ...
         */
        const defaultRpcUrl = override.network === "base"
            ? defaultBaseRpcUrl
    ...
    for (const [network, netConfig] of networksByName) {
        ...
        if (netConfig.scheme.exact) {
            if (isSolana) {
                client.register(caip2Network, new client_7.ExactSvmScheme(svmSigner, { rpcUrl: netConfig.rpcUrl }));
                client.registerV1(network, new client_8.ExactSvmSchemeV1(svmSigner, { rpcUrl: netConfig.rpcUrl }));
            }
            else {
                client.register(caip2Network, new client_4.ExactEvmScheme(evmSigner, evmRpcUrlsByChainId));
                client.registerV1(network, new client_5.ExactEvmSchemeV1(evmSigner));
            }
        }
        if (netConfig.scheme.upto) {
            if (isSolana) {
                client.register(caip2Network, new client_9.UptoSvmScheme(svmSigner, { rpcUrl: netConfig.rpcUrl }));
                /*
                ...
                console.warn(`CdpX402Client: skipping network "${network}": upto scheme is not supported for Smart Contract Wallets (Permit2 approval cannot be sponsored).`);
            }
            else {
                client.register(caip2Network, new client_6.UptoEvmScheme(evmSigner, evmRpcUrlsByChainId));
            }
        }
        if (netConfig.scheme.batchSettlement) {
            if (isSolana) {
                ...
                client.register(caip2Network, new client_3.BatchSettlementEvmScheme(evmSigner, { rpcUrl: netConfig.rpcUrl }));
            }
        }
        if (netConfig.scheme.authCapture) {
            if (isSolana) {
                // eslint-disable-next-line no-console
                console.warn(`CdpX402Client: skipping network "${network}": Solana Auth Capture scheme is not supported (EVM-only upstream).`);
            }
            else if (walletType === "smart") {
                /*
                 ...
                 */
                // eslint-disable-next-line no-console
                console.warn(`CdpX402Client: skipping network "${network}": auth-capture is not supported for Smart Contract Wallets.`);
            }
            else {
                client.register(caip2Network, new client_2.AuthCaptureEvmScheme(evmSigner));
            }
        }
    }
...
class CdpX402Client extends client_1.x402Client {
    ...
    constructor(config) {
        ...
        if (!config?.spendControls) {
            // eslint-disable-next-line no-console
            console.warn("CdpX402Client: spendControls not configured. No default spend cap is applied. " +
