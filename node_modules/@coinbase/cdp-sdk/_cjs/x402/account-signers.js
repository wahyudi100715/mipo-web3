...
exports.fromCdpEvmAccount = fromCdpEvmAccount;
exports.resolveNetworkFromChainId = resolveNetworkFromChainId;
exports.fromCdpSmartWallet = fromCdpSmartWallet;
exports.cdpSolanaAccountToSvmSigner = cdpSolanaAccountToSvmSigner;
 ...
 */
function fromCdpEvmAccount(account) {
    return (0, evm_1.toClientEvmSigner)(account);
}
/**
 ...
 */
function resolveNetworkFromChainId(chainId) {
    if (chainId === undefined) {
        throw new Error("Cannot derive CDP network: domain.chainId is missing from the typed data. " +
            "EIP-712 domain must include chainId when using a Smart Contract Wallet.");
    }
    const network = constants_js_1.CHAIN_ID_TO_CDP_NETWORK[chainId];
    if (!network) {
        throw new Error(`Unsupported chainId ${chainId} for CDP Smart Contract Wallet. ` +
            `Supported networks: ${Object.values(constants_js_1.CHAIN_ID_TO_CDP_NETWORK).join(", ")}`);
    }
    return network;
}
/**
 ...
 * @returns A `ClientEvmSigner` compatible with `@x402/evm`'s `registerExactEvmScheme`
 */
function fromCdpSmartWallet(account) {
    const signerShape = {
    ...
    return (0, evm_1.toClientEvmSigner)(signerShape);
}
/**
 ...
 */
function decodeShortVecLength(bytes) {
    let value = 0;
    let shift = 0;
    let bytesRead = 0;
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function resolveSignerIndex(transaction, signerAddress) {
    ...
    throw new Error(`Unable to locate signer "${signerAddress}" in transaction signatures. ` +
 ...
 */
function cdpSolanaAccountToSvmSigner(account) {
    const signerAddress = (0, kit_1.address)(account.address);
    const signerAddressKey = signerAddress;
    const encoder = (0, kit_1.getTransactionEncoder)();
    return {
        ...
        async signTransactions(transactions) {
            ...
            for (const transaction of transactions) {
                ...
                const { value: signatureCount, bytesRead: signatureSectionOffset } = decodeShortVecLength(signedBytes);
                if (signerIndex >= signatureCount) {
                    throw new Error(`Signer index ${signerIndex} is out of bounds for ${signatureCount} signatures.`);
                }
                ...
                if (signatureEnd > signedBytes.length) {
                    throw new Error("Invalid Solana transaction wire format: signed transaction is missing signature bytes.");
                }
...
//# sourceMappingURL=account-signers.js.map