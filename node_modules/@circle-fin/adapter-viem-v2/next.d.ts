...
import { Chain, PublicClient, WalletClient, Abi as Abi$1, TransactionReceipt, EIP1193Provider } from 'viem';
...
interface RetryOptions {
    ...
    /**
     * Whether to add jitter (±25% randomness) to the delay.
     * @defaultValue true
     */
    jitter?: boolean;
    /**
     ...
     *
     * @remarks
     * If not provided, uses the default from `@core/errors` which
...
/**
 * Transaction-related behavioral settings.
 *
 * @remarks
 * Controls how transactions are priced and confirmed.
 * All fields are optional — each adapter falls back to its own
 * defaults when a value is not provided.
 *
 * | Setting | Used by | Default |
 * |---------|---------|---------|
 * | `feePriceBufferBps` | execute (EVM), calculateFee | 2 000 (20%) |
 * | `minPriorityFeeWei` | execute (EVM) | 1.5 gwei |
 * | `confirmationTimeoutMs` | waitForTransaction | client default |
 *
 * **EVM adapters** — `feePriceBufferBps` is injected into viem's
 * `chain.fees.baseFeeMultiplier` (e.g. 2 000 bps → 1.2×). This
 * preserves viem's full gas handling pipeline while applying the
 * buffer natively. A `minPriorityFeeWei` floor is enforced as a
 * safety net for chains/RPCs that report near-zero priority fees.
 *
 * @example
 * ```typescript
 * import type { TransactionConfig } from '@core/runtime'
 *
 * const txConfig: TransactionConfig = {
 *   feePriceBufferBps: 2000n,       // 20% buffer on gas pricing
 *   confirmationTimeoutMs: 120_000, // 2 min confirmation timeout
 * }
 * ```
 */
interface TransactionConfig {
    /**
     * Buffer applied to gas/fee pricing in basis points.
     *
     * @remarks
     * Protects against price fluctuations between estimation and block
     * inclusion.
     *
     * **EVM (viem)** — maps to `chain.fees.baseFeeMultiplier`, which
     * buffers the base fee (EIP-1559) or legacy gas price natively
     * through viem's pipeline.
     *
     * **Fee estimation** — used as the default buffer for
     * `calculateFee` when `bufferBasisPoints` is not passed per-call.
     *
     * - 500 = 5%
     * - 2000 = 20%
     * - 5000 = 50%
     *
     * @defaultValue `2000n` (20%) — matches the MetaMask default and
     * provides reliable block inclusion with free/public RPCs.
     */
    readonly feePriceBufferBps?: bigint | undefined;
    /**
     * Minimum priority fee floor in wei (EVM only).
     *
     * @remarks
     * Free/public RPCs frequently report a `maxPriorityFeePerGas` near
     * zero. On chains with near-zero base fees (e.g. Avalanche where
     * `baseFeePerGas` can be 1 wei), the entire gas price depends on the
     * priority fee alone. Without a floor, transactions are silently
     * dropped by validators.
     *
     * This floor is applied as the minimum value for
     * `maxPriorityFeePerGas`. If the RPC reports a higher value, the
     * RPC value is used instead.
     *
     * On EVM, unused gas is always refunded — only `gasUsed *
     * effectiveGasPrice` is charged. A generous floor increases
     * willingness-to-pay but not actual cost when the network price is
     * lower.
     *
     * @defaultValue `1_500_000_000n` (1.5 gwei)
     */
    readonly minPriorityFeeWei?: bigint | undefined;
    /**
     * Default timeout for waiting for transaction confirmation in milliseconds.
     *
     * @remarks
     * Applied by `waitForTransaction` when no per-call `timeout` is
     * specified. When omitted, the underlying client's default is used.
     *
     * @defaultValue `undefined` (client default)
     */
    readonly confirmationTimeoutMs?: number | undefined;
}
/**
 * Operational configuration for behavioral settings.
 *
 * @remarks
 * Groups operational behavior settings that control how operations execute.
 * This type is used at multiple levels of the stack:
 *
 * - **Adapter level**: Default config for all operations via `AdapterContext.config`
 * - **Invocation level**: Per-call overrides via `InvocationMeta.config`
 *
 * **Design Rationale**
 *
 * A single `OperationalConfig` type ensures consistency across the stack.
 * Settings cascade from adapter defaults → invocation overrides.
 *
 * This design is extensible - future settings (timeout, circuit breaker, etc.)
 * can be added without polluting higher-level interfaces.
 *
 * @example
 * ```typescript
 * import type { OperationalConfig } from '@core/runtime'
 *
 * // Adapter-level defaults
 * const adapterConfig: OperationalConfig = {
 *   retry: { maxAttempts: 3, baseDelayMs: 1000 },
 *   transaction: { feePriceBufferBps: 1000n },
 * }
 *
 * // Per-call override
 * const callConfig: OperationalConfig = {
 *   retry: { maxAttempts: 5 },
 * }
 * ```
 */
interface OperationalConfig {
     ...
     *
     * @remarks
     * Retry is **enabled by default** with exponential backoff for retryable
     * errors (network failures, rate limits, etc.). Pass a `RetryOptions`
     * object to customise behaviour, or `false` to disable retries entirely.
     */
    readonly retry?: RetryOptions | false | undefined;
    /**
     * Transaction pricing, caching, and confirmation settings.
     *
     * @remarks
     * Controls gas/fee price buffering, cache TTL, and confirmation
     * timeouts. Each primitive falls back to sensible defaults when
     * individual fields are omitted.
     */
    readonly transaction?: TransactionConfig | undefined;
}
/**
...
/**
 * Context passed to error normalizers for richer error messages.
 *
 * @remarks
 * A partial view of execution context used when normalizing errors.
 * All fields are optional since errors can occur at various points in
 * the execution lifecycle where not all context is available.
 */
interface ErrorContext {
    ...
    /**
     * Observability tags for the operation.
     *
     * @remarks
     * This is where the canonical `chain` dimension lives — it is populated
     * by `buildOperationTags` on the execution context. Error normalizers
     * should read the chain name from here (tags-first) and treat
     * {@link ErrorContext.meta} only as a fallback overlay.
     */
    readonly tags?: Tags | undefined;
    /** Additional metadata (chain, adapter, etc.). */
    readonly meta?: Record<string, unknown> | undefined;
}
/**
 * Function signature for error normalizers.
 *
 * @remarks
 * Normalizers inspect an error and either:
 * - **Throw a KitError** if they recognize the error type
 * - **Return void** to allow fallback handling
 *
 * This design is simpler than returning intermediate types because:
 * - Adapters use `@core/errors` factories directly
 * - No intermediate type conversion needed
 * - Clear control flow (throw = handled, return = not handled)
 *
 * **Adapter implementors** should handle at least these categories:
 *
 * | Category | Example framework errors | Maps to (`@core/errors`) |
 * |----------|--------------------------|--------------------------|
 * | Revert | `ContractFunctionRevertedError` | `createTransactionRevertedError` |
 * | Insufficient funds | `InsufficientFundsError` | `createInsufficientFundsError` |
 * | User rejection | `UserRejectedRequestError` | `createUserRejectedError` |
 * | RPC / transport | `HttpRequestError`, `TimeoutError` | `createRpcError` |
 * | Nonce conflict | `NonceTooLowError` | `createNonceError` |
 *
 * @param err - The error to normalize.
 * @param ctx - Optional context for richer error messages.
 *
 * @example
 * ```typescript
 * import type { ErrorNormalizer } from '@core/runtime'
 * import { createTransactionRevertedError } from '@core/errors'
 *
 * export const viemErrorNormalizer: ErrorNormalizer = (err, ctx) => {
 *   if (err instanceof ContractFunctionRevertedError) {
 *     throw createTransactionRevertedError(
 *       ctx?.chain ?? 'Unknown',
 *       err.reason ?? 'Transaction reverted',
 *       { rawError: err }
 *     )
 *   }
 *   // Not recognized - let fallback handle it
 * }
 * ```
 */
type ErrorNormalizer = (err: unknown, ctx?: ErrorContext) => void;
...
/**
 * Signing strategy contract for pluggable transaction authorization.
 *
 * @remarks
 * A {@link SigningStrategy} sits between an adapter's transaction
 * preparation and submission: the adapter builds a ready-to-sign
 * {@link TransactionPayload} and hands it to the strategy, which decides
 * how the transaction gets authorized — sign it in-process, hand it to
 * the caller's own infrastructure, or ask a remote service.
 *
 * The contract is intentionally minimal and synchronous-in-process: the
 * strategy's `execute()` resolves within the lifetime of the calling
 * process. Long-running flows (multi-sig quorums, challenge approvals
 * that survive restarts) are a future, additive extension and are out of
 * scope here.
 *
 * @packageDocumentation
 */
...
/**
 * A single pre-encoded EVM call within a {@link EvmCallsPayload}.
 */
interface EvmCall {
    /** The target contract or recipient address. */
    readonly to: `0x${string}`;
    /** ABI-encoded calldata. Omitted for plain native-value transfers. */
    readonly data?: `0x${string}` | undefined;
    /** Native token value to send with the call, in wei. */
    readonly value?: bigint | undefined;
}
/**
 * ERC-20 approval context attached to a {@link EvmCallsPayload}.
 *
 * @remarks
 * When present, the payload's calls require the given ERC-20 allowance
 * to succeed. It is informational for the strategy: the layer that
 * builds the payload remains responsible for sequencing any `approve`
 * transaction. A strategy may use it to display or verify what the
 * signer is consenting to.
 */
interface EvmApprovalContext {
    /** The ERC-20 token contract address requiring approval. */
    readonly token: `0x${string}`;
    /** The spender being approved (e.g. a router or token messenger). */
    readonly spender: `0x${string}`;
    /** The required allowance in base units. */
    readonly amount: bigint;
}
/**
 * Pre-encoded EVM call batch — the `evm-calls` payload variant.
 *
 * @remarks
 * Produced by an adapter at the moment a step's transaction becomes
 * buildable, and consumed by a {@link SigningStrategy}. The shape is the
 * single envelope every strategy receives for EVM execution, whether the
 * signer is in-process, the caller's own infrastructure, or a remote
 * service.
 */
interface EvmCallsPayload {
    /** Discriminator for the {@link TransactionPayload} union. */
    readonly type: 'evm-calls';
    /** The target EVM chain for execution. */
    readonly chain: EVMChainDefinition;
    /** Pre-encoded calls to execute, in order. */
    readonly calls: readonly EvmCall[];
    /** Optional ERC-20 approval context the calls depend on. */
    readonly approval?: EvmApprovalContext | undefined;
    /**
     * The address expected to authorize and send the calls.
     *
     * @remarks
     * A strategy that signs must sign as this address. The adapter
     * validates the {@link SignedSigningResult.signerAddress} echo against
     * this field before submitting, preventing "valid signature, wrong
     * account" bugs.
     */
    readonly fromAddress: `0x${string}`;
}
/**
 * A single field of an EIP-712 struct definition.
 */
interface EvmTypedDataField {
...
/**
 * EIP-712 domain separator carried in a {@link EvmTypedDataPayload}.
 *
 * @remarks
 * Every field is optional: some verifiers bind replay protection into
 * the signed message body instead (e.g. Gateway burn intents use a
 * domain with only `name` and `version`).
 */
interface EvmTypedDataDomain {
    /** Human-readable name of the signing domain (e.g. `'USD Coin'`). */
    readonly name?: string | undefined;
    /** Major version of the signing domain (e.g. `'2'`). */
    readonly version?: string | undefined;
    /** EVM chain ID where the verifying contract is deployed. */
    readonly chainId?: number | bigint | undefined;
...
/**
 * The EIP-712 structure inside a {@link EvmTypedDataPayload}.
 */
interface EvmTypedData {
    /** EIP-712 domain separator. */
    readonly domain: EvmTypedDataDomain;
    /** Mapping of struct names to their field definitions. */
    readonly types: Record<string, readonly EvmTypedDataField[]>;
    /** The root struct type being signed (a key of `types`). */
    readonly primaryType: string;
    /** The message payload to be signed. */
    readonly message: Record<string, unknown>;
}
/**
 * EIP-712 typed-data signing request — the `evm-typed-data` payload
 * variant.
 *
 * @remarks
 * Produced when a provider needs a detached signature rather than a
 * transaction: EIP-2612 permits (swap), ERC-3009 authorizations (earn),
 * Gateway burn intents. Nothing is broadcast — the strategy resolves
 * with a {@link SignatureSigningResult} and the layer that requested the
 * signature decides what to do with it.
 */
interface EvmTypedDataPayload {
    /** Discriminator for the {@link TransactionPayload} union. */
    readonly type: 'evm-typed-data';
    /** The EVM chain the signature is intended for. */
    readonly chain: EVMChainDefinition;
    /** The EIP-712 structure to sign. */
    readonly typedData: EvmTypedData;
    /**
     * The address expected to produce the signature.
     *
     * @remarks
     * The adapter validates the {@link SignatureSigningResult.signerAddress}
     * echo against this field before returning the signature.
     */
    readonly fromAddress: `0x${string}`;
}
/**
 * The transaction envelope handed to a {@link SigningStrategy}.
 *
 * @remarks
 * A discriminated union over payload families. Strategies `switch` on
 * `type`; adding a new variant (e.g. `solana-message`) is a compile
 * error for strategies that do not handle it exhaustively.
 */
type TransactionPayload = EvmCallsPayload | EvmTypedDataPayload;
/**
 * A payload family a {@link SigningStrategy} can declare support for.
 *
 * @remarks
 * The discriminator values of {@link TransactionPayload}, used by
 * {@link SigningStrategyManifest.supportedPayloadFamilies} to
 * capability-gate dispatch before any payload is built.
 */
type SigningPayloadFamily = TransactionPayload['type'];
/**
 * Describes the operation a payload belongs to.
 *
 * @remarks
 * Lets a strategy present meaningful context to whoever authorizes the
 * transaction (a human approving in a UI, an audit log, a policy
 * engine) without parsing calldata.
 */
interface SigningIntent {
    /** The kit-level action, e.g. `'bridge'` or `'swap'`. */
    readonly action: string;
    /** The step within the action, e.g. `'approve'`, `'burn'`, `'mint'`. */
    readonly step?: string | undefined;
}
/**
 * Per-invocation context passed alongside a {@link TransactionPayload}.
 */
interface SigningContext {
    /** What this transaction is for. See {@link SigningIntent}. */
    readonly intent: SigningIntent;
    /**
     * Cooperative cancellation signal.
     *
     * @remarks
     * Strategies must check the signal before performing irreversible
     * work (signing, broadcasting) and should abort in-progress waits
     * when it fires. Once a transaction is broadcast, aborting cannot
     * recall it.
     */
    readonly signal?: AbortSignal | undefined;
}
/**
 * Result of a strategy that signed and broadcast the transaction itself.
 *
 * @remarks
 * The adapter must not submit anything for this payload — doing so
 * would double-spend. Strategies returning this shape declare
 * `broadcasts: true` in their {@link SigningStrategyManifest}.
 */
interface BroadcastedSigningResult {
    /** Discriminator for the {@link SigningResult} union. */
    readonly type: 'broadcasted';
    /** Hash of the already-broadcast transaction. */
    readonly txHash: `0x${string}`;
    /** Optional echo of the address that authorized the transaction. */
    readonly signerAddress?: `0x${string}` | undefined;
}
/**
 * Result of a strategy that signed but did not broadcast.
 *
 * @remarks
 * The adapter submits the signed bytes (e.g. via
 * `eth_sendRawTransaction`) after validating `signerAddress` against
 * the payload's `fromAddress`.
 */
interface SignedSigningResult {
    /** Discriminator for the {@link SigningResult} union. */
    readonly type: 'signed';
    /** The serialized, signed transaction ready for submission. */
    readonly signedTransaction: `0x${string}`;
    /**
     * The address that produced the signature.
     *
     * @remarks
     * Required: the adapter rejects the result when it does not match the
     * payload's `fromAddress`.
     */
    readonly signerAddress: `0x${string}`;
}
/**
 * Result of a strategy that produced a detached signature.
 *
 * @remarks
 * The response to a {@link EvmTypedDataPayload}: nothing was broadcast
 * and nothing will be — the adapter returns the signature verbatim to
 * whoever requested it.
 *
 * The signature is opaque bytes of any length. It is never parsed or
 * split into v/r/s components by the adapter: contract-wallet (ERC-1271)
 * signers produce signatures longer than the 65 bytes ECDSA allows, and
 * the deployed USDC verifiers accept them via their `bytes signature`
 * overloads.
 */
interface SignatureSigningResult {
    /** Discriminator for the {@link SigningResult} union. */
    readonly type: 'signature';
    /** The detached signature over the requested payload. */
...
/**
 * What a {@link SigningStrategy} returns from `execute()`.
 */
type SigningResult = BroadcastedSigningResult | SignedSigningResult | SignatureSigningResult;
/**
 * Static description of a {@link SigningStrategy}.
 *
 * @remarks
 * Read by the adapter before any payload is built:
 * `supportedPayloadFamilies` capability-gates dispatch — a request for
 * an unsupported family fails with a clear error instead of reaching the
 * strategy — and `broadcasts` tells the adapter whether to submit the
 * result itself, letting it operate without a wallet client at all when
 * the strategy owns both signing and submission.
 */
interface SigningStrategyManifest {
    /** Stable identifier for diagnostics and events, e.g. `'external'`. */
    readonly name: string;
    /**
     * The payload families the strategy can handle.
     *
     * @remarks
     * One entry per {@link TransactionPayload} variant the strategy's
     * `execute()` accepts. New families extend this list rather than
     * adding per-surface booleans.
     */
    readonly supportedPayloadFamilies: readonly SigningPayloadFamily[];
    /**
     * Whether the strategy submits transactions to the network itself.
     *
     * @remarks
     * When `true`, the strategy resolves `evm-calls` payloads with a
     * {@link BroadcastedSigningResult} and the adapter must not submit.
     * Detached-signature payloads are unaffected — nothing is ever
     * broadcast for them.
     */
    readonly broadcasts: boolean;
    /**
     * Whether the strategy executes a multi-call `evm-calls` payload as a
     * single atomic unit.
     *
     * @remarks
     * Not a payload family — it qualifies `evm-calls`: when `true`, the
     * adapter may hand the strategy one payload whose `calls` carry an
     * entire batch (e.g. approve + burn) and the strategy resolves with a
     * single transaction outcome for the whole batch (a smart-contract
     * account's `executeBatch`, an EIP-5792 wallet, a bundler). When
     * absent or `false`, payloads always carry exactly one call.
     *
     * @defaultValue `false`
     */
    readonly atomicBatch?: boolean | undefined;
}
/**
 * Pluggable transaction authorization.
 *
 * @remarks
 * Implementations decide how a built, unsigned transaction gets
 * authorized and (optionally) submitted. Everything else — building the
 * transaction, sequencing steps, waiting for confirmations — stays with
 * the adapter and kits.
 *
 * @example
 * ```typescript
 * const logOnly: SigningStrategy = {
 *   manifest: {
 *     name: 'my-signer',
 *     supportedPayloadFamilies: ['evm-calls'],
 *     broadcasts: true,
 *   },
 *   async execute(payload, ctx) {
 *     const txHash = await myOwnFlow(payload, ctx.signal)
 *     return { type: 'broadcasted', txHash }
 *   },
 * }
 * ```
 */
interface SigningStrategy {
    /** Static description of the strategy. See {@link SigningStrategyManifest}. */
    readonly manifest: SigningStrategyManifest;
    /**
     * Authorize (and, per the manifest, optionally submit) a transaction.
     *
     * @remarks
     * When the signer deliberately declines (a denied PIN challenge, a
     * dismissed approval prompt), reject with the error from
     * `createSigningRejectedError` so callers can tell "the signer said
     * no" apart from infrastructure failure.
     *
     * @param payload - The ready-to-sign transaction envelope.
     * @param ctx - Cancellation and intent context for this invocation.
     * @returns The signing outcome. See {@link SigningResult}.
     */
    execute(payload: TransactionPayload, ctx: SigningContext): Promise<SigningResult>;
}

/**
 * Outcome of an external `sign` callback that broadcast the transaction
 * itself.
 */
interface ExternalBroadcastOutcome {
    /** Hash of the transaction the caller already broadcast. */
    readonly txHash: `0x${string}`;
    /** Optional echo of the address that authorized the transaction. */
    readonly signerAddress?: `0x${string}` | undefined;
}
/**
 * Outcome of an external `sign` callback that signed but did not
 * broadcast — the adapter submits the bytes.
 */
interface ExternalSignedOutcome {
    /** The serialized, signed transaction ready for submission. */
    readonly signedTransaction: `0x${string}`;
    /** The address that produced the signature. */
    readonly signerAddress: `0x${string}`;
...
/**
 * What an external `sign` callback may resolve with.
 */
type ExternalSigningOutcome = ExternalBroadcastOutcome | ExternalSignedOutcome;
/**
 * Outcome of an external `signTypedData` callback — a detached EIP-712
 * signature, nothing broadcast.
 */
interface ExternalTypedDataOutcome {
    /**
     * The detached signature over the typed data.
     *
     * @remarks
     * Opaque bytes of any length — contract-wallet (ERC-1271) signatures
     * exceed the 65 bytes ECDSA allows and are passed through untouched.
     */
    readonly signature: `0x${string}`;
    /** The address that produced the signature. */
    readonly signerAddress: `0x${string}`;
...
/**
 * Options for {@link externalSigning}.
 */
interface ExternalSigningOptions {
    /**
     * The integrator's signing flow for transactions.
     *
     * @remarks
     * Called once per transaction, at the moment the payload becomes
     * buildable, with the ready-to-sign envelope and the invocation
     * context (intent, cancellation signal). Resolve with a
     * {@link ExternalSigningOutcome} matching the declared
     * {@link ExternalSigningOptions.broadcasts | broadcasts} mode.
     */
    readonly sign: (payload: EvmCallsPayload, ctx: SigningContext) => Promise<ExternalSigningOutcome>;
    /**
     * The integrator's signing flow for EIP-712 typed data.
     *
     * @remarks
     * Optional — providing it adds `evm-typed-data` to the strategy's
     * supported payload families, which kits need for detached-signature
     * authorization (EIP-2612 permits, ERC-3009 authorizations, Gateway
     * burn intents). When omitted, typed-data requests are rejected at
     * dispatch with a capability error before this strategy is invoked.
     */
    readonly signTypedData?: ((payload: EvmTypedDataPayload, ctx: SigningContext) => Promise<ExternalTypedDataOutcome>) | undefined;
    /**
     * Whether the `sign` callback broadcasts the transaction itself.
     *
     * @remarks
     * Declared up front because the adapter reads it from the manifest
     * before any payload exists (e.g. to allow operating without a wallet
     * client). The strategy enforces the declaration at runtime: a
     * callback that resolves with the other outcome shape is rejected
     * rather than risking a double-broadcast or a never-submitted
     * transaction.
     *
     * @defaultValue `true`
     */
    ...
    /**
     * Whether the caller's signer executes multi-call payloads atomically.
     *
     * @remarks
     * Declare `true` when the signing flow behind `sign` can execute an
     * entire batch as one unit — a smart-contract account's
     * `executeBatch`, an EIP-5792 wallet, a bundler. The `sign` callback
     * may then receive payloads whose `calls` carry the whole batch (e.g.
     * approve + burn) and must resolve with the single outcome for all of
     * them. When omitted, every payload carries exactly one call.
     *
     * @defaultValue `false`
     */
    readonly atomicBatch?: boolean | undefined;
 ...
 * ```
 */
declare function externalSigning(options: ExternalSigningOptions): SigningStrategy;
 ...
 * ```
 */
declare function createSigningRejectedError(strategyName: string, intent: SigningIntent, trace?: Record<string, unknown>): KitError;
/**
 ...
 * ```
 */
declare function isSigningRejected(error: unknown): boolean;

/**
 * Base shape for an ecosystem-owned authorization payload.
 *
 * @remarks
 * Ecosystems define their own discriminated payload unions and bind them to
 * the generic authorization contract. EVM uses `TransactionPayload`; Solana
 * can add an independent payload later without widening the EVM union.
 */
interface AuthorizationPayload {
    /** Ecosystem-owned payload discriminator. */
    readonly type: string;
}
/**
 * Optional semantic data attached to an authorization request.
 *
 * @remarks
 * Review kinds are stable, namespaced identifiers such as `earn.execute`.
 * The data remains `unknown` because adapter core cannot depend on kit types.
 */
interface AuthorizationReview {
    /** Stable, namespaced review discriminator. */
    readonly kind: string;
    /** Kit-owned semantic review data. */
    readonly data: unknown;
}
/** Create optional semantic review data from an isolated payload snapshot. */
type AuthorizationReviewFactory<TPayload extends AuthorizationPayload> = (payload: TPayload) => AuthorizationReview | undefined | Promise<AuthorizationReview | undefined>;
/**
 * Request-scoped authorization metadata supplied by a kit or provider.
 *
 * @typeParam TPayload - The ecosystem authorization payload type.
 */
interface AuthorizationDescriptor<TPayload extends AuthorizationPayload> {
    /** Optional intent override for this authorization unit. */
    readonly intent?: SigningIntent | undefined;
    /** Optional lazy semantic review factory. */
    readonly createReview?: AuthorizationReviewFactory<TPayload> | undefined;
}
/**
 * Immutable request passed to an adapter authorization-review hook.
 *
 * @typeParam TPayload - The ecosystem authorization payload type.
 */
interface AuthorizationRequest<TPayload extends AuthorizationPayload> {
    /** What the authorization unit is intended to accomplish. */
    readonly intent: SigningIntent;
    /** Isolated snapshot of the canonical payload being authorized. */
    readonly payload: TPayload;
    /** Optional kit-supplied semantic review data. */
    readonly review?: AuthorizationReview | undefined;
    /** Optional cooperative cancellation signal. */
    readonly signal?: AbortSignal | undefined;
}
/** Explicit decision returned by an authorization-review hook. */
type AuthorizationDecision = 'approve' | 'reject';
/**
 * Hook invoked immediately before an adapter asks a wallet or signer to
 * authorize a payload.
 *
 * @typeParam TPayload - The ecosystem authorization payload type.
 */
type OnBeforeAuthorize<TPayload extends AuthorizationPayload> = (request: AuthorizationRequest<TPayload>) => Promise<AuthorizationDecision>;
/**
 * Adapter configuration for authorization review.
 *
 * @typeParam TPayload - The ecosystem authorization payload type.
 */
interface AuthorizationOptions<TPayload extends AuthorizationPayload> {
    /** Optional awaited, fail-closed authorization-review hook. */
    readonly onBeforeAuthorize?: OnBeforeAuthorize<TPayload> | undefined;
}
/**
 * Adapter-owned invocation metadata that remains request scoped.
 *
 * @remarks
 * This deliberately does not use `ActionOptions.meta`, which is emitted to
 * observability middleware and must never carry callback closures.
 *
 * @typeParam TPayload - The ecosystem authorization payload type.
 */
interface AdapterInvocationMeta<TPayload extends AuthorizationPayload = AuthorizationPayload> extends InvocationMeta {
    /** Optional authorization descriptor for this invocation. */
    readonly authorization?: AuthorizationDescriptor<TPayload> | undefined;
}
/**
 * Resolved adapter invocation context with request-scoped authorization data.
 *
 * @typeParam TPayload - The ecosystem authorization payload type.
 */
interface AdapterInvocationContext<TPayload extends AuthorizationPayload = AuthorizationPayload> extends InvocationContext {
    /** Optional authorization descriptor for this invocation. */
    readonly authorization?: AuthorizationDescriptor<TPayload> | undefined;
}
/**
 * Input accepted by an adapter context's shared authorization gate.
 *
 * @typeParam TPayload - The ecosystem authorization payload type.
 */
interface AuthorizeInput<TPayload extends AuthorizationPayload> {
    /** Canonical payload that will be authorized downstream. */
    readonly payload: TPayload;
    /** Fallback intent when the descriptor does not override it. */
    readonly intent: SigningIntent;
    /** Optional request-scoped descriptor. */
    readonly descriptor?: AuthorizationDescriptor<TPayload> | undefined;
    /** Optional cooperative cancellation signal. */
    readonly signal?: AbortSignal | undefined;
}
/** Shared authorization gate exposed by an adapter context. */
type Authorize<TPayload extends AuthorizationPayload> = (input: AuthorizeInput<TPayload>) => Promise<void>;
 ...
 * ```
 */
declare function createAuthorizationRejectedError(intent: SigningIntent): KitError;

/**
 * Context types for adapter operations.
 *
 * @remarks
 * This module provides the core context types for adapter operations.
 *
 * **Mental Model**
 *
 * - **WHAT**: `OperationContext` defines the operation target (chain, address)
 * - **WHO/HOW**: `InvocationContext` defines tracing and runtime (traceId, runtime, tokens, callers)
 *
 * @packageDocumentation
 */

/**
 * Defines the capabilities of an adapter.
 *
 * @remarks
 * The `addressContext` property determines address handling:
 * - `user-controlled`: Address resolved from wallet (e.g., MetaMask, private key)
 * - `developer-controlled`: Address must be explicitly provided (e.g., Fireblocks)
 */
interface AdapterCapabilities$1 {
    /**
     * Defines who controls address selection.
     *
     * @remarks
     * - `user-controlled`: User controls via wallet UI; address resolved automatically
     * - `developer-controlled`: Address must be explicitly provided per operation
     */
    addressContext: 'user-controlled' | 'developer-controlled';
    /**
     * The blockchain networks this adapter supports.
     */
    supportedChains: readonly ChainDefinition[];
}
/**
 * Adapter identification for telemetry and debugging.
 */
interface AdapterIdentity {
    /** Name of the adapter (e.g., 'viem', 'ethers'). */
    readonly name: string;
    /** Version of the adapter. */
    readonly version: string;
}
/**
 * Policy for handling wallet chain mismatches before write operations.
 *
 * @remarks
 * - `'prompt'` (default for user-controlled adapters): ask the wallet to
 *   switch chains (and add the chain when supported).
 * - `'throw'`: fail fast with a typed `CHAIN_MISMATCH` error without
 *   prompting. Useful for server-driven integrations that want to
 *   surface a clean failure rather than interrupt the user flow.
 * - `'skip'`: disable the hook entirely. Factories for developer-controlled
 *   or private-key signers default to this mode because the chain is
 *   pinned by the factory itself.
 */
type ChainSwitchOnMismatch = 'prompt' | 'throw' | 'skip';
/**
 * Adapter-level configuration for the `ensureChain` hook.
 *
 * @remarks
 * Adapter factories forward this object to their context factory. Adapter
 * packages own the actual chain-switching implementation — this config is
 * the knob consumers use to opt in to alternative policies.
 *
 * @example
 * ```typescript
 ...
 * ```
 */
interface ChainSwitchConfig {
    /**
     * Policy applied when the wallet reports a different chain than the
     * operation targets.
     *
     * @defaultValue `'prompt'` for user-controlled factories, `'skip'`
     *   for developer-controlled / private-key factories.
     */
    readonly onMismatch?: ChainSwitchOnMismatch | undefined;
}
/**
 * Adapter context containing runtime, capabilities, and utilities.
 *
 * @typeParam TCapabilities - The adapter capabilities type.
 *
 * @remarks
 * This is the adapter's setup/configuration object. It's bound to the adapter
 * instance and provides access to runtime services, capabilities, identity,
 * and shared resources like token registry.
 *
 * @example
 * ```typescript
 * import type { AdapterContext } from '@core/adapter-base'
 *
 * const ctx: AdapterContext = {
 *   identity: { name: 'viem', version: '1.0.0' },
 *   capabilities: { addressContext: 'user-controlled', supportedChains: [...] },
 ...
 * ```
 */
interface AdapterContext$1<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1, TAuthorizationPayload extends AuthorizationPayload = AuthorizationPayload> {
    /** Adapter identity for telemetry. */
    ...
    /** Token registry for resolving token information. */
    readonly tokens: TokenRegistry;
    /** Error normalizer for this adapter. */
    readonly normalizeError: ErrorNormalizer;
    /**
     * Get the current address for a chain.
     *
     * @remarks
     * Returns the address that will be used as the sender/signer for operations.
     * Implementation depends on the adapter type (wallet connection, key management, etc.).
     */
    getAddress: (chain: ChainDefinition) => Promise<string>;
    /**
     * Validate that a chain is supported by this adapter.
     *
     * @remarks
     * Throws a `KitError` with `INVALID_CHAIN` code if the chain is not
     * in `capabilities.supportedChains`. Automatically provided by
     * `createAdapterContext`.
     */
    readonly validateChainSupport: (chain: ChainDefinition) => void;
    /**
     * Optional operational configuration for this adapter.
     *
     * @remarks
     * Contains behavioral settings like retry configuration that apply to
     * all operations executed through this adapter.
     */
    readonly config?: OperationalConfig | undefined;
    /**
     * Ensure the wallet is on the target chain before a write operation.
     *
     * @remarks
     * Populated by adapters that can actually switch chains (EVM wallet
     * adapters). Read-only primitives must never invoke it — only
     * primitives that opt in via `ensureChainBefore: true` on their
     * `createPrimitive` options will trigger this hook.
     *
     * Implementations are expected to be idempotent: calling `ensureChain`
     * when the wallet is already on `chain` must not surface a prompt.
     */
    readonly ensureChain?: ((chain: ChainDefinition) => Promise<void>) | undefined;
    /** Review a canonical payload immediately before wallet authorization. */
    readonly authorize: Authorize<TAuthorizationPayload>;
}
/**
 * Extract the address context from adapter capabilities.
 *
 * @typeParam TCapabilities - The adapter capabilities type.
 * @returns The address context type ('user-controlled' | 'developer-controlled').
 */
type ExtractAddressContext$1<TCapabilities extends AdapterCapabilities$1> = TCapabilities extends {
    addressContext: infer TContext;
} ? TContext : never;
/**
 * Conditional address field based on adapter capabilities.
 *
 * @typeParam TAddressContext - The address context type.
 * @returns Address field type:
 *   - `user-controlled` → `address?: never` (forbidden)
 *   - `developer-controlled` → `address: string` (required)
 *   - Unknown → `address?: string` (optional, legacy)
 */
type AddressField$1<TAddressContext> = TAddressContext extends 'user-controlled' ? {
    address?: never;
...
/**
 * Capability-aware operation meta that enforces address requirements.
 *
 * @typeParam TCapabilities - The adapter capabilities type.
 *
 * @remarks
 * Use this when you need compile-time enforcement of address requirements.
 * For simpler cases, use {@link OperationMeta}.
 */
type CapabilityAwareOperationMeta<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> = {
    readonly chain: ChainIdentifier$1;
} & AddressField$1<ExtractAddressContext$1<TCapabilities>>;
/**
 * User input specifying the operation target.
 *
 * @remarks
 * Defines **WHAT** we're operating on: which chain and optionally which address.
 * This is the user-facing input type, resolved to `OperationContext` internally.
 *
 * @example
 * ```typescript
 * // Minimal input
 * const meta: OperationMeta = { chain: 'Ethereum' }
 *
...
interface OperationMeta {
     ...
     *
     * @remarks
     * Accepts chain name (string) or full ChainDefinition.
     * Resolved to ChainDefinition internally.
     */
    readonly chain: ChainIdentifier$1;
    /**
     * Sender/signer address (optional).
     *
     * @remarks
     * - User-controlled adapters: Resolved from wallet if not provided
     * - Developer-controlled adapters: Required
     */
...
/**
 * Types for the createPrepare factory.
 *
 * @packageDocumentation
 */

/**
 * Result of sequentially executing multiple transactions.
 *
 * @remarks
 * When executing multiple transactions (e.g., USDT approve reset → set),
 * this result shows what completed and where any failure occurred.
 *
 * @typeParam TRaw - Ecosystem-specific raw data type.
 */
interface SequentialExecuteResult<TRaw> {
    /**
     ...
     *
     * @remarks
     * If success is false, the last confirmation is the failed one.
     */
    readonly results: Confirmation<TRaw>[];
    /**
     * Index of the failed transaction (0-based), if any.
     */
    readonly failedIndex?: number | undefined;
}
/**
 * Aggregated fee estimate for multiple transactions.
 */
interface AggregateFeeEstimate {
    /**
     * Total fee across all transactions.
     *
     * @remarks
     * Access `.raw` for the bigint value.
     *
     * @example
     * ```typescript
     * const { totalFee, count } = await result.estimateTotal()
     * console.log(`Total: ${totalFee.toString()} for ${count} transactions`)
     * ```
     */
    readonly totalFee: Amount;
    /**
     ...
     */
    readonly estimates: FeeEstimate[];
}
/**
 * Options for executing prepared transactions.
 */
interface ExecuteOptions {
    /**
     * Called after each transaction is sent (before confirmation).
     *
     * @param txId - The transaction ID/hash.
     * @param index - The zero-based index of the transaction in the batch.
     */
    onTxSent?: ((txId: string, index: number) => void) | undefined;
}
/**
 * Result of preparing one or more transactions.
 *
 * @remarks
 * This wrapper provides a unified interface regardless of whether
 * the prepare function returned a single transaction or multiple.
 *
 * **Key Features:**
 * - `transactions` - Always an array of the specified prepared transaction type
 * - `single()` - Convenience for single-transaction case (returns TPrepared)
 * - `estimateTotal()` - Aggregate fee estimation
 * - `execute()` - Sequential execution with proper error handling
 *
 * **Custom Prepared Transaction Types:**
 * The `TPrepared` type parameter allows ecosystems to use their own prepared
 * transaction interfaces while maintaining type safety throughout.
 *
 * @typeParam TInput - The user-provided input type.
 * @typeParam TBuilt - The built transaction type.
 * @typeParam TRaw - The ecosystem-specific raw data type.
 * @typeParam TPrepared - The prepared transaction type (defaults to base PreparedTransaction).
 *
 * @example
 * ```typescript
 * // With custom prepared transaction type
 * const result: PrepareResult<Input, Built, Raw, PreparedEVMTransaction> = await prepare(...)
 *
 * // single() returns PreparedEVMTransaction with proper types
 * const tx = result.single()
 * const fee = await tx.estimate()  // Returns EstimateResult
 * ```
 */
interface PrepareResult<TInput, TBuilt, TRaw, TPrepared extends PreparedTransactionBase = PreparedTransaction<TInput, TBuilt, TRaw>, TAuthorizationPayload extends AuthorizationPayload = AuthorizationPayload> {
    /**
     * All prepared transactions.
     *
     * @remarks
     * Returns array of TPrepared - the custom prepared transaction type.
     */
    readonly transactions: TPrepared[];
    /**
     * Operation name (for debugging/logging).
     */
    readonly name: string;
    /**
     * Invocation metadata (passed through for downstream calls).
     */
    readonly meta: AdapterInvocationMeta<TAuthorizationPayload>;
    /**
     * Get the single prepared transaction.
     *
     * @remarks
     * Throws if there are zero or multiple transactions.
     * Returns TPrepared with all custom method signatures intact.
     */
    single(): TPrepared;
    /**
     * Estimate total fees across all transactions.
     */
    estimateTotal(): Promise<AggregateFeeEstimate>;
    /**
     * Execute all transactions sequentially.
     *
     * @param options - Optional execution callbacks.
     */
    execute(options?: ExecuteOptions): Promise<SequentialExecuteResult<TRaw>>;
}

/**
 * Adapter types: interface, universal result types, and prepared transactions.
 *
 * @packageDocumentation
 */

/**
 * Generic adapter interface for action bindings.
 *
 * @typeParam TContext - The adapter context type.
 * @typeParam TReadInput - Input type for read operations.
 * @typeParam TWriteInput - Input type for write/prepare operations.
 * @typeParam TChain - Chain definition type (defaults to ChainDefinition).
 */
interface Adapter$1<TContext, TReadInput, TWriteInput, TChain extends ChainDefinition = ChainDefinition, TAuthorizationPayload extends AuthorizationPayload = AuthorizationPayload> {
    /** The adapter context containing configuration, clients, and capabilities. */
    readonly ctx: TContext;
    /** Execute a read operation. */
    read<T>(input: TReadInput, operation: OperationMeta, invocation?: AdapterInvocationMeta<TAuthorizationPayload>): Promise<T>;
    /** Prepare one or more write operations. */
    prepare(input: TWriteInput | TWriteInput[], operation: OperationMeta, invocation?: AdapterInvocationMeta<TAuthorizationPayload>): Promise<PrepareResult<unknown, unknown, unknown, PreparedTransactionBase, TAuthorizationPayload>>;
    /** Get the connected wallet address for a chain. */
    getAddress(chain: TChain): Promise<string>;
}
interface AdapterGenerics {
    readonly read: never;
    readonly write: never;
    readonly chain: never;
    readonly authorization: AuthorizationPayload;
}
type AdapterGenericsOf<T> = T extends Adapter$1<unknown, infer TRead, infer TWrite, infer TChain, infer TAuthorizationPayload> ? {
    readonly read: TRead;
    readonly write: TWrite;
    readonly chain: TChain;
    readonly authorization: TAuthorizationPayload;
} : AdapterGenerics;
/** Extract the read input type from an adapter. */
type AdapterReadInputOf<T> = AdapterGenericsOf<T>['read'];
/** Extract the chain type from an adapter. */
type AdapterChainOf<T> = AdapterGenericsOf<T>['chain'];
/** Extract the authorization payload type from an adapter. */
type AdapterAuthorizationPayloadOf<T> = AdapterGenericsOf<T>['authorization'];
/** Universal fee estimate with ecosystem-specific raw data. */
interface FeeEstimate<TRaw = unknown> {
    /** Total fee as an Amount. */
    readonly fee: Amount;
    /**
     * Compute units used in the calculation.
     * EVM: gas units | Solana: compute units | Aptos: gas units
     */
    readonly units: bigint;
    /**
     * Price per compute unit at time of calculation.
     * EVM: wei per gas | Solana: micro-lamports per CU | Aptos: octas per gas
     *
     * @remarks
     * **`units * unitPrice` is NOT a portable fee formula.** The
     * unit/price denominations differ per ecosystem and the fee
     * formula differs accordingly:
     *
     * | Ecosystem | `unitPrice` denomination | Fee formula                                                        |
     * |-----------|--------------------------|--------------------------------------------------------------------|
     * | EVM       | wei per gas              | `units * unitPrice` (modulo bufferBps)                             |
     * | Solana    | micro-lamports per CU    | `units * unitPrice / 1_000_000 + 5000` (priority fee + base fee)   |
     * | Aptos     | octas per gas            | `units * unitPrice` (modulo bufferBps)                             |
     *
     * Notably on Solana the `unitPrice` is in *micro*-lamports (so
     * needs a `/ 1_000_000` divisor to convert to lamports) and the
     * 5 000-lamport base fee per signature is added separately. Always
     * use the precomputed `fee.raw` field if you need a number you can
     * compare to a wallet balance — that is the single value the
     * adapter has done all the unit-conversion work for. The
     * `units` and `unitPrice` fields are exposed for ecosystem-aware
     * code paths (auction-style fee bumping, observability dashboards
     * scoped to one ecosystem) that already understand the formula.
     */
    readonly unitPrice: bigint;
    /** Full ecosystem-specific fee data. */
    readonly raw: TRaw;
    /**
     * Correlation identifier for the primitive invocation that produced
     * this result.
     *
     * @remarks
     * Matches the `traceId` in the emitted lifecycle events and is safe
     * to log in downstream systems for end-to-end correlation. Optional
     * for backward compatibility with pre-2.0 adapters.
     */
    readonly traceId?: string | undefined;
}
/** Universal transaction confirmation with ecosystem-specific raw data. */
interface Confirmation<TRaw = unknown> {
    /** Whether the transaction succeeded. */
    readonly success: boolean;
    /** Transaction identifier (hash for EVM, signature for Solana, etc.). */
    readonly txId: string;
    /** Block or slot identifier where the transaction was confirmed. */
    readonly blockId: string | bigint;
    /** Actual cost consumed by the transaction. */
    readonly costUsed: Amount;
    /** Full ecosystem-specific confirmation data. */
    readonly raw: TRaw;
    /**
     * Correlation identifier for the primitive invocation that produced
     * this confirmation.
     *
     * @remarks
     * Matches the `traceId` in the emitted lifecycle events. Optional
     * for backward compatibility.
     */
    readonly traceId?: string | undefined;
}
/** Universal simulation result with ecosystem-specific raw data. */
interface Simulation<TRaw = unknown> {
     ...
     *
     * @remarks
     * Retained for display/logging ergonomics. Agents and programmatic
     * callers should prefer {@link Simulation.errorInfo}, which is the
     * structured `KitError` payload.
     */
    readonly error?: string | undefined;
    /**
     * Structured error information when the simulation reverts.
     *
     * @remarks
     * Normalized through the adapter's `normalizeError` pipeline so
     * callers get the same typed error shape as any other primitive
     * failure (`code`, `type`, `recoverability`, etc.). Present only
     * when `success === false`.
     */
    readonly errorInfo?: ErrorInfo | undefined;
    /** Full ecosystem-specific simulation data. */
    readonly raw: TRaw;
    /**
     * Correlation identifier for the primitive invocation that produced
     * this simulation.
 ...
 *
 * @remarks
 * Returned by `prepared.execute()` and the standalone `execute` primitive.
 * Two-phase shape so callers can persist a stable transaction
 * identifier immediately and `await` for confirmation later (or never,
 * if fire-and-forget).
 *
 * The `txId` field is named for cross-ecosystem symmetry rather than
 * the ecosystem-native term:
 * - On EVM (viem, ethers): `txId` is the 0x-prefixed transaction hash.
 *   Equivalent to viem's `result.hash` and ethers' `tx.transactionHash`.
 * - On Solana (solana, solana-kit): `txId` is the base58 transaction
 *   signature.
 *
 * @example
 * ```typescript
 ...
 *
 * console.log('submitted:', result.txId) // canonical id, available now
 *
 * // Block until the transaction is mined (or rejected on-chain).
 ...
 * ```
 *
 * @typeParam TRaw - Ecosystem-specific receipt shape surfaced by `wait()`.
 */
interface ExecuteResult<TRaw = unknown> {
    /**
     * Canonical transaction identifier, available immediately after the
     * RPC accepts the submission. Persist this before calling `wait()`
     * so dropped requests can be reconciled by the operator.
     */
    readonly txId: string;
    /**
     * Resolve once the transaction is confirmed on-chain.
     *
     * @remarks
     * Returns a {@link Confirmation} containing the receipt (`success`,
     * `blockNumber`, gas used, ecosystem-native `raw` payload, etc.).
     * `wait()` is idempotent — call it multiple times to re-derive the
     * confirmation envelope without re-submitting the transaction.
     */
    readonly wait: () => Promise<Confirmation<TRaw>>;
    /**
     * Correlation identifier for the `execute` invocation that produced
     * this result.
     *
     * @remarks
     * Matches the `traceId` on all `op.phase.*` and `rpc.call.*` events
     * emitted while this transaction was being built and submitted.
     * Persist alongside the `txId` to later reconstruct the submission
     * trail (e.g. for support/audit) even across process restarts.
     * Optional for backward compatibility.
     */
    readonly traceId?: string | undefined;
}
/**
 * Minimal prepared transaction interface for chain-agnostic code.
 *
 * @remarks
 * Ecosystem adapters extend this with more specific types.
 */
interface PreparedTransactionBase {
    readonly input: unknown;
    readonly built: unknown;
    estimate(): Promise<FeeEstimate>;
    readonly simulate?: (() => Promise<Simulation>) | undefined;
    readonly buildRaw?: (() => Promise<unknown>) | undefined;
    execute(...args: unknown[]): Promise<ExecuteResult>;
}
/**
 * Typed prepared transaction interface.
 *
 * @typeParam TInput - The user-provided input type.
 * @typeParam TBuilt - The built/encoded transaction type.
 * @typeParam TRaw - Ecosystem-specific raw data in results.
 */
interface PreparedTransaction<TInput = unknown, TBuilt = unknown, TRaw = unknown> extends PreparedTransactionBase {
    readonly input: TInput;
    readonly built: TBuilt;
    estimate(): Promise<FeeEstimate<TRaw>>;
    execute(...args: unknown[]): Promise<ExecuteResult<TRaw>>;
}

/**
 * A primitive function created by `createPrimitive`.
 *
 * @typeParam TInput - The input type for the primitive.
 * @typeParam TOutput - The output type of the primitive.
 *
 * @remarks
 * This is the signature of the function returned by `createPrimitive`.
 * It takes input, operation metadata, and optional invocation metadata.
 *
 * **Arguments:**
 * - `input` - The operation input
 * - `operation` - **WHAT**: Target chain and optional address
 * - `invocation` - **WHO/HOW**: Optional traceId, runtime override, callers
 */
type PrimitiveFunction<TInput, TOutput, TAuthorizationPayload extends AuthorizationPayload = AuthorizationPayload> = (input: TInput, operation: OperationMeta, invocation?: AdapterInvocationMeta<TAuthorizationPayload>) => Promise<TOutput>;
/**
 * A primitive function with a generic output type.
 *
 * @remarks
 * This type allows callers to specify the output type at call time,
 * useful for operations like `read<T>()` where the return type depends
 * on the contract being called.
 *
 * @typeParam TInput - The input type for the primitive.
 */
type GenericPrimitiveFunction<TInput, TAuthorizationPayload extends AuthorizationPayload = AuthorizationPayload> = <TOutput = unknown>(input: TInput, operation: OperationMeta, invocation?: AdapterInvocationMeta<TAuthorizationPayload>) => Promise<TOutput>;
/**
 * Result of waiting for transaction confirmation.
 *
 * @typeParam TRaw - Ecosystem-specific receipt type.
 */
interface WaitResult<TRaw = unknown> {
    /** Transaction success status. */
    readonly success: boolean;
    /** Transaction identifier that was waited for. */
    readonly txId: string;
    /** Block/slot identifier where confirmed. */
    readonly blockId: string | bigint;
    /** Cost consumed (access `.raw` for bigint value). */
    readonly costUsed: Amount;
    /** Ecosystem-specific raw receipt. */
    readonly raw: TRaw;
}

/**
 * Type definitions for the defineAction factory.
 *
 * @remarks
 * **Naming Conventions:**
 * - `OperationMeta`: User-facing input (WHAT - chain, address) - from `@core/adapter-base/context`
 * - `ResolvedActionContext`: Internal resolved context (chain as ChainDefinition)
 * - `ActionOptions`: Invocation options (WHO/HOW - runtime, callers, skipEstimate)
 *
 * @packageDocumentation
 */
 ...
 *
 * @remarks
 * Combined options object passed as the 3rd argument to actions.
 * Extends `InvocationMeta` with action-specific options like `skipEstimate`.
 *
 * Defines **WHO** is calling and **HOW** to observe:
 * - `runtime`: Runtime override for logging, events, metrics
 * - `callers`: Caller chain for distributed tracing
 * - `skipEstimate`: Skip gas estimation for faster execution
 *
 * @example
 * ```typescript
 * // Simple usage (no options)
 * await action(input, ctx)
 *
 * // With options
 * await action(input, ctx, { skipEstimate: true })
 *
 * // Provider calling adapter (with caller chain)
 * await adapter.actions.approve(input, ctx, {
 *   callers: [
 *     { type: 'kit', name: 'cctp-kit', version: '1.0.0' },
 *     { type: 'provider', name: 'evm-provider', version: '2.1.0' },
 *   ],
 *   runtime: kitRuntime,
 * })
 * ```
 */
interface ActionOptions<TAuthorizationPayload extends AuthorizationPayload = AuthorizationPayload> extends AdapterInvocationMeta<TAuthorizationPayload> {
    /**
     * Skip gas/cost estimation (write actions only).
     *
     * @remarks
     * When true, uses fallback gas values instead of estimating.
     * Useful when estimation is slow or unreliable.
     */
    readonly skipEstimate?: boolean | undefined;
    /**
     * Custom tags for observability dimensions.
     *
     * @remarks
     * Merged into all lifecycle events, log entries, and metric labels
     * emitted during this action invocation. Use for caller-specific
     * dimensions (e.g., `{ env: 'staging', feature: 'checkout' }`).
     */
    readonly tags?: Tags | undefined;
    /**
     * Custom metadata propagated to middleware.
     *
     * @remarks
     * Attached to the execution context and included in lifecycle event
     * payloads (e.g., `op.phase.started` `data.meta`). Use for
     * domain-specific context (e.g., `{ orderId: '123', source: 'api' }`).
     */
    readonly meta?: Record<string, unknown> | undefined;
}
/**
 * Fully resolved action context.
 *
 * @remarks
 * Created internally by the action system after resolving:
 * - Chain → ChainDefinition
 * - Address → derived from adapter or provided
 * - TraceId → generated or provided
 *
 * All fields are guaranteed to be present.
 */
interface ResolvedActionContext {
    /** Fully resolved chain definition. */
    readonly chain: ChainDefinition;
    /** Sender/signer address (derived or provided). */
    readonly address: string;
    /** Trace ID for correlation (generated or provided). */
    readonly traceId: string;
}
/**
 * Context passed to READ action execute functions.
 *
 * @remarks
 * Contains resolved context fields, the adapter instance, and typed read capability.
 * The read input type is automatically derived from the adapter's generic parameters.
 *
 * @typeParam TAdapter - The adapter type (extends `Adapter`).
 *
 * @example
 * ```typescript
 * execute: async (input, ctx) => {
 *   // ctx.chain - ChainDefinition
 *   // ctx.address - string (guaranteed)
 *   // ctx.traceId - string (guaranteed)
 *   // ctx.adapter - the adapter instance
 *   // ctx.read - typed read function (input derived from adapter)
 *
 *   const balance = await ctx.read<bigint>({
 *     address: input.token,
 *     abi: ERC20_ABI,
 ...
 * ```
 */
interface ExecuteContext<TAdapter> extends ResolvedActionContext {
    /**
     * Fully resolved chain definition, narrowed to the adapter's chain type.
     *
     * @remarks
     * For example, `ExecuteContext<EVMAdapter>` yields `EVMChainDefinition` here,
     * so adapter methods like `getAddress(chain)` receive the correct type without
     * a cast. Falls back to the base `ChainDefinition` when the adapter's chain
     * type cannot be inferred.
     */
    readonly chain: AdapterChainOf<TAdapter> extends ChainDefinition ? AdapterChainOf<TAdapter> : ChainDefinition;
    /**
     * The adapter instance.
     *
     * @remarks
     * Provides access to the adapter context via `ctx.adapter.ctx`,
     * and allows calling adapter methods directly if needed.
     */
     ...
     *
     * @remarks
     * Use in bound read actions to resolve token addresses or decimals
     * (e.g. `ctx.tokens.resolve('USDC', ctx.chain.name).locator`).
     */
    readonly tokens: TokenRegistry;
    /** Resolved request-scoped invocation metadata. */
    readonly invocation: AdapterInvocationContext<AdapterAuthorizationPayloadOf<TAdapter>>;
    /**
     * Execute a read operation.
     *
     * @remarks
     * The input type is automatically derived from the adapter's TReadInput parameter.
     *
     * @typeParam T - Expected return type.
     * @param input - Adapter-specific read input.
     * @returns The read result.
     */
    read<T>(input: AdapterReadInputOf<TAdapter>): Promise<T>;
}
/**
 * Bound read action - callable function that returns output directly.
 *
 * @typeParam TInput - Input type.
 * @typeParam TOutput - Output type.
 */
interface ReadAction<TInput, TOutput, TAuthorizationPayload extends AuthorizationPayload = AuthorizationPayload> {
    /**
     * Execute the read action.
     *
     * @param input - Action input.
     * @param operation - Operation metadata (chain, address?).
     * @param options - Optional action options (callers, runtime override).
     * @returns Action output.
     */
    (input: TInput, operation: OperationMeta, options?: ActionOptions<TAuthorizationPayload>): Promise<TOutput>;
    /** Action name. */
    readonly name: string;
    /**
     ...
     *
     * @remarks
     * Carried over from the {@link ActionDefinition} so consumers (logs,
     * progress UIs, tracing tools) can show what is happening without
     * decoding the `name`. Always present for actions produced by
     * `defineAction`; optional only because derived/synthetic actions may be
     * constructed without going through a definition.
     */
...
/**
 * Result of executing a write action (fast path).
 *
 * @remarks
 * - `txId`: The primary (last) transaction ID, available immediately
 * - `txIds`: All transaction IDs in execution order
 * - `wait()`: Lazily waits for the last transaction's on-chain confirmation
 *
 * For single-tx actions (95% of cases), `txId === txIds[0]`.
 * For multi-tx actions (e.g. USDT safe-approve), previous txs are already
 * confirmed by the time the result is returned.
 */
interface WriteResult {
    /** Primary transaction ID (last in sequence). */
    readonly txId: string;
    /** All transaction IDs in execution order. */
    readonly txIds: readonly string[];
    /** Wait for the last transaction to confirm. */
    readonly wait: () => Promise<Confirmation>;
}
/**
 * What `.prepare()` returns — a `PrepareResult` with the build output attached.
 *
 * @typeParam TOutput - Semantic output from the build phase.
 */
type PreparedWriteAction<TOutput = void, TAuthorizationPayload extends AuthorizationPayload = AuthorizationPayload> = PrepareResult<unknown, unknown, unknown, PreparedTransactionBase, TAuthorizationPayload> & {
    /** Semantic output computed during the build phase. May be undefined if build returns undefined. */
    readonly output: TOutput | undefined;
};
/**
 * Bound write action — callable function with `.prepare()` method.
 *
 * @typeParam TInput - Input type.
 * @typeParam TOutput - Semantic build output type (optional, defaults to void).
 */
interface WriteAction<TInput, TOutput = void, TAuthorizationPayload extends AuthorizationPayload = AuthorizationPayload> {
    /**
     * Execute the write action (fast path).
     *
     * @remarks
     * Prepares, estimates, executes sequentially, and returns the result.
     * Use `{ skipEstimate: true }` in options to skip fee estimation.
     */
    (input: TInput, operation: OperationMeta, options?: ActionOptions<TAuthorizationPayload>): Promise<WriteResult>;
    /**
     * Prepare the action for inspection before execution.
     *
     * @remarks
     * Returns a `PrepareResult` (with `output` attached) giving full control:
     * - `estimateTotal()` — aggregate fee estimates
     * - `transactions[]` — inspect individual prepared transactions
     * - `single()` — get the single transaction (throws if multiple)
     * - `execute()` — sequential execution with confirmations
     */
    prepare(input: TInput, operation: OperationMeta, options?: ActionOptions<TAuthorizationPayload>): Promise<PreparedWriteAction<TOutput, TAuthorizationPayload>>;
    /** Action name. */
    readonly name: string;
    /**
     * Human-readable description of what the action does.
...
/**
 * Standard output containing an Amount.
 *
 * @remarks
 * Used by all read actions that return a quantity (balance, allowance)
 * and all write actions that confirm a quantity (approve, transfer).
 */
interface AmountOutput {
    /** The resolved amount (includes raw bigint + decimals). */
    readonly amount: Amount;
}
/** Output for balance queries. */
type BalanceOutput = AmountOutput;
/** Output for allowance queries. */
type AllowanceOutput = AmountOutput;
/** Output for approve transactions. */
type ApproveOutput = AmountOutput;

/**
 * Input for token.balanceOf action.
 */
interface TokenBalanceOfInput {
    /**
     * The token to query.
     *
     * @remarks
     * Accepts a known symbol (`'USDC'`), a raw address (`'0xA0b8...'`),
     * or an object `{ locator, decimals? }` for arbitrary tokens.
     *
     * Known symbols resolve decimals from the token registry (no RPC call).
     * Raw addresses may trigger an on-chain `decimals()` call if decimals
     * are not provided.
     */
    readonly token: RegistryTokenSelector;
    /**
     * The wallet address to check balance for.
     *
     * @remarks
     * Defaults to the connected address if not provided.
     */
    readonly walletAddress?: string | undefined;
}

/**
 * Input for token.allowance action.
 */
interface TokenAllowanceInput {
    /**
     * The token to query allowance for.
     *
     * @remarks
     * Accepts a known symbol (`'USDC'`), a raw address, or `{ locator, decimals? }`.
     */
    readonly token: RegistryTokenSelector;
    /**
     * The wallet address whose allowance is being queried.
     *
     * @remarks
     * Defaults to the connected address if not provided.
     */
    readonly walletAddress?: string | undefined;
    /**
     * The delegate address authorized to spend tokens.
     */
    readonly delegate: string;
}

/**
 * Input for token.approve action.
 */
interface TokenApproveInput {
    /**
     * The token to approve spending for.
     *
     * @remarks
     * Accepts a known symbol (`'USDC'`), a raw address, or `{ locator, decimals? }`.
     */
    readonly token: RegistryTokenSelector;
    /**
     * The delegate address to authorize.
     */
    readonly delegate: string;
    /**
     * The amount to approve (in smallest units).
     */
    readonly amount: bigint;
}

/**
 * Input for token.transfer action.
 */
...
/**
 * Input for token.increaseAllowance action.
 *
 * @remarks
 * Same shape as token.approve — the difference is the contract function called.
 * Not all tokens support this (it's an ERC20 extension).
 */
interface TokenIncreaseAllowanceInput {
    readonly token: RegistryTokenSelector;
    readonly delegate: string;
    readonly amount: bigint;
}
...
/**
 * Input for token.name action.
 */
interface TokenNameInput {
    /**
     * The token to query.
     *
     * @remarks
     ...
     * or an object `{ locator, decimals? }` for arbitrary tokens.
     *
     * Resolution matches every other `token.*` action, so a known symbol
     * comes from the token registry and a raw address is used as given.
     */
    readonly token: RegistryTokenSelector;
}
/**
 * Output for token.name action.
 *
 * @remarks
 * The value is wrapped in an object so the shape stays stable if the action
 * grows more fields. The legacy-compat bridge unwraps the single key, so
 * callers on the old `prepareAction` surface receive the bare string.
 */
interface TokenNameOutput {
    /**
     * The on-chain name of the token contract.
     *
     * @remarks
     * For USDC this is also the EIP-712 domain name, for example `'USD Coin'`.
     * Permit and authorize signing flows use it to build the domain separator.
     */
    readonly name: string;
}

/**
 * Input for native.balanceOf action.
 */
interface NativeBalanceOfInput {
    /**
     * The wallet address to check balance for.
     *
     * @remarks
...
/**
 * Input for native.transfer action.
 */
interface NativeTransferInput {
    /** The recipient address. */
    readonly to: string;
    /** The amount to transfer (in smallest units, e.g. wei). */
    readonly amount: bigint;
}

/**
 * EIP-2612 permit parameters for gasless token approvals.
 *
 * @remarks
 * Enables a user to approve a token transfer via an off-chain signature, allowing
 * for gasless approvals in compliant smart contracts.
 */
interface PermitParams$1 {
    /** Expiry timestamp for the permit signature (in seconds since epoch). */
    readonly deadline: bigint;
    /** Signature recovery ID (EIP-2098/ECDSA 'v' value). */
    readonly v: number;
    /** ECDSA signature 'r' value (as 0x-prefixed hex string). */
    readonly r: string;
    /** ECDSA signature 's' value (as 0x-prefixed hex string). */
    readonly s: string;
}
/**
 * Input for cctp.v2.depositForBurn action.
 *
 * @remarks
 * Burns USDC on the source chain to initiate a cross-chain transfer.
 */
interface CCTPDepositForBurnInput {
    /** Amount of USDC to burn (in smallest units). */
    readonly amount: bigint;
    /** Recipient address on the destination chain. */
    readonly mintRecipient: string;
    /** Optional caller restriction on destination chain. */
    readonly destinationCaller?: string | undefined;
    /** Maximum fee for the transfer. */
    readonly maxFee: bigint;
    /** Minimum finality threshold (block confirmations). */
    readonly minFinalityThreshold: number;
    /** Destination chain where tokens will be minted. */
    readonly toChain: ChainIdentifier$1;
}
/**
 * Input for cctp.v2.receiveMessage action.
 *
 * @remarks
 * Receives and mints USDC on the destination chain using the attestation.
 */
interface CCTPReceiveMessageInput {
    /** Event nonce from the burn transaction (0x-prefixed hex string). */
    readonly eventNonce: string;
    /** Attestation from Circle's attestation service. */
    readonly attestation: string;
    /** CCTP message bytes from the burn event. */
    readonly message: string;
    /** Source chain where the burn occurred. */
    readonly fromChain: ChainIdentifier$1;
    /** Optional destination address override. */
    readonly destinationAddress?: string | undefined;
    /** Optional mint recipient override. */
    readonly mintRecipient?: string | undefined;
}
/**
 * Input for cctp.v2.customBurn action.
 *
 * @remarks
 * Uses a custom bridge contract with preapproval funnel for protocol fees.
 */
interface CCTPCustomBurnInput extends CCTPDepositForBurnInput {
    /** Optional protocol fee amount. */
    readonly protocolFee?: bigint | undefined;
    /** Optional fee recipient address. */
    readonly feeRecipient?: string | undefined;
    /** Optional EIP-2612 permit parameters for gasless approval. */
    readonly permitParams?: PermitParams$1 | undefined;
}
/**
 * Input for cctp.v2.depositForBurnWithHook action.
 *
 * @remarks
 * Same as depositForBurn but includes hookData for Circle's Orbit relayer
 * to auto-relay/mint on the destination chain.
 */
interface CCTPDepositForBurnWithHookInput extends CCTPDepositForBurnInput {
    /** Hex-encoded hook data for the Orbit relayer (0x-prefixed). */
    readonly hookData: string;
}
/**
 * Input for cctp.v2.customBurnWithHook action.
 *
 * @remarks
 * Same as customBurn but includes hookData for Circle's Orbit relayer
 * to auto-relay/mint on the destination chain.
 */
interface CCTPCustomBurnWithHookInput extends CCTPCustomBurnInput {
    /** Hex-encoded hook data for the Orbit relayer (0x-prefixed). */
    readonly hookData: string;
}

interface GatewayDepositInput {
    readonly token: string;
    readonly value: bigint;
}
interface GatewayDepositForInput {
    readonly token: string;
    readonly depositor: string;
    readonly value: bigint;
}
interface GatewayDepositWithPermitInput {
    readonly token: string;
...
interface GatewayDepositWithAuthorizationInput {
    readonly token: string;
...
interface GatewayRemoveDelegateInput {
    readonly token: string;
    readonly delegate: string;
}
interface GatewayIsDelegateInput {
    readonly token: string;
    readonly depositor: string;
    readonly delegate: string;
}
interface GatewayInitiateWithdrawalInput {
    readonly token: string;
    readonly value: bigint;
}
interface GatewayWithdrawInput {
    readonly token: string;
}
interface GatewayWithdrawingBalanceInput {
    readonly token: string;
    readonly depositor: string;
}
interface GatewayWithdrawalBlockInput {
    readonly token: string;
    readonly depositor: string;
}
interface GatewayBurnInput {
    readonly calldataBytes: `0x${string}`;
    readonly signature: `0x${string}`;
}
interface GatewayMintInput {
    readonly attestation: `0x${string}`;
    readonly signature: `0x${string}`;
}
interface GatewaySignBurnIntentsInput {
    readonly typedData: unknown;
}
/** Output for isDelegate read action. */
interface GatewayIsDelegateOutput {
    readonly isDelegate: boolean;
}
/**
 * Output for `gateway.v1.withdrawingBalance` read action.
 *
 * @remarks
 * The pending withdrawal balance is returned as a decimal-encoded u64 string
 * in base units (for example, `'5000000'` for 5 USDC on a 6-decimal token).
 * Bindings stringify the raw u64 rather than returning a `bigint` so that
 * the shape is stable across the legacy-compat bridge (`adaptReadAction`)
 * and downstream consumers such as `providers/gateway.v1`.
 */
interface GatewayWithdrawingBalanceOutput {
    readonly balance: string;
}
/**
 * Output for `gateway.v1.withdrawalBlock` read action.
 *
 * @remarks
 * The block number at which a pending withdrawal may be completed, returned
 * as a `bigint` because block numbers are u64 quantities and are not
 * balance-like values. The legacy-compat bridge (`adaptReadAction`)
 * unwraps this field transparently for callers that still consume the
 * legacy `PreparedChainRequest.execute()` surface.
 */
interface GatewayWithdrawalBlockOutput {
    readonly blockNumber: bigint;
}
/** Output for signBurnIntents action. */
interface GatewaySignatureOutput {
    readonly signature: string;
    /**
     * Whether Gateway must validate `signature` with ERC-1271 rather than
     * `ecrecover`, i.e. whether the signer is a contract account. Absent on
     * ecosystems where ERC-1271 does not apply (Solana) and on wirings that
     * cannot classify the signer, both of which mean "validate as an EOA".
     */
    readonly contractSigner?: boolean;
}

/**
 * Permit signature standards for gasless token approvals used by the Earn
 * Adapter Contract.
 *
 * @remarks
 * - `NONE` (0): Tokens must be pre-approved via a separate transaction.
 * - `EIP2612` (1): Standard ERC-20 permit (USDC and most modern tokens).
 */
declare enum EarnPermitType {
...
/**
 * Token input with optional permit signature for use with the Earn Adapter
 * Contract.
 *
 * @remarks
 * Passed verbatim to the contract's `execute(executeParams, tokenInputs,
 * signature)` call.  Today the earn provider always uses `NONE` (pre-approval
 * path); the permit fields are reserved for a future gasless approval path.
 */
interface EarnTokenInput {
    /** Type of permit to execute. */
    readonly permitType: EarnPermitType;
    /** Token contract address. */
    readonly token: `0x${string}`;
    /** Amount of tokens in the smallest unit. */
    readonly amount: bigint;
    /** ABI-encoded permit calldata (empty bytes for `NONE`). */
    readonly permitCalldata: `0x${string}`;
}
/**
 * Input for all three earn action keys.
 *
 * @remarks
 * `executeParams` is intentionally typed as `Record<string, unknown>` to
 * preserve the raw service-signed payload unchanged — the Adapter Contract
 * ABI-decodes it on-chain.
 *
 * @example
 * ```typescript
 * import type { EarnExecuteInput } from '@core/adapter-base'
 *
 * const input: EarnExecuteInput = {
 *   executeParams: {
 *     instructions: [],
 *     tokens: [],
 *     execId: 1n,
 ...
 * ```
 */
interface EarnExecuteInput {
    /**
     * Service-signed execution parameters.
     *
     * Kept as an opaque record so the adapter forwards the service-signed
     * struct unchanged.
     */
    readonly executeParams: Record<string, unknown>;
    /**
     * Token inputs with permit signatures for gasless approvals.
     *
     * Populated by the earn provider after deciding how token spending is
     * authorised. Today deposit uses a separate token allowance transaction and
     * passes `PermitType.NONE`; a future permit-enabled path can populate this
     * field without a breaking change.
     */
    readonly tokenInputs: readonly EarnTokenInput[];
    /**
     * EIP-712 signature from the earn service proxy.
     *
     * The Adapter Contract verifies this signature on-chain before executing
 ...
 *
 * @remarks
 * - `NONE` (0): No permit, tokens must be pre-approved via separate transaction.
 * - `EIP2612` (1): Standard ERC-20 permit (USDC, DAI v2, and most modern tokens).
 */
declare enum SwapPermitType {
    /** No permit required — tokens must be pre-approved. */
    NONE = 0,
    /** EIP-2612 standard permit. */
    EIP2612 = 1
 ...
 *
 * @remarks
 * The Adapter Contract uses this to pull tokens from the user's wallet using
 * permit signatures instead of requiring a separate approval transaction.
 */
interface SwapTokenInput {
    /** Type of permit to execute. */
    readonly permitType: SwapPermitType;
...
interface SwapInstruction {
    ...
    /** Native value to send with the call (for native token operations). */
    readonly value: bigint;
    ...
    readonly amountToApprove: bigint;
 ...
 *
 * @remarks
 * After all instructions complete, the Adapter Contract sweeps any remaining
 * balances to the specified beneficiaries.
 */
interface SwapTokenRecipient {
 ...
 *
 * @remarks
 * Signed via EIP-712 by Circle's proxy service and verified on-chain. These
 * values are supplied by the stablecoin-service and must be forwarded to the
 * Adapter Contract exactly as received (no modification).
 */
interface SwapExecuteParams {
    /** Array of instructions to execute sequentially. */
    readonly instructions: readonly SwapInstruction[];
    /** Token recipients for residual sweep. */
    readonly tokens: readonly SwapTokenRecipient[];
    /** Unique execution identifier for replay protection. */
    readonly execId: bigint;
...
/**
 * Parameters for executing a swap via the EVM Adapter Contract.
 *
 * @remarks
 * The SDK calls `AdapterContract.execute(executeParams, tokenInputs, signature)`,
 * which pulls tokens via permits, executes swaps, validates outputs, and
 * sweeps residuals — all atomically in a single transaction.
 */
interface ExecuteSwapEVMInput {
    /** Execution parameters from the stablecoin-service (EIP-712 signed). */
    readonly executeParams: SwapExecuteParams;
    /** Token inputs with permit signatures for gasless approvals. */
    readonly tokenInputs: readonly SwapTokenInput[];
    /** EIP-712 signature from the Circle proxy service. */
    readonly signature: `0x${string}`;
    /** Swap input amount in base units (tx.value for native swaps). */
    readonly inputAmount: bigint;
    /** Token address being swapped from (native sentinel triggers value transfer). */
    readonly tokenInAddress: `0x${string}`;
}
/**
 * Parameters for executing a swap on Solana.
 *
 * @remarks
 * The stablecoin-service returns a fully built `VersionedTransaction` that
 * already contains routing, fees, slippage, ATA setup and Address Lookup
 * Tables. The SDK deserialises, signs, and submits it unchanged.
 */
interface ExecuteSwapSolanaInput {
    /** Base64-encoded serialized Solana transaction. */
    readonly serializedTransaction: string;
}
/**
 * Union of all chain-specific swap inputs.
 *
 * @remarks
 * Each ecosystem binding narrows this union at runtime using a
 * property-based guard (EVM has `executeParams`/`tokenInputs`, Solana has
 * `serializedTransaction`).
 */
type ExecuteSwapInput = ExecuteSwapEVMInput | ExecuteSwapSolanaInput;
...
/** Input for stablecoin balance queries (token pre-filled). */
type StablecoinBalanceOfInput = Omit<TokenBalanceOfInput, 'token'>;
/** Input for stablecoin allowance queries (token pre-filled). */
type StablecoinAllowanceInput = Omit<TokenAllowanceInput, 'token'>;
/** Input for stablecoin approve actions (token pre-filled). */
type StablecoinApproveInput = Omit<TokenApproveInput, 'token'>;
/** Input for stablecoin transfer actions (token pre-filled). */
type StablecoinTransferInput = Omit<TokenTransferInput, 'token'>;
/** Input for stablecoin increaseAllowance actions (token pre-filled). */
type StablecoinIncreaseAllowanceInput = Omit<TokenIncreaseAllowanceInput, 'token'>;
/** Input for stablecoin name queries (token pre-filled). */
type StablecoinNameInput = Omit<TokenNameInput, 'token'>;
/**
 * The minimum set of actions every adapter must expose.
 *
 * @remarks
 * Ecosystem packages extend this with chain-specific actions
 * (e.g. CCTP v2 bridge actions).
 */
interface BaseActionRegistry<TAuthorizationPayload extends AuthorizationPayload = AuthorizationPayload> {
    'token.balanceOf': ReadAction<TokenBalanceOfInput, BalanceOutput, TAuthorizationPayload>;
    'token.allowance': ReadAction<TokenAllowanceInput, AllowanceOutput, TAuthorizationPayload>;
    'token.approve': WriteAction<TokenApproveInput, ApproveOutput, TAuthorizationPayload>;
    'token.transfer': WriteAction<TokenTransferInput, void, TAuthorizationPayload>;
    'token.increaseAllowance': WriteAction<TokenIncreaseAllowanceInput, void, TAuthorizationPayload>;
    'token.name': ReadAction<TokenNameInput, TokenNameOutput, TAuthorizationPayload>;
    'native.balanceOf': ReadAction<NativeBalanceOfInput, BalanceOutput, TAuthorizationPayload>;
    'native.transfer': WriteAction<NativeTransferInput, void, TAuthorizationPayload>;
    /**
     * @deprecated Use `token.balanceOf` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.balanceOf': ReadAction<StablecoinBalanceOfInput, BalanceOutput, TAuthorizationPayload>;
    /**
     * @deprecated Use `token.allowance` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.allowance': ReadAction<StablecoinAllowanceInput, AllowanceOutput, TAuthorizationPayload>;
    /**
     * @deprecated Use `token.approve` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.approve': WriteAction<StablecoinApproveInput, ApproveOutput, TAuthorizationPayload>;
    /**
     * @deprecated Use `token.transfer` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.transfer': WriteAction<StablecoinTransferInput, void, TAuthorizationPayload>;
    /**
     * @deprecated Use `token.increaseAllowance` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.increaseAllowance': WriteAction<StablecoinIncreaseAllowanceInput, void, TAuthorizationPayload>;
    /**
     * @deprecated Use `token.name` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.name': ReadAction<StablecoinNameInput, TokenNameOutput, TAuthorizationPayload>;
}

/**
 * Overloaded action dispatch signature.
 *
 * @typeParam TRegistry - The action registry mapping keys to functions.
 *
 * @remarks
 * - `action(key)` returns the typed action object
 * - `action(key, ...args)` calls the action directly (fast path)
 */
interface ActionDispatch$1<TRegistry extends Record<string, (...args: never[]) => unknown>> {
    <K extends keyof TRegistry>(key: K): TRegistry[K];
    <K extends keyof TRegistry>(key: K, ...args: Parameters<TRegistry[K]>): ReturnType<TRegistry[K]>;
}

/**
 * Strongly-typed event subscription interface for adapters.
 *
 * @remarks
 * Mirrors the `kit.on` / `kit.off` pattern from BridgeKit but uses
 * the augmentable {@link KitEventMap} from `@core/runtime`. Packages
 * can extend `KitEventMap` via module augmentation to register
 * adapter-specific events.
 *
 * The `on` method returns an unsubscribe function (modern pattern).
 * The `off` method accepts the original handler reference and
 * removes it (convenience for callers who prefer explicit cleanup).
 *
 ...
 * adapter.on('op.phase.failed', (event) => {
 *   console.error('Operation failed:', event.data)
 * })
 *
 * // Subscribe with pattern matching
 * adapter.on('op.phase.*', (event) => {
 *   console.log(event.name, event.data)
 * })
 *
 * // Explicit unsubscribe via off()
 * const handler = (event) => console.log(event)
 * adapter.on('op.phase.started', handler)
 * adapter.off('op.phase.started', handler)
 *
 * // Or use the returned unsubscribe function
 * const unsub = adapter.on('op.phase.succeeded', (event) => {
 *   console.log('Succeeded:', event.data)
 * })
 * unsub() // unsubscribe
 * ```
 */
interface AdapterEventSubscription<M extends EventMap = KitEventMap> {
    /**
     * Subscribe to all events.
     *
    ...
    /**
     * Remove a handler registered for a specific event name.
     *
     * @typeParam K - The event name literal.
     * @param name - The event name.
     * @param handler - The handler function reference to remove.
...
/**
 * Types for the {@link assembleAdapter} factory.
 *
 * @packageDocumentation
 */

/**
 * Generic function bound for type parameters in adapter assembly.
 *
 * @remarks
 * TypeScript requires `any` in function type bounds for correct
 * contravariant parameter inference. Concrete types are always inferred
 * from call sites — `any` never leaks to consumers.
 */
type AnyFunction = (...args: any[]) => any;
/**
 * Generic record bound for action registries.
 *
 * @remarks
 * TypeScript interfaces lack implicit index signatures, so
 * `Record<string, (...args: never[]) => unknown>` rejects them.
 * This alias centralises the necessary `any` escape.
 */
type AnyRecord = Record<string, any>;
/**
 * The core shape returned by {@link assembleAdapter}.
 *
 * @remarks
 * Contains all the fields that every adapter must expose. When
 * `extras` are provided, they are intersected onto this shape.
 *
 * This type satisfies `StandardAdapter` from `@core/adapter-compat`
 * so the result can be passed directly to `withLegacyCompat`.
 *
 * @typeParam TCtx - The adapter-specific context type.
 * @typeParam TChainType - Literal chain-type string.
 * @typeParam TRead - Read primitive type (preserved from input).
 * @typeParam TPrepare - Prepare primitive type (preserved from input).
 * @typeParam TWait - WaitForTransaction primitive type (preserved from input).
 * @typeParam TActions - Action registry type.
 */
interface AssembledAdapterShape<TCtx extends AdapterContext$1<AdapterCapabilities$1, any>, TChainType extends string, TRead extends AnyFunction, TPrepare extends AnyFunction, TWait extends AnyFunction, TActions extends AnyRecord> extends AdapterEventSubscription {
    /** Adapter context (clients, capabilities, runtime, etc.). */
    readonly ctx: TCtx;
    /**
     * Adapter capabilities (shortcut for `ctx.capabilities`).
     *
     * @remarks
     * Derived from `TCtx['capabilities']` so the concrete capability literal
     * (e.g. `addressContext: 'developer-controlled'`) is preserved through
     * assembly. This is what lets kits infer address-context from
     * `typeof adapter` instead of seeing the widened `AdapterCapabilities`.
     */
    readonly capabilities: TCtx['capabilities'];
    ...
    /** Get the connected wallet address for a chain. */
    readonly getAddress: TCtx['getAddress'];
    /**
     ...
     * - `action(key, ...args)` — calls the action directly (fast path)
     */
    readonly action: ActionDispatch$1<TActions & Record<string, AnyFunction>>;
    /** Wait for a transaction to be confirmed. */
    readonly waitForTransaction: TWait;
    /** Validate that a chain is supported by this adapter. */
    readonly validateChainSupport: (chain: ChainDefinition) => void;
}

/**
 * Shared EVM types for all EVM adapters (viem, ethers, etc.).
 *
 * @packageDocumentation
 */

/**
 * Input for reading from an EVM smart contract.
 *
 * @remarks
 * This is the standard input format for contract read operations
 * across all EVM adapters.
 */
interface EVMReadInput {
    /** Contract address (hex string). */
    readonly address: `0x${string}`;
    /** Contract ABI (array of ABI items). */
...
/**
 * Input for writing to an EVM smart contract or performing a native transfer.
 *
 * @remarks
 * This is the standard input format for contract write operations
 * across all EVM adapters.
 *
 * **Native transfers**: When `abi` is empty and `functionName` is `''`,
 * the write represents a plain native-token transfer (ETH, MATIC, etc.)
 * rather than a contract call. Use {@link isNativeTransfer} to detect
 * this case in adapter implementations. Use {@link createNativeTransferWrite}
 * to construct native transfer inputs.
 */
interface EVMWriteInput {
    /** Contract address or transfer recipient (hex string). */
    readonly address: `0x${string}`;
    /** Contract ABI (array of ABI items). Empty for native transfers. */
    readonly abi: readonly unknown[];
    /** Function name to call. Empty string for native transfers. */
    readonly functionName: string;
    /** Function arguments. */
    readonly args?: readonly unknown[] | undefined;
...
/** Canonical payload reviewed by phase-one EVM authorization hooks. */
type EvmAuthorizationPayload = TransactionPayload;
/** Adapter configuration bound to the canonical EVM payload union. */
type EvmAuthorizationOptions = AuthorizationOptions<EvmAuthorizationPayload>;
/**
 * Base EVM adapter interface.
 *
 * @remarks
 * All EVM adapters (viem, ethers, etc.) implement this interface.
 * It extends the generic `Adapter` with EVM-specific input types.
 *
 * Native balance is provided via `createEVMActions({ getNativeBalance })`,
 * not as an adapter method.
 *
 * @typeParam TContext - The adapter-specific context type.
 *
 * @example
 * ```typescript
 * import type { EVMAdapter } from '@core/adapter-evm-base'
 * import type { AdapterContext } from '@core/adapter-base'
 *
 * type MyEVMAdapter = EVMAdapter<AdapterContext>
 * ```
 */
type EVMAdapter<TContext extends AdapterContext$1<AdapterCapabilities$1, EvmAuthorizationPayload> = AdapterContext$1<AdapterCapabilities$1, EvmAuthorizationPayload>> = Adapter$1<TContext, EVMReadInput, EVMWriteInput, EVMChainDefinition, EvmAuthorizationPayload>;

/** Callback to fetch the native token balance for a given wallet on an EVM chain. */
type GetNativeBalance = (chain: EVMChainDefinition, wallet: string) => Promise<bigint>;

/**
 * Gateway v1 EVM action bindings.
 *
 * @remarks
 * Each binding maps a chain-agnostic Gateway action definition to the EVM
 * contract call pattern: resolve the Gateway Wallet / Minter address from
 * the chain, then queue a `ctx.write()` or execute a `ctx.read()`.
 *
 * @packageDocumentation
 */

/**
 * Callback shape for EIP-712 typed-data signing, injected by the ecosystem
 * adapter (viem, ethers, …) into the gateway `signBurnIntents` binding.
 *
 * @remarks
 * `signBurnIntents` is an off-chain signing operation — not a contract call —
 * so it cannot go through the generic `ctx.read` / `ctx.write` surface used
 * by the other gateway bindings. Instead, the ecosystem adapter provides
 * its `signTypedData` primitive via {@link BindGatewaySignBurnIntentsOptions}
 * and the binding delegates to it.
 *
 * The callback receives the raw `typedData` payload plus the resolved
 * `ExecuteContext` so the ecosystem adapter can route the signature through
 * its wallet client, preserving observability and error-normalization.
 *
 * Accepts the typed data as `unknown` because Gateway burn-intents omit
 * `chainId`/`verifyingContract` from the domain — structural shape is
 * validated by the ecosystem adapter's own EIP-712 schema (e.g. viem's
 * `assertTypedData`) rather than the base binding.
 */
type EVMSignTypedDataCallback = (typedData: unknown, ctx: ExecuteContext<EVMAdapter>) => Promise<`0x${string}`>;
/**
 * Callback shape for an EVM `eth_getCode` lookup, injected by the ecosystem
 * adapter (viem, ethers, …) into the gateway `signBurnIntents` binding.
 *
 * @remarks
 * Used by {@link isContractSigner} to detect on-chain bytecode at the
 * prospective signer address. Gateway validates EOA signatures with
 * `ecrecover` and contract-account signatures with ERC-1271, but it does not
 * infer which one applies — the transfer request has to declare it, and this
 * hook is how the binding works out which to declare.
 *
 * Implementations should return `'0x'` (or `undefined`) for an EOA and the
 * raw deployed bytecode for a contract account. Errors are caught at the call
 * site so an RPC failure falls back to the EOA path rather than blocking
 * signing.
 */
type EVMReadBytecodeCallback = (address: string, chain: ChainDefinition) => Promise<`0x${string}` | undefined>;

/**
 * EVM action factory.
 *
 * @remarks
 * Composes individual token and native bindings into a full `EVMActionRegistry`.
 * Each binding lives in its own `bindings/` subfolder; this file only assembles them.
 *
 * @packageDocumentation
 */

/**
 * EVM action registry extending base actions with CCTP v2 bridge operations.
 *
 * @example
 * ```typescript
 * import type { EVMActionRegistry } from '@core/adapter-evm-base'
 *
 * const actions: EVMActionRegistry = createEVMActions(adapter, { getNativeBalance })
 * await actions['cctp.v2.depositForBurn'].prepare({ amount: 1000000n, ... }, { chain })
 * ```
 */
interface EVMActionRegistry extends BaseActionRegistry<EvmAuthorizationPayload> {
    'cctp.v2.depositForBurn': WriteAction<CCTPDepositForBurnInput, void, EvmAuthorizationPayload>;
    'cctp.v2.receiveMessage': WriteAction<CCTPReceiveMessageInput, void, EvmAuthorizationPayload>;
    'cctp.v2.customBurn': WriteAction<CCTPCustomBurnInput, void, EvmAuthorizationPayload>;
    'cctp.v2.depositForBurnWithHook': WriteAction<CCTPDepositForBurnWithHookInput, void, EvmAuthorizationPayload>;
    'cctp.v2.customBurnWithHook': WriteAction<CCTPCustomBurnWithHookInput, void, EvmAuthorizationPayload>;
    'gateway.v1.deposit': WriteAction<GatewayDepositInput, void, EvmAuthorizationPayload>;
    'gateway.v1.depositFor': WriteAction<GatewayDepositForInput, void, EvmAuthorizationPayload>;
    'gateway.v1.depositWithPermit': WriteAction<GatewayDepositWithPermitInput, void, EvmAuthorizationPayload>;
    'gateway.v1.depositWithAuthorization': WriteAction<GatewayDepositWithAuthorizationInput, void, EvmAuthorizationPayload>;
    'gateway.v1.addDelegate': WriteAction<GatewayAddDelegateInput, void, EvmAuthorizationPayload>;
    'gateway.v1.removeDelegate': WriteAction<GatewayRemoveDelegateInput, void, EvmAuthorizationPayload>;
    'gateway.v1.isDelegate': ReadAction<GatewayIsDelegateInput, GatewayIsDelegateOutput, EvmAuthorizationPayload>;
    'gateway.v1.initiateWithdrawal': WriteAction<GatewayInitiateWithdrawalInput, void, EvmAuthorizationPayload>;
    'gateway.v1.withdraw': WriteAction<GatewayWithdrawInput, void, EvmAuthorizationPayload>;
    'gateway.v1.withdrawingBalance': ReadAction<GatewayWithdrawingBalanceInput, GatewayWithdrawingBalanceOutput, EvmAuthorizationPayload>;
    'gateway.v1.withdrawalBlock': ReadAction<GatewayWithdrawalBlockInput, GatewayWithdrawalBlockOutput, EvmAuthorizationPayload>;
    'gateway.v1.gatewayBurn': WriteAction<GatewayBurnInput, void, EvmAuthorizationPayload>;
    'gateway.v1.gatewayMint': WriteAction<GatewayMintInput, void, EvmAuthorizationPayload>;
    'gateway.v1.signBurnIntents': ReadAction<GatewaySignBurnIntentsInput, GatewaySignatureOutput, EvmAuthorizationPayload>;
    'swap.execute': WriteAction<ExecuteSwapInput, void, EvmAuthorizationPayload>;
    'usdt.transfer': WriteAction<{
        to: string;
        amount: bigint;
    }, void, EvmAuthorizationPayload>;
    'earn.deposit': WriteAction<EarnExecuteInput, void, EvmAuthorizationPayload>;
    'earn.withdraw': WriteAction<EarnExecuteInput, void, EvmAuthorizationPayload>;
    'earn.claimRewards': WriteAction<EarnExecuteInput, void, EvmAuthorizationPayload>;
}
/**
 * Configuration options for creating EVM actions.
 *
 * @remarks
 * `getNativeBalance` is the only factory option because native-token balance
 * queries are platform-specific: viem calls `getBalance`, ethers calls
 * `provider.getBalance`, etc. All other actions go through the generic
 * `ctx.read` abstraction (contract calls via ABI), so no injection is needed.
 *
 * @example
 * ```typescript
 * import type { CreateEVMActionsOptions } from '@core/adapter-evm-base'
 *
 * const options: CreateEVMActionsOptions = {
 *   getNativeBalance: async (chain, wallet) => 1000000000000000000n,
 * }
 * ```
 */
interface CreateEVMActionsOptions {
    /**
     * Fetch the native token balance for a wallet.
     *
     * @remarks
     * Injected here because native balance queries bypass the ERC-20 ABI path
     * and require a platform-specific RPC call (e.g. `eth_getBalance`).
     */
    readonly getNativeBalance: GetNativeBalance;
    /**
     * Sign EIP-712 typed data via the ecosystem adapter's wallet surface.
     *
     * @remarks
     * Injected here because off-chain signing bypasses the `ctx.read` /
     * `ctx.write` ABI paths — ecosystem adapters (viem, ethers, …) each
     * expose their own `signTypedData` primitive. When provided, the
     * `gateway.v1.signBurnIntents` action delegates to this callback; when
     * omitted, the action returns a no-op sentinel signature (`'0x'`) to
     * preserve backward-compat with callers that have not wired a signer.
     *
     * @example
     * ```typescript
     * createEVMActions(adapter, {
     *   getNativeBalance,
     *   signTypedData: async (typedData, ctx) => {
     *     return adapter.signTypedData(typedData, ctx)
     *   },
     * })
     * ```
     */
    readonly signTypedData?: EVMSignTypedDataCallback;
    /**
     * Read raw on-chain bytecode (`eth_getCode`) for a given address and chain.
     *
     * @remarks
     * Injected here so the `gateway.v1.signBurnIntents` binding can classify
     * the signer before producing a Gateway burn-intent signature. Gateway
     ...
     * transfer request has to declare it.
     *
     * When provided alongside `signTypedData`, the binding fetches bytecode for
     * the resolved signer address and reports `contractSigner: true` when
     * bytecode is present (and is not an EIP-7702 delegation prefix). When
     * omitted, the signer is reported as an EOA.
     *
     * @example
     * ```typescript
     * createEVMActions(deps, {
     *   getNativeBalance,
     *   signTypedData,
     ...
     * ```
     */
    readonly readBytecode?: EVMReadBytecodeCallback;
}
/**
 ...
 */
declare function createEVMActions(adapter: EVMAdapter, options: CreateEVMActionsOptions): EVMActionRegistry;

/**
 * Shared EVM Zod schemas and types.
 *
 * @packageDocumentation
 */
 ...
 * ```
 */
declare const hexString: z.ZodType<`0x${string}`, z.ZodTypeDef, `0x${string}`>;
/**
 ...
 * ```
 */
declare const hexAddress: z.ZodType<`0x${string}`, z.ZodTypeDef, `0x${string}`>;
/**
 ...
 * ```
 */
declare const hexBytes32: z.ZodType<`0x${string}`, z.ZodTypeDef, `0x${string}`>;
/** A raw EVM transaction ready for estimation, simulation, or execution. */
interface RawTransaction {
    /** The recipient contract or wallet address. */
    readonly to: `0x${string}`;
...
/**
 * Gas and nonce overrides for transaction execution.
 *
 * @remarks
 * Two pricing models are supported — use one or the other, not both:
 *
 * - **EIP-1559** (`maxFeePerGas` + `maxPriorityFeePerGas`): Preferred
 *   for most modern EVM chains (mainnet, L2s). Set the maximum total
 *   fee you're willing to pay and the tip for validators.
 *
 * - **Legacy** (`gasPrice`): For chains that don't support EIP-1559
 *   (some L2s, sidechains). Sets a flat per-gas price.
 *
 * In most cases you should **not** set these — the gas pricing resolver
 * handles estimation automatically with a buffer for reliable inclusion.
 * Only override when you have specific requirements (e.g. priority
 * transactions, fixed-price environments).
 */
interface GasOverrides {
    /** Gas limit override. When omitted, estimated automatically. */
    readonly gas?: bigint | undefined;
    /** EIP-1559 max fee per gas (in wei). Mutually exclusive with `gasPrice`. */
    readonly maxFeePerGas?: bigint | undefined;
    /** EIP-1559 max priority fee per gas (in wei). Mutually exclusive with `gasPrice`. */
    readonly maxPriorityFeePerGas?: bigint | undefined;
    /** Legacy gas price (in wei). Mutually exclusive with `maxFeePerGas`/`maxPriorityFeePerGas`. */
    readonly gasPrice?: bigint | undefined;
    /** Transaction nonce override. When omitted, resolved from the network. */
    readonly nonce?: number | undefined;
}
/** Raw transaction with optional gas/nonce overrides. */
interface ExecuteTransactionInput {
    /** The raw transaction to execute. */
    readonly raw: RawTransaction;
    /** Optional gas and nonce overrides. */
    readonly overrides?: GasOverrides | undefined;
}

/**
 * Fee calculation raw data for EVM transactions.
 *
 * @example
 * ```typescript
 * import type { EVMFeeRaw } from '@core/adapter-evm-base'
 *
 * const feeRaw: EVMFeeRaw = {
 *   gasEstimate: 21000n,
...
/**
 * EVM fee estimation result with required gas estimate.
 *
 * @remarks
 * Extends {@link EVMFeeRaw} with a mandatory `gasEstimate` field.
 *
 * @example
 * ```typescript
 * import type { EVMEstimateRaw } from '@core/adapter-evm-base'
 *
 * const estimateRaw: EVMEstimateRaw = {
 *   gasEstimate: 21000n,
 *   maxFeePerGas: 30_000_000_000n,
 *   maxPriorityFeePerGas: 2_000_000_000n,
 * }
 * ```
 */
interface EVMEstimateRaw extends EVMFeeRaw {
    readonly gasEstimate: bigint;
}
/**
 * Input parameters for EVM transaction fee estimation.
 *
 * @example
 * ```typescript
 * import type { EstimateInput } from '@core/adapter-evm-base'
 *
 ...
 * ```
 */
type EstimateInput = RawTransaction;
/**
 * Fee estimation result for EVM transactions.
 *
 * @example
 * ```typescript
 * import type { EstimateResult } from '@core/adapter-evm-base'
 *
 * const result: EstimateResult = {
 *   raw: { gasEstimate: 21000n, maxFeePerGas: 30_000_000_000n },
 *   cost: Amount.of(630_000_000_000_000n, { decimals: 18 }),
 * }
 * ```
 */
type EstimateResult = FeeEstimate<EVMEstimateRaw>;
/**
 * Primitive function type for estimating EVM transaction fees.
 *
 * @example
 * ```typescript
 * import type { EstimatePrimitive } from '@core/adapter-evm-base'
 *
 * const estimate: EstimatePrimitive = async (input) => ({
 *   raw: { gasEstimate: 21000n },
 *   cost: Amount.of(0n, { decimals: 18 }),
 * })
 * ```
 */
type EstimatePrimitive = PrimitiveFunction<EstimateInput, EstimateResult, EvmAuthorizationPayload>;

/** Raw simulation result from an EVM transaction simulation. */
interface EVMSimulateRaw {
    readonly returnData?: `0x${string}` | undefined;
}
/** Input for EVM transaction simulation (raw transaction). */
type SimulateInput = RawTransaction;
/** Result of an EVM transaction simulation. */
type SimulateResult = Simulation<EVMSimulateRaw>;
/** Primitive function signature for EVM transaction simulation. */
type SimulatePrimitive = PrimitiveFunction<SimulateInput, SimulateResult, EvmAuthorizationPayload>;

/**
 * Input for a pre-built raw EVM transaction.
 *
 * @remarks
 * Use this variant when you already have an encoded transaction
 * (e.g. from a third-party aggregator or multisig builder) and
 * want the prepare pipeline to handle estimation, simulation,
 * and execution without re-encoding.
 *
 * @example
 * ```typescript
 * const input: PrepareRawInput = {
 *   type: 'raw',
 *   raw: {
 ...
 * ```
 */
interface PrepareRawInput {
    /** Discriminator — must be `'raw'` for pre-built transactions. */
    readonly type: 'raw';
    /** The pre-built raw transaction. */
    readonly raw: RawTransaction;
}
/**
 * Input for a plain native-token transfer (ETH send).
 *
 * @remarks
 * Carries only the recipient address and value — there is no ABI, function
 * name, or calldata. Use this variant (or {@link createNativeTransferWrite})
 * to transfer the chain's native currency without triggering a smart-contract
 * call. The prepare pipeline emits a `sendTransaction` (no `data`) for this
 * variant.
 *
 * @example
 * ```typescript
 * const input: PrepareNativeInput = {
 *   type: 'native',
 ...
 * ```
 */
interface PrepareNativeInput {
    /** Discriminator — must be `'native'` for native-token transfers. */
    readonly type: 'native';
    /** Recipient address. */
    readonly address: `0x${string}`;
    /** Amount in wei (or the chain's smallest unit). */
    readonly value: bigint;
}

/**
 * Chain-native finality requirement.
 *
 * @remarks
 * For EVM chains this is the number of block confirmations.
 *
 * @example
 * ```typescript
 * import type { ChainNativeFinality } from '@core/adapter-evm-base'
 *
 * const finality: ChainNativeFinality = 12 // 12 block confirmations
 * ```
 */
type ChainNativeFinality = number | string;
/**
 * Input for waiting for transaction confirmation.
 *
 * @example
 * ```typescript
 * import type { WaitForTransactionInput } from '@core/adapter-evm-base'
 *
 * const input: WaitForTransactionInput = {
 ...
 * ```
 */
interface WaitForTransactionInput {
    readonly txId: string;
    readonly finality?: ChainNativeFinality | undefined;
...
/**
 * Types for the legacy compatibility layer.
 *
 * @remarks
 * Defines:
 * - `StandardAdapter` — the new clean adapter contract all adapters implement
 * - `LegacyCompatConfig` — ecosystem-specific configuration for the HOC
 * - `LegacyCompatibleAdapterBase` — concrete `Adapter` subclass that the HOC instantiates
 * - `LegacyCompatibleAdapter<T>` — result type that combines new methods with the `Adapter` class
 * - Legacy return types (`LegacyEstimatedGas`, `LegacyPreparedChainRequest`, `LegacyWaitForTransactionResponse`)
 *
 * @packageDocumentation
 */

/**
 * The new clean adapter contract that all adapters implement.
 *
 * @remarks
 * This interface captures the canonical adapter shape. Ecosystem adapters
 * (viem, ethers, solana) may extend it with additional methods (e.g. `signTypedData`),
 * but this is the minimum surface that the compat layer wraps.
 *
 * @typeParam TContext - The adapter context type (e.g. viem's `AdapterContext`).
 * @typeParam TActions - The action registry type (e.g. `EVMActionRegistry`).
 * @typeParam TChain - The chain definition type.
 */
interface StandardAdapter {
    /** Adapter capabilities. */
    readonly capabilities: AdapterCapabilities$1;
    /** Chain ecosystem type (e.g. `'evm'`, `'solana'`). */
    readonly chainType: ChainType;
    /** Validate that a chain is supported by this adapter. */
    readonly validateChainSupport: (chain: ChainDefinition) => void;
    /**
     * Wait for a transaction to be confirmed (new 2-arg signature).
     */
    readonly waitForTransaction: (input: {
        txId: string;
        finality?: number | string;
        timeout?: number;
    }, meta: OperationMeta) => Promise<WaitResult>;
    /**
     * Action dispatch — resolve an action by key.
     *
     * @remarks
     * Used by the legacy `prepareAction` wrapper to look up individual
     * actions without requiring the full action registry.
     */
    readonly action: (key: string) => unknown;
    /**
     * Read a token's decimal precision on-chain.
     *
     * @remarks
     * Declared here (rather than left as an ecosystem-specific extra) so the
     * type system — not naming convention — ties each adapter's
     * implementation to the {@link LegacyCompatibleAdapterBase.getTokenDecimals}
     * stub that `withLegacyCompat` falls back to. Every adapter passed to
     * `withLegacyCompat` must supply this, or the call fails to typecheck.
     *
     * @param tokenAddress - The token's contract address or mint.
     * @param chain - The chain the token lives on.
     * @returns The token's decimal precision.
     */
    readonly getTokenDecimals: (tokenAddress: string, chain: ChainDefinition) => Promise<number>;
}
/**
 * Legacy estimated gas result.
 *
 * @deprecated Prefer the structured `FeeEstimate` returned by `calculateFee`.
 */
...
/**
 * Legacy prepared chain request returned by `prepareAction`.
 *
 * @remarks
 * The optional `type` field enables noop detection: when a write action
 * calls `ctx.noop()`, the compat layer sets `type: 'noop'` so that
 * downstream consumers (e.g. `executePreparedChainRequest`) can skip
 * execution without calling `estimate()` or `execute()`. For EVM
 * ecosystems the enricher hook may additionally set `type: 'evm'` and
 * expose `getCallData()` so batched-execution consumers can access the
 * raw `{ to, data, value }` tuple.
 *
 * @deprecated Use the action-based API instead.
 */
interface LegacyPreparedChainRequest {
    /**
     * Discriminator.
     *
     * - `'noop'` for no-op results.
     * - `'evm'` when an EVM enricher has attached `getCallData()`.
     */
    readonly type?: 'noop' | 'evm' | undefined;
    estimate(): Promise<LegacyEstimatedGas>;
    /**
     * Submit the prepared transaction and return the tx identifier
     * **without waiting for confirmation**.
     *
     * @remarks
     * Mirrors the legacy `EvmPreparedChainRequest.execute(overrides?)` /
     * `SolanaPreparedChainRequest.execute(overrides?)` contract: the
     * promise resolves with the submitted txId as soon as the wallet
     * accepts the transaction. Callers (typically AppKit and the swap
     * provider) perform their own confirmation wait via
     * `adapter.waitForTransaction(...)`.
     *
     * The `overrides` argument forwards an ecosystem-specific override
     * record. The compat layer normalizes legacy field names (e.g. EVM
     * `gasLimit` → `gas`) before handing it to the underlying primitive.
     */
    execute(overrides?: unknown): Promise<unknown>;
    /**
     * Access the raw call tuple for EIP-5792 batched execution.
     *
     * @remarks
     * Populated by an ecosystem-specific enricher (see
     * {@link LegacyCompatConfig.enrichWriteRequest}) for single-transaction
     * EVM write actions. Adapters that don't implement calldata extraction
     * (or multi-step write actions) will omit this accessor.
     *
     * @returns The raw `{ to, data, value }` tuple.
     * @since 2.0.0
     */
    getCallData?(): {
        to: `0x${string}`;
...
/**
 * The minimal shape of a prepared write result the enricher can read from.
 *
 * @remarks
 * Matches the structural shape produced by `@core/adapter-base`'s write
 * action `.prepare()` — just enough for ecosystem-specific enrichers to
 * peek at the first built transaction without taking a hard dependency
 * on `PrepareResult<...>`'s generics.
 */
interface LegacyEnrichablePreparedResult {
    /** The prepared transactions. Ecosystem enrichers narrow this structurally. */
    readonly transactions: readonly unknown[];
}
/**
 * Hook used by the compat layer to enrich a single-transaction legacy
 * prepared request with ecosystem-specific fields (e.g. EVM `type: 'evm'`
 * and `getCallData()`).
 *
 * @param legacy - The base legacy prepared request (estimate + execute only).
 * @param prepared - The underlying new-style prepare result (read-only).
 * @returns The enriched legacy prepared request, or the unchanged input.
 */
type LegacyWriteRequestEnricher = (legacy: LegacyPreparedChainRequest, prepared: LegacyEnrichablePreparedResult) => LegacyPreparedChainRequest;
type InvocationAwarePrepareActionFn<TAuthorizationPayload extends AuthorizationPayload> = (action: string, params: Record<string, unknown>, ctx: OperationMeta, invocation?: AdapterInvocationMeta<TAuthorizationPayload>) => Promise<ReturnType<LegacyWriteRequestEnricher>>;
/**
 ...
 */
declare class LegacyCompatibleAdapterBase extends Adapter {
    chainType: ChainType | 'hybrid';
    prepare(_params: PreparedChainRequestParams, _ctx: OperationContext): Promise<PreparedChainRequest>;
    getAddress(_chain: ChainDefinition): Promise<string>;
    switchToChain(_chain: ChainDefinition): Promise<void>;
    waitForTransaction(_txHash: string, _config: WaitForTransactionConfig | undefined, _chain: ChainDefinition): Promise<WaitForTransactionResponse>;
    calculateTransactionFee(_baseComputeUnits: bigint, _bufferBasisPoints: bigint | undefined, _chain: ChainDefinition): Promise<EstimatedGas>;
    /**
     ...
     * ```
     */
    getTokenDecimals(_tokenAddress: string, _chain: ChainDefinition): Promise<number>;
}
/**
 * The public result type of `withLegacyCompat`.
 *
 * @remarks
 * Combines the new adapter's unique methods (those NOT on `Adapter`) with
 * `LegacyCompatibleAdapterBase extends Adapter`. This avoids the intersection
 * conflicts that occur when both `TAdapter` and `Adapter` define the same
 * method with different signatures (e.g. `waitForTransaction`).
 *
 * **`prepare` is exposed as an overload** of `TAdapter['prepare']` AND the
 * legacy `Adapter.prepare(params, ctx)` signature. At runtime
 * `Object.assign(instance, adapter, legacyMethods)` overwrites the legacy
 * `Adapter.prepare(...)` stub with the new `prepare(input, meta) =>
 * PrepareResult<...>` signature from `TAdapter`, but consumers still treat
 * the wrapped adapter as a structural `Adapter` in many places (e.g. the
 * compass-e2e fork helpers). Using the intersection
 * `TAdapter['prepare'] & LegacyCompatibleAdapterBase['prepare']` preserves
 * assignability to the legacy `Adapter` shape **and** lets the new README
 * examples (`prepared.transactions`, `prepared.single()`,
 * `prepared.estimateTotal()`, `prepared.execute()`) compile under strict
 * mode without `as any`. See #853 round-2 review (finding N9).
 *
 * Other properties overlapping with `Adapter` (like `chainType`,
 * `capabilities`, `waitForTransaction`, `getAddress`, `validateChainSupport`,
 * `calculateTransactionFee`) come from the `Adapter` side via the class.
 * New adapter-specific properties (`ctx`, `read`, `signTypedData`, `action`,
 * `calculateFee`) are preserved from `TAdapter`.
 *
 * At runtime, `Object.assign` copies all methods from `TAdapter` onto the
 * class instance, so both old and new calling conventions work.
 */
type LegacyCompatibleAdapter<TAdapter extends StandardAdapter = StandardAdapter> = Omit<TAdapter, keyof Adapter | 'prepare'> & Omit<LegacyCompatibleAdapterBase, 'prepare' | 'prepareAction' | 'capabilities'> & {
    /**
     * Adapter capabilities, preserved from the wrapped adapter (A1).
     *
     * @remarks
     * Sourced from `TAdapter['capabilities']` rather than the legacy
     * `Adapter` base so the concrete `addressContext` literal (e.g.
     * `'developer-controlled'`) survives compat wrapping. This is what lets
     * kits infer address-context requirements from `typeof adapter`. A
     * narrowed capability type is still structurally assignable to the legacy
     * `Adapter['capabilities']` shape, so existing consumers are unaffected.
     */
    readonly capabilities: TAdapter['capabilities'];
    prepare: ('prepare' extends keyof TAdapter ? TAdapter['prepare'] : LegacyCompatibleAdapterBase['prepare']) & LegacyCompatibleAdapterBase['prepare'];
    /** Prepare an action while preserving request-scoped adapter metadata. */
    prepareAction: InvocationAwarePrepareActionFn<AdapterAuthorizationPayloadOf<TAdapter>> & LegacyCompatibleAdapterBase['prepareAction'];
};

/**
 * Configuration options for creating a Viem adapter context.
 *
 * @typeParam TCapabilities - The adapter capabilities type.
 *
 * @example
 ...
 * ```
 */
interface AdapterContextOptions<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> extends EvmAuthorizationOptions {
    /**
     * Adapter capabilities defining address control and supported chains.
     */
    readonly capabilities: TCapabilities;
    /**
     * Factory function to create PublicClient instances for read operations.
     *
     * @remarks
     * Called when a PublicClient is needed for a specific chain.
     * Results are cached per chain ID.
     *
     * @param params - Parameters including the target viem Chain
     * @returns PublicClient instance (or promise resolving to one)
     */
    getPublicClient: (params: {
        chain: Chain;
    }) => Promise<PublicClient> | PublicClient;
    /**
     * Factory function to create WalletClient instances for signing/write operations.
     *
     * @remarks
     * Called when a WalletClient is needed for a specific chain.
     * - For `user-controlled` adapters: Called fresh each time (no caching)
     * - For `developer-controlled` adapters: Results are cached per chain ID
     *
     * Optional when a {@link AdapterContextOptions.signing | signing} strategy is
     * provided (the strategy owns authorization, so the adapter runs keyless) or
     * for a pure read-only adapter (reads, gas estimation, and simulation use only
     * the PublicClient). When omitted, any operation that needs a WalletClient
     * fails with a clear error.
     *
     * @param params - Parameters including the target viem Chain
     * @returns WalletClient instance (or promise resolving to one)
     */
    getWalletClient?: (params: {
        chain: Chain;
    }) => Promise<WalletClient> | WalletClient;
    /**
     * Pluggable signing strategy for transaction authorization.
     *
     * @remarks
     * When provided, write operations (`execute`, `signTypedData`, `batchExecute`)
     * are routed through the strategy instead of a WalletClient: the adapter
     * builds each ready-to-sign payload and the strategy decides how it gets
     * authorized — in-process, by the caller's own infrastructure (see
     * `externalSigning` from `@core/adapter-base`), or by a remote service.
     * Read operations, gas estimation, pre-flight simulation, and receipt waiting
     * are unaffected and keep using the PublicClient.
     *
     * When the strategy returns an already-broadcast transaction hash the adapter
     * does not submit anything itself; when it returns signed bytes the adapter
     * validates the echoed signer against the expected sender and submits them via
     * `eth_sendRawTransaction`. Dispatch is capability-gated by the strategy's
     * manifest — a payload family the manifest does not declare fails with a clear
     * error before the strategy is invoked.
     */
    signing?: SigningStrategy | undefined;
    /**
     * A fixed address to bind to a keyless or read-only adapter.
     *
     * @remarks
     * For `user-controlled` adapters that have no WalletClient (a read-only
     * adapter, or one whose signing is owned by a {@link
     * AdapterContextOptions.signing | signing} strategy), there is no wallet to
     * resolve the current address from. Set this so `getAddress()` — and the
     * per-operation address resolution that read verbs rely on — returns a
     * concrete sender without a hand-rolled wallet stub. Ignored for
     * `developer-controlled` adapters, which always take the address per call.
     */
     ...
     *
     * @remarks
     * Pass a pre-created `Runtime` to share the same event bus and logger
     * across the adapter and transport instrumentation, or pass
     * `RuntimeOptions` to let the context create one.
     */
    ...
    /**
     * Optional operational configuration (retry, etc.).
     */
    config?: OperationalConfig;
    /**
     * Force-enable WalletClient caching regardless of `addressContext`.
     *
     * @remarks
     * By default, WalletClients are only cached for `developer-controlled`
     * adapters (because user-controlled wallets may switch accounts).
     *
     * Set to `true` for adapters with a deterministic account (e.g. private
     * key) where the account never changes and reusing the client is safe and
     * necessary for viem's internal nonce tracking (`nonceManager`).
     *
     * @defaultValue Inferred from `capabilities.addressContext`
     */
    cacheWalletClients?: boolean;
    /**
     * Optional chain-switch behaviour applied before write operations.
     *
     * @remarks
     * When provided, the context exposes an `ensureChain` hook that
     * `createPrimitive` invokes (via `ensureChainBefore: true`) before
     * running a primitive. The hook reads the wallet's current chain
     * and reconciles it with the target chain according to the chosen
     * `onMismatch` policy.
     *
     * Defaults applied by the factory helpers:
     * - `createViemAdapterFromProvider` — `'prompt'`
     *   (user-controlled wallets should be asked to switch).
     * - `createViemAdapterFromPrivateKey` — `'skip'`
     *   (no wallet UI exists; transport pins the chain).
     */
    chainSwitch?: ChainSwitchConfig;
}
/**
 * Extended adapter context with Viem-specific client accessors.
 *
 * @typeParam TCapabilities - The adapter capabilities type.
 *
 * @remarks
 * Extends the base `AdapterContext` with methods to access Viem clients
 * (PublicClient, WalletClient) with proper caching and lifecycle management.
 *
 * @example
 * ```typescript
 * import { createAdapterContext } from '@circle-fin/adapter-viem-v2/next'
 ...
 * ```
 */
interface AdapterContext<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> extends AdapterContext$1<TCapabilities, EvmAuthorizationPayload> {
    /** Fixed address configured for keyless or read-only authorization. */
    readonly boundAddress?: `0x${string}` | undefined;
    /**
     * Get a PublicClient for the specified chain.
     *
     * @remarks
     * PublicClients are cached per chain ID for efficiency.
     * They are stateless and safe to cache.
     *
     * @param chain - The EVM chain definition
     * @returns Promise resolving to the PublicClient
     */
    getPublicClient: (chain: EVMChainDefinition) => Promise<PublicClient>;
    /**
     * Get a WalletClient for the specified chain.
     *
     * @remarks
     * Caching behavior depends on `addressContext`:
     * - `developer-controlled`: Cached per chain ID (address is explicit)
     * - `user-controlled`: Not cached (user may switch accounts)
     *
     * @param chain - The EVM chain definition
     * @returns Promise resolving to the WalletClient
     */
     ...
     *
     * @remarks
     * Call this when you need to force re-initialization of clients,
     * for example after detecting an account or network change.
     */
    resetClients: () => void;
    /**
     * Whether a WalletClient factory was configured.
     *
     * @remarks
     * `true` when the adapter can produce a WalletClient (wallet-backed);
     * `false` for read-only adapters and for keyless adapters that route
     * authorization through a {@link AdapterContext.signing | signing} strategy.
     * Capability probes such as `supportsSignTypedData` consult this so a
     * read-only adapter never claims a signing capability it cannot fulfil.
     */
    readonly hasWalletClient: boolean;
    /**
     * The signing strategy that owns transaction authorization, when one was
     * configured. Primitives route write operations through it instead of the
     * WalletClient. Absent for wallet-backed and read-only adapters.
     */
    readonly signing?: SigningStrategy | undefined;
}
 ...
 * ```
 */
declare function createAdapterContext<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1>(options: AdapterContextOptions<TCapabilities>): AdapterContext<TCapabilities>;

/**
 * Resolve a viem {@link Chain} from a `ChainDefinition` object — the
 * Phase-D, enum-free replacement for `getViemChainByEnum` used by the
 ...
 * ```
 */
declare function resolveViemChain(definition: EVMChainDefinition): Chain;

/**
 * Input for reading a contract function.
 *
 * @remarks
 * Mirrors viem's `readContract` parameters with type-safe address format.
 */
interface ReadContractInput {
    /**
     ...
     *
     * @remarks
     * Can be a full ABI array or a narrowed subset containing just the
     ...
     *
     * @remarks
     * Arguments are passed in order and must match the function signature.
     */
    readonly args?: readonly unknown[] | undefined;
}
/**
 * The read primitive function type.
 *
 * @remarks
 * Returns a generic primitive that allows callers to specify the output type.
 */
type ReadPrimitive = GenericPrimitiveFunction<ReadContractInput, EvmAuthorizationPayload>;
/**
 ...
 * ```
 */
declare function createRead(ctx: AdapterContext): ReadPrimitive;

/**
 * EVM-specific raw receipt data.
 */
interface EVMReceiptRaw {
    /** Full viem transaction receipt. */
    readonly receipt: TransactionReceipt;
}
/**
 * Result type for waitForTransaction.
 */
type WaitForTransactionResult = WaitResult<EVMReceiptRaw>;
/**
 * The waitForTransaction primitive function type.
 */
type WaitForTransactionPrimitive = PrimitiveFunction<WaitForTransactionInput, WaitForTransactionResult, EvmAuthorizationPayload>;
/**
 ...
 * ```
 */
declare function createWaitForTransaction(ctx: AdapterContext): WaitForTransactionPrimitive;

/**
 * Viem-adapter-specific raw data for fee estimates.
 *
 * @remarks
 * Contains both EIP-1559 and legacy fee data, depending on chain support.
 * Distinct from `EVMFeeRaw` in `@core/adapter-evm-base` where `gasEstimate`
 * is optional — here it is always present on the calculate-fee result.
 *
 * @example
 * ```typescript
 * import type { ViemFeeRaw } from '@circle-fin/adapter-viem-v2/next'
 *
 * const feeRaw: ViemFeeRaw = {
 *   gasEstimate: 21000n,
 *   maxFeePerGas: 30_000_000_000n,
 *   maxPriorityFeePerGas: 2_000_000_000n,
 * }
 * ```
 */
interface ViemFeeRaw {
    /** Gas estimate (same as units, included for convenience). */
    readonly gasEstimate: bigint;
    /** Max fee per gas (EIP-1559 chains). */
    readonly maxFeePerGas?: bigint | undefined;
    /** Max priority fee per gas (EIP-1559 chains). */
    readonly maxPriorityFeePerGas?: bigint | undefined;
    /** Legacy gas price (pre-EIP-1559 chains or fallback). */
    readonly gasPrice?: bigint | undefined;
}
/**
 * Input for the calculateFee primitive.
 *
 * @remarks
 * Use this when you know the gas units (from historical data, constants,
 * or a previous estimate) and want to calculate the current fee.
 */
interface CalculateFeeInput {
    /**
     * Gas units for the transaction.
     *
     * @remarks
     * This is the gas limit or estimated gas consumption.
     */
    readonly gasUnits: bigint;
    /**
     * Buffer to add to the fee in basis points.
     *
     * @remarks
     * Default is 2 000 (20%). This accounts for gas price fluctuations
     * between estimation and execution, especially with free/public RPCs.
     *
     * Cascade: per-call value → `config.transaction.feePriceBufferBps` → 2 000.
     *
     * @example
     * - 500 = 5% buffer
     * - 2000 = 20% buffer (default)
     * - 5000 = 50% buffer
     */
    readonly bufferBasisPoints?: bigint | undefined;
}
/**
 * EVM fee estimate type alias.
 *
 * @remarks
 * Uses the universal `FeeEstimate` type with EVM-specific raw data.
 * The `fee` Amount uses 18 decimals (native token precision for EVM chains).
 */
type EVMFeeEstimate = FeeEstimate<ViemFeeRaw>;
/**
 * The calculateFee primitive function type.
 */
type CalculateFeePrimitive = (input: CalculateFeeInput, operation: OperationMeta, invocation?: AdapterInvocationMeta<EvmAuthorizationPayload>) => Promise<EVMFeeEstimate>;
/**
 ...
 * ```
 */
declare function createCalculateFee(ctx: AdapterContext): CalculateFeePrimitive;
 ...
 * ```
 */
declare function createEstimate(ctx: AdapterContext, calculateFee: CalculateFeePrimitive): EstimatePrimitive;
 ...
 * ```
 */
declare function createSimulate(ctx: AdapterContext): SimulatePrimitive;

/**
 * Execute primitive for sending EVM transactions.
 *
 * @packageDocumentation
 */

/**
 * EVM-specific execution raw data.
 */
interface EVMExecuteRaw {
    /** Transaction receipt (available after waiting). */
    readonly receipt?: TransactionReceipt | undefined;
}
/**
 * Execute result type.
 */
type EVMExecuteResult = ExecuteResult<EVMExecuteRaw>;
/**
 * Execute primitive function type.
 */
type ExecutePrimitive = PrimitiveFunction<ExecuteTransactionInput, EVMExecuteResult, EvmAuthorizationPayload>;
/**
 ...
 * ```
 */
declare function createExecute(ctx: AdapterContext, waitForTransaction: WaitForTransactionPrimitive): ExecutePrimitive;
...
/**
 * Input for the signTypedData primitive.
 *
 * @typeParam Types - Mapping of struct names to their field definitions.
 * @typeParam Message - The message payload type.
 *
 * @remarks
 * Follows the EIP-712 typed data structure for secure off-chain signing.
 * The signature can be used for permits, meta-transactions, and other
 * gasless operations.
 */
interface SignTypedDataInput<Types extends Record<string, TypedDataField[]> = Record<string, TypedDataField[]>, Message extends Record<string, unknown> = Record<string, unknown>> {
    /**
     * EIP-712 domain separator.
     *
     * @remarks
     * Identifies the dApp/contract and prevents replay attacks across chains.
     */
    readonly domain: {
        readonly name?: string | undefined;
        readonly version?: string | undefined;
     ...
     *
     * @remarks
     * Defines the structure of the message being signed.
     * Must include all types referenced in the message.
     */
    readonly types: Types;
    /**
     * The root struct type being signed.
     *
     * @remarks
     * Must be a key in the `types` mapping.
     */
    readonly primaryType: keyof Types & string;
    /**
     * The message payload to be signed.
     *
     * @remarks
     * Must conform to the structure defined in `types[primaryType]`.
     */
    readonly message: Message;
}
/**
 * Result of signing typed data.
 *
 * @remarks
 * Returns the full signature as a hex string. Use `parseSignature` from
 * `@core/adapter-evm` to extract (v, r, s) components if needed.
 */
type SignTypedDataResult = `0x${string}`;
/**
 * The signTypedData primitive function type.
 */
type SignTypedDataPrimitive = PrimitiveFunction<SignTypedDataInput, SignTypedDataResult, EvmAuthorizationPayload>;
/**
 ...
 * ```
 */
declare function createSignTypedData(ctx: AdapterContext): SignTypedDataPrimitive;

/**
 * Prepare primitive for EVM write operations.
 *
 * @remarks
 * Composes standalone primitives (estimate, simulate, execute) into a single
 * prepare pipeline. Each prepared transaction delegates to the primitives
 * rather than reimplementing their logic.
 *
 * Supports two input modes via the {@link PrepareInput} discriminated union:
 * - `PrepareContractInput` — provide ABI + function name; calldata is encoded internally.
 * - `PrepareRawInput` — provide a pre-built {@link RawTransaction}; encoding is skipped.
 *
 * @packageDocumentation
 */

/**
 * Input for a contract write (state-changing call).
 *
 * @remarks
 * Mirrors viem's `writeContract` parameters with type-safe address format.
 * The optional `type` discriminator enables the {@link PrepareInput} union.
 * Omitting `type` (or passing `'contract'`) is equivalent — existing
 * callers that do not provide `type` continue to work unchanged.
 *
 * @example
 * ```typescript
 * import type { PrepareContractInput } from '@circle-fin/adapter-viem-v2/next'
 * import { erc20Abi } from 'viem'
 *
 * const input: PrepareContractInput = {
 *   address: '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48',
 *   abi: erc20Abi,
 ...
 * ```
 */
interface PrepareContractInput {
    /**
     * Discriminator for the prepare input union.
     *
     * @defaultValue `'contract'` (inferred when omitted)
     */
    readonly type?: 'contract' | undefined;
...
/**
 * Discriminated union of all EVM prepare input types.
 *
 * @remarks
 * - `PrepareContractInput` — provide ABI + function name + args; the adapter
 *   encodes calldata internally via viem's `encodeFunctionData`.
 * - `PrepareRawInput` — provide a pre-built {@link RawTransaction}; the
 *   adapter skips encoding and passes it directly to estimate / simulate / execute.
 * - `PrepareNativeInput` — native-token transfer; emits `sendTransaction` with
 *   no calldata.
 */
type PrepareInput = PrepareContractInput | PrepareRawInput | PrepareNativeInput;
/**
 * A prepared EVM transaction with properly typed methods.
 *
 * @remarks
 * Each method delegates to the corresponding standalone primitive:
 * - `estimate()` → EstimatePrimitive
 * - `simulate()` → SimulatePrimitive
 * - `execute(overrides?)` → ExecutePrimitive
 *
 * @example
 * ```typescript
 * import { createPrepare } from '@circle-fin/adapter-viem-v2/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
 * const prepare = createPrepare({ ctx, estimate, simulate, execute })
 * const result = await prepare(input, { chain: Ethereum, address: '0x...' })
 * const tx = result.single()
 *
 * // Use the prepared transaction methods
 * const fee = await tx.estimate()
 * const sim = await tx.simulate()
 * const exec = await tx.execute({ gas: 100000n })
 * ```
 */
interface PreparedEVMTransaction extends PreparedTransactionBase {
    readonly input: PrepareInput;
    readonly built: RawTransaction;
    estimate(): Promise<EstimateResult>;
    simulate(): Promise<SimulateResult>;
    execute(overrides?: GasOverrides): Promise<EVMExecuteResult>;
}
/**
 * The result type returned by the prepare primitive.
 *
 * TRaw is `unknown` because TypeScript's conditional type inference cannot
 * extract it from `execute(overrides?: GasOverrides)`. Individual transaction
 * `execute()` calls still return fully-typed `EVMExecuteResult`.
 */
type EVMPrepareResult = PrepareResult<PrepareInput, RawTransaction, unknown, PreparedEVMTransaction, EvmAuthorizationPayload>;
/**
 * The prepare primitive function type.
 *
 * @remarks
 * Accepts both contract call inputs and pre-built raw transactions
 * via the {@link PrepareInput} discriminated union.
 */
type PreparePrimitive = (input: PrepareInput | PrepareInput[], operation: OperationMeta, invocation?: AdapterInvocationMeta<EvmAuthorizationPayload>) => Promise<EVMPrepareResult>;
/**
 * Options for creating a prepare primitive.
 *
 * @remarks
 * The prepare primitive composes standalone primitives. Each adapter creates
 * these primitives from its context and passes them here.
 */
interface CreatePrepareOptions {
    /** The adapter context. */
    readonly ctx: AdapterContext;
    /** Estimate primitive for fee calculation. */
    readonly estimate: EstimatePrimitive;
    /** Simulate primitive for dry-run testing. */
    readonly simulate: SimulatePrimitive;
    /** Execute primitive for sending transactions. */
    readonly execute: ExecutePrimitive;
}
/**
 ...
 * ```
 */
declare function createPrepare({ ctx: adapterContext, estimate, simulate, execute, }: CreatePrepareOptions): PreparePrimitive;

/**
 * EIP-5792 batched execution primitive for the `/next` Viem adapter.
 *
 * @remarks
 * Exposes two methods — `supportsAtomicBatch(chain)` and
 * `batchExecute(calls, chain, options?)` — that together mirror the legacy
 * `ViemAdapter` public API and can be structurally consumed by providers
 * (e.g. `@circle-fin/provider-cctp.v2`) that gate batched approve+burn on
 * these capabilities.
 *
 * @packageDocumentation
 */
...
/**
 * Per-call receipt information returned by {@link BatchExecuteAPI.batchExecute}.
 *
 * @since 2.0.0
 */
interface BatchExecuteReceipt {
 ...
 *
 * @remarks
 * Once `wallet_sendCalls` has been accepted by the wallet the batch has
 * been submitted — errors during polling are captured as an empty
 * `receipts` array (with the triggering error stashed on `error`) so
 * callers never retry a batch that was already sent.
 *
 * @since 2.0.0
 *
 * @example
...
interface BatchExecuteResult {
    ...
    readonly receipts: BatchExecuteReceipt[];
    /**
...
/**
 * Options for {@link BatchExecuteAPI.batchExecute}.
 *
 * @since 2.0.0
 *
 * @example
 * ```typescript
 * const result = await adapter.batchExecute(calls, Base, {
 *   timeout: 60_000,
...
interface BatchExecuteOptions {
    ...
    /**
     * Whether to require the wallet to execute every call atomically — all
     * succeed or all revert (EIP-5792 `atomicRequired`, surfaced to viem as
     * `forceAtomic`).
     *
     * @remarks
     * Defaults to `true`. EIP-5792 `wallet_sendCalls` itself defaults to
     * NON-atomic execution, so without this the wallet MAY run calls
     * sequentially with independent failure — e.g. a CCTP approve+burn batch
     * could leave the approve confirmed and the burn failed. Set to `false`
     * only when partial execution is acceptable for the flow.
     *
     * @defaultValue true
     */
    readonly atomicRequired?: boolean | undefined;
    /**
     * The address expected to authorize the batch.
     *
     * @remarks
     * Required when execution is routed through a signing strategy — there is no
     * wallet client account to read the sender from. Ignored on the wallet-client
     * (EIP-5792) path, which uses the wallet's own account.
     *
     * @since 2.0.0
     */
    readonly fromAddress?: `0x${string}` | undefined;
    /**
     * Cancellation signal forwarded to the signing strategy.
     *
     * @remarks
     * Threaded into the strategy's `SigningContext` on the signing-strategy batch
     * path so a caller can cancel a pending authorization, matching the
     * single-transaction `execute` primitive. Ignored on the wallet-client
     * (EIP-5792) path, which has no interruptible await before submission.
     *
     * @since 2.0.0
     */
    readonly signal?: AbortSignal | undefined;
    /** Request-scoped intent and lazy semantic review data for this batch. */
    readonly authorization?: AuthorizationDescriptor<EvmAuthorizationPayload> | undefined;
}
/**
 * The shape returned by {@link createBatchExecute}.
 *
 * @remarks
 * Spread onto the adapter via `extras` so consumers see top-level
 * `adapter.supportsAtomicBatch` and `adapter.batchExecute` methods matching
 * the legacy `ViemAdapter` signatures.
 */
interface BatchExecuteAPI {
    /**
     * Report whether atomic batching is available on the given chain.
     *
     * @remarks
     * When a `SigningStrategy` is configured, the answer comes from the
     * strategy's `atomicBatch` manifest capability — no wallet is consulted.
     * Otherwise it detects EIP-5792 support on the connected wallet.
     *
     * Expressed as a property so consumers can destructure without tripping
     * `@typescript-eslint/unbound-method` — the returned functions are
     * closure-bound and do not reference `this`.
     */
    readonly supportsAtomicBatch: (chain: EVMChainDefinition) => Promise<boolean>;
    /**
     * Submit multiple EVM calls as a single atomic batch.
     *
     * @remarks
     * When a `SigningStrategy` is configured, the batch is authorized through
     * the strategy (one authorization for all calls) and requires
     * `options.fromAddress`. Otherwise it is sent to the connected wallet via
     * EIP-5792 `wallet_sendCalls`, polling `wallet_getCallsStatus` until all
     * calls are confirmed (or the poll times out).
     */
    readonly batchExecute: (calls: EvmCallData[], chain: EVMChainDefinition, options?: BatchExecuteOptions) => Promise<BatchExecuteResult>;
}

/**
 * Adapter factory: composes all primitives, then builds the adapter.
 *
 * @packageDocumentation
 */

type AdapterOptions<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> = AdapterContextOptions<TCapabilities>;
/**
 * The clean viem adapter interface (new API).
 *
 * @remarks
 * Derived from {@link AssembledAdapterShape} — the standard adapter shape
 * returned by `assembleAdapter` — plus viem-specific extras.
 *
 * Legacy methods (`prepareAction`, `calculateTransactionFee`,
 * 3-arg `waitForTransaction`) are injected by `withLegacyCompat`
 * and are not part of this type.
 */
type ViemAdapterCore<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> = AssembledAdapterShape<AdapterContext<TCapabilities>, 'evm', GenericPrimitiveFunction<ReadContractInput, EvmAuthorizationPayload>, PreparePrimitive, WaitForTransactionPrimitive, EVMActionRegistry> & {
    /** Calculate fees using the new fee primitive. */
    readonly calculateFee: CalculateFeePrimitive;
    /** Sign EIP-712 typed data. */
    readonly signTypedData: SignTypedDataPrimitive;
    /**
     * Read deployed bytecode for signer classification.
     *
     * @remarks
     * `chain` stays `EVMChainDefinition` on purpose. This member is not part of
     * `StandardAdapter`, so nothing forces it to the wider `ChainDefinition`
     * that `getTokenDecimals` takes, and the narrower type keeps the
     * compile-time check. Do not widen it for symmetry.
     *
     * @param address - EVM account address to inspect.
     * @param chain - Source chain on which to inspect the address.
     * @returns Deployed bytecode, or `'0x'` when the address is an EOA.
     */
    readonly readBytecode: (address: string, chain: EVMChainDefinition) => Promise<`0x${string}` | undefined>;
    /**
     * Read and validate an ERC-20 token's decimal precision.
     *
     * @remarks
     * Takes the ecosystem-agnostic `ChainDefinition` (not `EVMChainDefinition`)
     * so this member's signature matches `StandardAdapter.getTokenDecimals`
     * from `@core/adapter-compat`. `resolveEVMTokenDecimalsFromContract`
     * validates at runtime that `chain` is actually an EVM chain.
     *
     * @param tokenAddress - ERC-20 token contract address.
     * @param chain - Chain containing the token contract. Must be an EVM chain.
     * @returns The validated decimal precision.
     * @throws `KitError` with `INPUT_INVALID_CHAIN` for a non-EVM chain.
     * @throws `KitError` with `INPUT_INVALID_ADDRESS` for an invalid address.
     * @throws `KitError` with `RPC_ENDPOINT_ERROR` when the read fails.
     * @throws `KitError` with `INPUT_VALIDATION_FAILED` when the decoded value is
     * not a uint8.
     */
    readonly getTokenDecimals: (tokenAddress: string, chain: ChainDefinition) => Promise<number>;
    /**
     * Report whether this adapter can produce EIP-712 typed-data signatures.
     *
     * @remarks
     * Implements the `SignTypedDataAdapter` capability contract from
     * `@core/adapter-evm` (consulted by `canSignTypedData`). When a signing
     * strategy is configured the answer comes from its manifest — a strategy
     * that does not declare the `evm-typed-data` payload family reports `false`,
     * letting callers choose an on-chain alternative instead of a permit the
     * capability gate would reject. Wallet-backed adapters report `true`.
     */
    readonly supportsSignTypedData: () => boolean;
    /**
     * Detect whether the connected wallet supports EIP-5792 atomic batching
     * on the given chain.
     */
    readonly supportsAtomicBatch: BatchExecuteAPI['supportsAtomicBatch'];
    /**
     * Submit multiple EVM calls as a single batched wallet request via
     * EIP-5792 `wallet_sendCalls`, polling for completion.
     */
    readonly batchExecute: BatchExecuteAPI['batchExecute'];
};
/**
 * The public viem adapter type — clean interface + legacy compat methods.
 *
 * @remarks
 * Consumers get the new API plus deprecated legacy methods for backward
 * compatibility with older providers/kits.
 *
 * @typeParam TCapabilities - The adapter capabilities type. Preserved so
 *   address-context-aware operation metadata can be inferred from
 *   `typeof adapter`.
 */
type ViemAdapter<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> = LegacyCompatibleAdapter<ViemAdapterCore<TCapabilities>>;
/**
 ...
 * ```
 */
declare function createAdapter<const TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1>(options: AdapterOptions<TCapabilities>): ViemAdapter<TCapabilities>;
/**
 ...
 * ```
 */
declare function createAdapterFromContext<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1>(ctx: AdapterContext<TCapabilities>): ViemAdapter<TCapabilities>;

/**
 * Adapter action bindings.
 *
 * @remarks
 * The only framework-specific piece is `getNativeBalance`.
 * Everything else is standardized in `@core/adapter-evm-base`.
 *
 * @packageDocumentation
 */

/**
 * Minimal shape the action factory needs from the adapter.
 *
 * @internal
 */
interface ActionFactoryDeps {
    readonly ctx: AdapterContext;
    readonly read: GenericPrimitiveFunction<ReadContractInput, EvmAuthorizationPayload>;
    readonly prepare: PreparePrimitive;
    readonly getAddress: (chain: EVMChainDefinition) => Promise<string>;
    readonly signTypedData: SignTypedDataPrimitive;
}
/**
 ...
 * ```
 */
declare function createActionsFromAdapter(deps: ActionFactoryDeps): EVMActionRegistry;
/**
 * Overloaded action dispatch signature.
 *
 * @remarks
 * - `action(key)` returns the typed action object (for `.prepare()`, `.name`, etc.)
 * - `action(key, ...args)` calls the action directly (fast path)
 */
interface ActionDispatch {
    <K extends keyof EVMActionRegistry>(key: K): EVMActionRegistry[K];
    <K extends keyof EVMActionRegistry>(key: K, ...args: Parameters<EVMActionRegistry[K]>): ReturnType<EVMActionRegistry[K]>;
}
/**
 ...
 * ```
 */
declare function createActionDispatch(actions: EVMActionRegistry): ActionDispatch;

/**
 * Shared base parameters for all adapter factories.
 *
 * @remarks
 * Every adapter factory accepts these fields. Factory-specific parameters
 * (e.g., `privateKey`, `provider`) are added by extending this interface.
 *
 * @example
 * ```typescript
 * import { createPublicClient, http } from 'viem'
 * import { mainnet } from 'viem/chains'
 * import type { BaseAdapterParams } from '@circle-fin/adapter-viem-v2/next'
 *
 * const params: BaseAdapterParams = {
 *   getPublicClient: ({ chain }) => createPublicClient({ chain, transport: http() }),
 *   capabilities: { addressContext: 'user-controlled' },
 * }
 * ```
 */
interface BaseAdapterParams extends EvmAuthorizationOptions {
    /** Optional custom public client factory. Defaults to HTTP transport with vetted RPC endpoints. */
    getPublicClient?: (params: {
        chain: Chain;
    }) => Promise<PublicClient> | PublicClient;
    /** Partial capabilities (defaults to user-controlled, all EVM chains). */
    capabilities?: Partial<AdapterCapabilities$1>;
    /** Runtime services or configuration options (logger, metrics). */
    runtime?: Runtime | RuntimeOptions;
    /** Token registry for resolving token selectors. */
    tokens?: TokenRegistry;
    /** Operational config (retry policy, etc.). */
    config?: OperationalConfig;
    /**
     * Chain-switch policy for write operations.
     *
     * @remarks
     * Forwarded to `createAdapterContext` which installs the `ensureChain`
     * hook. Each factory applies its own sensible default when this field
     * is omitted (`prompt` for browser/provider flows, `skip` for
     * private-key/server flows).
     */
    chainSwitch?: ChainSwitchConfig;
}

/**
 * Factory for creating the new Viem adapter from a private key.
 *
 * @packageDocumentation
 */

/**
 * Parameters for creating a Viem adapter from a private key.
 *
 * @example
 * ```typescript
 ...
 * const adapter = createViemAdapterFromPrivateKey({
 *   privateKey: '0xabc...def',
 *   getPublicClient: ({ chain }) => createPublicClient({ chain, transport: http('https://my-rpc.com') }),
 *   getWalletClient: ({ chain, account }) => createWalletClient({ chain, account, transport: http('https://my-rpc.com') }),
 * })
 * ```
 */
interface CreateViemAdapterFromPrivateKeyParams extends BaseAdapterParams {
    /** Private key (hex string, with or without `0x` prefix). */
    privateKey: string;
    /**
     * Optional custom wallet client factory.
     * Receives the derived account so it can be attached to the client.
     * Defaults to HTTP transport with the derived account.
     */
    getWalletClient?: (params: {
        chain: Chain;
 ...
 * ```
 */
declare function createViemAdapterFromPrivateKey(params: CreateViemAdapterFromPrivateKeyParams): ViemAdapter;

/**
 * Factory for creating the new Viem adapter from an EIP-1193 provider.
 *
 * @packageDocumentation
 */

/**
 * Parameters for creating a Viem adapter from an EIP-1193 provider.
 *
 * @example
 * ```typescript
 ...
 * })
 *
 * // Developer-controlled (server-side JSON-RPC)
 * const adapter = createViemAdapterFromProvider({
 ...
 * ```
 */
interface CreateViemAdapterFromProviderParams extends BaseAdapterParams {
    /** EIP-1193 compatible provider (e.g., `window.ethereum`). */
 ...
 * ```
 */
declare function createViemAdapterFromProvider(params: CreateViemAdapterFromProviderParams): ViemAdapter;
