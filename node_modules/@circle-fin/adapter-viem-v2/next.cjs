...
const hexAddressSchema$2 = zod.z.string().regex(/^0x[0-9a-fA-F]{40}$/, 'Expected a 0x-prefixed 20-byte EVM address (40 hex chars)');
const hexBytesSchema$1 = zod.z.string().regex(/^0x([0-9a-fA-F]{2})*$/, 'Expected a 0x-prefixed even-length hex string');
...
const hexAddressSchema$1 = zod.z.string().regex(/^0x[0-9a-fA-F]{40}$/, 'Expected a 0x-prefixed 20-byte hex address');
// EIP-712 signatures produced by Circle's proxy service are always 65 bytes
 ...
 */ function toChecksumBody(lowercaseHexBody) {
    ...
    const hash = sha3.keccak_256(ascii);
    let out = '';
 ...
 * ```
 */ const hexString = zod.z.custom((val)=>// Byte-aligned only: `0x` (zero bytes) or an even number of hex digits.
 ...
 * ```
 */ const hexAddress = zod.z.custom((val)=>typeof val === 'string' && /^0x[a-fA-F0-9]{40}$/.test(val), {
 ...
 * })
 * ```
 */ const gasOverridesSchema = zod.z.object({
    // A 0 gas limit can never execute; gas price / max fee must be > 0.
    gas: zod.z.bigint().positive().optional(),
    maxFeePerGas: zod.z.bigint().positive().optional(),
    // A 0 priority-fee tip is valid (e.g. on L2s), so only forbid negatives.
    maxPriorityFeePerGas: zod.z.bigint().nonnegative().optional(),
    gasPrice: zod.z.bigint().positive().optional(),
    nonce: zod.z.number().int().nonnegative().optional()
 ...
 * })
 * ```
 */ const executeTransactionInputSchema = zod.z.object({
...
const hexAddressSchema = zod.z.string().regex(/^0x[0-9a-fA-F]{40}$/, 'Expected a 0x-prefixed 20-byte EVM address (40 hex chars)');
const usdtTransferInputSchema = zod.z.object({
    to: hexAddressSchema,
...
const prepareContractInputShape = {
    address: hexAddress,
    abi: zod.z.array(zod.z.unknown()).min(1, 'ABI must not be empty'),
    functionName: zod.z.string().min(1, 'Function name is required'),
    args: zod.z.array(zod.z.unknown()).optional(),
    value: zod.z.bigint().optional()
};
/** Zod schema for {@link PrepareContractInput}. */ zod.z.object({
    type: zod.z.literal('contract').optional(),
    ...prepareContractInputShape
});
/** Zod schema for {@link PrepareRawInput}. */ zod.z.object({
    type: zod.z.literal('raw'),
    raw: rawTransactionSchema
});
/** Zod schema for {@link PrepareNativeInput}. */ const prepareNativeInputSchema = zod.z.object({
    type: zod.z.literal('native'),
    address: hexAddress,
    value: zod.z.bigint()
});
...
const prepareContractInputSchemaStrict = zod.z.object({
    type: zod.z.literal('contract'),
    ...prepareContractInputShape
});
const prepareRawInputSchemaStrict = zod.z.object({
    type: zod.z.literal('raw'),
    raw: rawTransactionSchema
});
/**
 ...
 */ const prepareInputSchema$1 = zod.z.preprocess((input)=>{
...
// Schema
// ============================================================================
const waitForTransactionInputSchema = zod.z.object({
    txId: hexBytes32,
    finality: zod.z.union([
 ...
 */ function extractRevertReasonText(error) {
    const reverted = walkErrorCause(error, (cause)=>cause instanceof viem.ContractFunctionRevertedError);
    if (reverted instanceof viem.ContractFunctionRevertedError) {
        return extractRevertReason(reverted);
    }
    const executionReverted = walkErrorCause(error, (cause)=>cause instanceof viem.ExecutionRevertedError);
    if (executionReverted instanceof viem.ExecutionRevertedError) {
...
const handleSwitchChain = (error, chain)=>{
    if (error instanceof viem.SwitchChainError) {
        throw createUserRejectedError(chain, extractViemErrorDetails(error));
    }
};
// ============================================================================
...
const handleInvalidAddress = (error, chain)=>{
    if (error instanceof viem.InvalidAddressError) {
...
const handleInvalidChainId = (error, chain)=>{
    if (error instanceof viem.InvalidChainIdError) {
...
const handleContractReverted = (error, chain)=>{
    if (error instanceof viem.ContractFunctionRevertedError) {
...
const handleContractZeroData = (error, chain)=>{
    if (error instanceof viem.ContractFunctionZeroDataError) {
...
const handleContractExecution = (error, chain)=>{
    if (error instanceof viem.ContractFunctionExecutionError) {
        ...
        if (cause instanceof viem.ContractFunctionRevertedError) {
            throw createSimulationFailedError(chain, extractRevertReason(cause), extractViemErrorDetails(error));
        }
        if (cause instanceof viem.ContractFunctionZeroDataError) {
...
const handleCallExecution = (error, chain)=>{
    if (error instanceof viem.CallExecutionError) {
        throw createSimulationFailedError(chain, error.shortMessage, extractViemErrorDetails(error));
    }
};
const handleEstimateGasError = (error, chain)=>{
    if (error instanceof viem.EstimateGasExecutionError) {
        throw createSimulationFailedError(chain, error.shortMessage, extractViemErrorDetails(error));
    }
};
const handleAbiErrors = (error, chain)=>{
    if (error instanceof viem.AbiFunctionNotFoundError || error instanceof viem.AbiEncodingLengthMismatchError || error instanceof viem.AbiDecodingZeroDataError) {
        throw createSimulationFailedError(chain, error.shortMessage, extractViemErrorDetails(error));
    }
};
// ============================================================================
...
const handleInsufficientBalance = (error, chain)=>{
    ...
    const insufficientFunds = walkErrorCause(error, (cause)=>cause instanceof viem.InsufficientFundsError);
...
const handleGasTooLow = (error, chain)=>{
    if (error instanceof viem.IntrinsicGasTooLowError || error instanceof viem.MaxFeePerGasTooLowError || error instanceof viem.FeeCapTooLowError) {
        throw createOutOfGasError(chain, extractViemErrorDetails(error));
    }
};
const handleGasTooHigh = (error, chain)=>{
    if (error instanceof viem.IntrinsicGasTooHighError || error instanceof viem.FeeCapTooHighError) {
        throw createGasLimitExceededError(chain, extractViemErrorDetails(error));
    }
};
const handleExecutionReverted = (error, chain)=>{
    if (error instanceof viem.ExecutionRevertedError) {
...
const handleTipAboveFeeCap = (error, chain)=>{
    if (error instanceof viem.TipAboveFeeCapError) {
        throw createOutOfGasError(chain, extractViemErrorDetails(error));
    }
};
// ============================================================================
...
const handleTransactionExecution = (error, chain)=>{
    if (error instanceof viem.TransactionExecutionError) {
        const cause = error.cause;
        if (cause instanceof viem.ContractFunctionRevertedError) {
...
const handleNonceErrors = (error, chain)=>{
    if (error instanceof viem.NonceTooLowError || error instanceof viem.NonceTooHighError || error instanceof viem.NonceMaxValueError) {
...
const handleTransactionNotFound = (error, chain)=>{
    if (error instanceof viem.TransactionNotFoundError) {
...
const handleReceiptNotFound = (error, chain)=>{
    if (error instanceof viem.TransactionReceiptNotFoundError) {
        throw createRpcEndpointError(chain, {
            ...extractViemErrorDetails(error),
...
const handleReceiptTimeout = (error, chain)=>{
    if (error instanceof viem.WaitForTransactionReceiptTimeoutError) {
...
const handleTimeout = (error, chain)=>{
    if (error instanceof viem.TimeoutError) {
...
const handleWebSocketRequest = (error, chain)=>{
    if (error instanceof viem.WebSocketRequestError) {
        throw createRpcEndpointError(chain, {
            ...extractViemErrorDetails(error),
            reason: redactErrorMessage(error.shortMessage)
        });
...
const handleProviderDisconnected = (error, chain)=>{
    if (error instanceof viem.ProviderDisconnectedError || error instanceof viem.ChainDisconnectedError || error instanceof viem.UnauthorizedProviderError) {
        throw createNetworkConnectionError(chain, extractViemErrorDetails(error));
    }
};
/**
 ...
 */ const handleTransportCause = (error, chain)=>{
    ...
    const transportCause = walkErrorCause(error, (cause)=>cause instanceof viem.HttpRequestError || cause instanceof viem.WebSocketRequestError || cause instanceof viem.TimeoutError || cause instanceof viem.SocketClosedError);
...
const handleRpcProtocolErrors = (error, chain)=>{
    if (error instanceof viem.TransactionRejectedRpcError) {
        throw createTransactionRevertedError(chain, error.shortMessage, extractViemErrorDetails(error));
    }
    if (error instanceof viem.LimitExceededRpcError) {
        throw createRpcEndpointError(chain, {
            ...extractViemErrorDetails(error),
            reason: 'Rate limit exceeded'
        });
    }
    if (error instanceof viem.InvalidInputRpcError || error instanceof viem.InvalidParamsRpcError || error instanceof viem.InvalidRequestRpcError || error instanceof viem.MethodNotFoundRpcError || error instanceof viem.MethodNotSupportedRpcError || error instanceof viem.ResourceNotFoundRpcError || error instanceof viem.ResourceUnavailableRpcError) {
        throw createRpcEndpointError(chain, {
            ...extractViemErrorDetails(error),
 ...
 */ const readContractInputSchema = zod.z.object({
    address: hexAddress,
    abi: zod.z.custom((val)=>Array.isArray(val) && val.length > 0, 'ABI must be a non-empty array'),
    functionName: zod.z.string().min(1, 'Function name is required'),
    args: zod.z.array(zod.z.unknown()).optional()
});
// ============================================================================
 ...
 */ const calculateFeeInputSchema = zod.z.object({
    gasUnits: zod.z.bigint().positive('Gas units must be positive'),
    bufferBasisPoints: zod.z.bigint().nonnegative().optional()
});
        ...
        case 'signed':
            {
                ...
                return {
                    kind: 'signed',
                    txHash: viem.keccak256(result.signedTransaction),
 ...
 * }
 * ```
 */ const typedDataSchema = zod.z.object({
 ...
 * ```
 */ const evmPrivateKeySchema = zod.z.string({
 ...
 */ const domainSchema = zod.z.object({
    name: zod.z.string().optional(),
    version: zod.z.string().optional(),
 ...
 */ const typedDataFieldSchema = zod.z.object({
    ...
    type: zod.z.string().min(1, 'Field type is required')
});
/**
 ...
 */ const signTypedDataInputSchema = zod.z.object({
    domain: domainSchema,
    types: zod.z.record(zod.z.array(typedDataFieldSchema)),
    primaryType: zod.z.string().min(1, 'Primary type is required'),
    message: zod.z.record(zod.z.unknown())
});
// ============================================================================
// Factory
 ...
 * ```
 */ zod.z.object({
    getPublicClient: zod.z.function().args(zod.z.object({
        chain: zod.z.unknown()
    })).returns(zod.z.unknown()),
    getWalletClient: zod.z.function().returns(zod.z.union([
        zod.z.unknown(),
        zod.z.promise(zod.z.unknown())
    ])).refine((fn)=>{
 ...
 */ const createAdapterFromPrivateKeyParamsSchema = zod.z.object({
    ...
    getWalletClient: zod.z.function().args(zod.z.object({
        chain: zod.z.unknown(),
        account: zod.z.unknown()
    })).returns(zod.z.union([
        zod.z.unknown(),
        zod.z.promise(zod.z.unknown())
    ])).optional(),
 ...
 */ function createEnhancedTransport(chainDefinition, customUrl) {
    if (chainDefinition && chainDefinition.rpcEndpoints.length > 0) {
        ...
        if (endpoints.length === 1) {
            ...
            return viem.http(rpcUrl, NO_TRANSPORT_RETRY);
        }
        // Multiple endpoints: create fallback transport for automatic failover
        const transports = endpoints.map((endpoint)=>viem.http(endpoint, NO_TRANSPORT_RETRY));
        return viem.fallback(transports);
    }
    // Fall back to Viem's default RPC endpoint
    return viem.http(undefined, NO_TRANSPORT_RETRY);
}
 ...
 * })
 * ```
 */ const createNarrowedWalletClient = viem.createWalletClient;
 ...
 */ function createViemAdapterFromPrivateKey(params) {
    ...
    const accountNonceManager = nonce.createNonceManager({
        source: nonce.jsonRpc()
    });
    let account;
    try {
    ...
    return createAdapter({
        ...
        getWalletClient: ({ chain })=>{
            ...
            return createNarrowedWalletClient({
                ...
                transport: wrapTransport(viem.http(undefined, {
                    retryCount: 0,
                    retryDelay: 0
                }), chain.name),
                account,
    ...
    return createAdapter({
        ...
        getWalletClient: async ({ chain })=>{
            ...
            if (isDeveloperControlled) {
                return createNarrowedWalletClient({
                    chain,
                    transport: wrapTransport(viem.custom(params.provider), chain.name)
                });
            }
            ...
            pendingAccount ??= (async ()=>{
                const tempClient = viem.createWalletClient({
...
exports.createAuthorizationRejectedError = createAuthorizationRejectedError;
exports.createCalculateFee = createCalculateFee;
exports.createEVMActions = createEVMActions;
exports.createEstimate = createEstimate;
exports.createExecute = createExecute;
exports.createPrepare = createPrepare;
exports.createRead = createRead;
exports.createRuntime = createRuntime;
exports.createSignTypedData = createSignTypedData;
exports.createSigningRejectedError = createSigningRejectedError;
exports.createSimulate = createSimulate;
exports.createTokenRegistry = createTokenRegistry;
exports.createViemAdapter = createAdapter;
exports.createViemAdapterContext = createAdapterContext;
exports.createViemAdapterFromContext = createAdapterFromContext;
...
exports.externalSigning = externalSigning;
exports.hexAddress = hexAddress;
exports.hexBytes32 = hexBytes32;
exports.hexString = hexString;
exports.isSigningRejected = isSigningRejected;
exports.resolveViemChain = resolveViemChain;
