...
import { zksyncSepoliaTestnet, zksync, xdcTestnet, xdc, worldchainSepolia, worldchain, unichainSepolia, unichain, sonic, seiTestnet, sei, plasmaTestnet, plasma, optimismSepolia, optimism, lineaSepolia, linea, sepolia, mainnet, cronosTestnet, cronos, celoAlfajores, celo, baseSepolia, base, avalancheFuji, avalanche, arbitrumSepolia, arbitrum } from 'viem/chains';
 ...
 * ```
 */ async function resolveOperationContext$1(adapter, ctx) {
 ...
 */ class Adapter {
   ...
   */ async prepareAction(action, params, ctx) {
        ...
        const resolvedContext = await resolveOperationContext$1(this, ctx);
 ...
 * ```
 */ const hexStringSchema$2 = z.string().min(1, 'Hex string is required').refine((value)=>value.trim().length > 0, 'Hex string cannot be empty').refine((value)=>value.startsWith('0x'), 'Hex string must start with 0x prefix').refine((value)=>{
 ...
 * ```
 */ const evmAddressSchema = hexStringSchema$2.refine((value)=>value.length === 42, 'EVM address must be exactly 42 characters long (0x + 40 hex characters)').transform((value)=>value);
/**
 ...
 * console.log(result.success) // true
 * ```
 */ hexStringSchema$2.refine((value)=>value.length === 66, 'Transaction hash must be exactly 66 characters long (0x + 64 hex characters)');
/**
 ...
 * console.log(result.success) // true
 * ```
 */ const evmSignatureSchema = hexStringSchema$2.refine((value)=>value.length === 132, 'EVM signature must be exactly 132 characters long (0x + 130 hex characters)').transform((value)=>value);
/**
 ...
 */ const tokenMessengerWithFeesAbi = [
    {
        type: 'function',
    ...
    // Custom error declared by the wrapper.
    {
        type: 'error',
    ...
    // Reverts documented on the prepaid FORWARD path (surfaced from the wrapper /
    // FeeManager). Declared here so revert data can be decoded and mapped to typed
    // SDK errors; see `mapPrepaidForwardError`.
    {
        type: 'error',
...
/**
 * Cross Chain Token Service (CCTS) ABI for CCTPx.
 *
 * This ABI is used to interact with the `CrossChainTokenService` contract on EVM networks.
 * CCTS is the service-level entry point for the CCTPx protocol; callers use
 * `crossChainTransfer` to initiate a cross-chain transfer of a registered token
 * (Circle-issued or otherwise) and `resolveTokenManager` to look up the per-token
 * `TokenManager` proxy that must be approved before transfer.
 *
 * Only the minimal surface needed by the SDK is exposed. The CCTS contract has other
 * admin and registry methods that the SDK does not call.
 */ const crossChainTokenServiceAbi = [
    {
        type: 'function',
 ...
 */ function resolveSignatureArgs(commonArgs, fields, functionName) {
    ...
    ].filter((c)=>c.value === undefined).map((c)=>c.name);
 ...
 */ const gatewayTypedDataSchema = z.object({
 ...
 * }
 * ```
 */ const evmTransactionHashSchema = z.string({
...
/**
 * Number of characters in a 32-byte hex string: `0x` followed by 64 hex digits.
 */ const BYTES32_HEX_LENGTH = 66;
/**
 * Assert that a value is a `0x`-prefixed hex string.
 *
 * Shared across EVM prepare helpers so hex-string validation stays consistent.
 * Throws a `KitError` (rather than returning) so callers can validate inputs
 * before encoding a contract call.
 *
 * @param field - Name of the parameter being validated, used in the error.
 * @param value - The candidate hex string.
 * @throws KitError if `value` is not a valid `0x`-prefixed hex string.
 *
 * @example
 * ```typescript
 * assertHex('hookData', '0xdeadbeef')
 * ```
 */ function assertHex(field, value) {
    const result = hexStringSchema$2.safeParse(value);
    if (!result.success) {
        throw createValidationFailedError(field, value, 'Must be a valid hex string starting with 0x');
    }
}
/**
 * Assert that a value is a Solidity `bytes32` value: a `0x`-prefixed hex string
 * of exactly 32 bytes (64 hex characters).
 *
 * Generic hex validation alone is insufficient for `bytes32` arguments because
 * ethers/viem silently right-pad short values to 32 bytes — so e.g. `'0x1234'`
 * would encode as a different identity without any pre-flight error. This guard
 * rejects wrong-length values up front.
 *
 * @param field - Name of the parameter being validated, used in the error.
 * @param value - The candidate `bytes32` hex string.
 * @throws KitError if `value` is not a valid `0x`-prefixed hex string.
 * @throws KitError if `value` is not exactly 32 bytes (66 characters including `0x`).
 *
 * @example
 * ```typescript
 * assertBytes32(
 ...
 */ function assertBytes32(field, value) {
    assertHex(field, value);
    if (value.length !== BYTES32_HEX_LENGTH) {
        throw createValidationFailedError(field, value, 'Must be a 32-byte hex string (0x followed by exactly 64 hex characters)');
    }
}
/**
 * Assert that a value is a Solidity `bytes` value: a `0x`-prefixed hex string
 * covering a whole number of bytes.
 *
 * Unlike {@link assertBytes32} this places no constraint on how many bytes the
 * value carries, because a `bytes` argument is a dynamic byte array whose width
 * is defined by whatever consumes it. It constrains the length only to a whole,
 * non-zero number of bytes: an odd hex-digit count cannot encode to bytes at
 * all, and the empty literal `'0x'` is rejected by the delegated
 * {@link assertHex}, which requires at least one hex digit. Callers that must
 * accept an empty `bytes` should test for `'0x'` before calling.
 *
 * @param field - Name of the parameter being validated, used in the error.
 * @param value - The candidate `bytes` hex string.
 * @throws KitError if `value` is not a valid `0x`-prefixed hex string, which
 *   includes the empty literal `'0x'`.
 * @throws KitError if `value` has an odd number of hex digits.
 *
 * @example
 * ```typescript
 * assertBytes('destinationAddress', '0x1234567890123456789012345678901234567890')
 * ```
 */ function assertBytes(field, value) {
    assertHex(field, value);
    if ((value.length - '0x'.length) % 2 !== 0) {
        throw createValidationFailedError(field, value, 'Must cover whole bytes (0x followed by an even number of hex characters)');
    }
}
 ...
 */ const isNativeFeeToken = (feeToken)=>feeToken.toLowerCase() === ZERO_ADDRESS.toLowerCase();
/**
 ...
 * await prepared.execute()
 * ```
 */ const prepareDepositForBurnWithFees = async (params, adapter, context)=>{
    if (params.fromChain.type !== 'evm') {
        throw createInvalidChainError(params.fromChain.name, `Expected EVM chain but received chain type: ${params.fromChain.type}`);
    }
    // The signed quote binds destinationCaller; unlike the plain burn it is not
    // optional. The zero address is valid for unrestricted forwarding.
    if (params.destinationCaller === undefined || params.destinationCaller === '') {
        throw createValidationFailedError('destinationCaller', params.destinationCaller, 'destinationCaller is required for depositForBurnWithFees');
    }
    // Validate the signed-quote claim. Treat it as unknown so runtime validation
    // holds for plain-JS callers even though the type marks it as present.
    const rawClaim = params.claim;
    const signedQuote = rawClaim?.signedQuote;
    const refundAddress = rawClaim?.refundAddress;
    if (!hexStringSchema$2.safeParse(signedQuote).success) {
        throw createValidationFailedError('claim.signedQuote', signedQuote, 'Must be a valid hex string starting with 0x');
    }
    if (!evmAddressSchema.safeParse(refundAddress).success) {
        throw createValidationFailedError('claim.refundAddress', refundAddress, 'Must be a valid EVM address');
    }
    // Validate the fee token address.
    if (!evmAddressSchema.safeParse(params.feeToken).success) {
        throw createValidationFailedError('feeToken', params.feeToken, 'Must be a valid EVM address (use the zero address for a native fee)');
    }
    // Validate the fee amount.
    if (typeof params.feeTotalAmount !== 'bigint' || params.feeTotalAmount < 0n) {
        throw createValidationFailedError('feeTotalAmount', params.feeTotalAmount, 'Must be a non-negative bigint');
    }
    const hasHookData = params.hookData !== undefined && params.hookData !== '';
    if (hasHookData && !hexStringSchema$2.safeParse(params.hookData).success) {
        throw createValidationFailedError('hookData', params.hookData, 'Must be a valid hex string starting with 0x');
    }
    const claim = {
        signedQuote: signedQuote,
        refundAddress: refundAddress
    };
    // Note: TokenMessengerWithFees does NOT take maxFee / minFinalityThreshold —
    // fee and finality behavior are derived from the signed quote.
    const baseArgs = [
        params.amount,
        params.toChain.cctp.domain,
        convertAddress(params.mintRecipient, 'bytes32'),
    ...
    // Native fee → attach exactly feeTotalAmount as msg.value; ERC-20 fee → no value.
 ...
 * await prepared.execute()
 * ```
 */ const depositForBurnWithFees = async (params, adapter, context)=>{
...
/**
 * Prepare an EVM `crossChainTransfer` transaction for CCTPx.
 *
 * Encode a call to `CrossChainTokenService.crossChainTransfer(...)` on the source chain
 * using the IRIS-signed fee quote and the resolved CCTS proxy address. The native fee
 * amount is passed as the transaction `value`.
 *
 * Mirrors the structure of the CCTP v2 `prepareDepositForBurn` helper: input validation
 * happens here (chain type, hex-string format on bytes-typed params); the actual contract
 * call is delegated to `adapter.prepare(...)`, which is implemented per-framework by
 * `viem.v2` / `ethers.v6`.
 *
 * @param params - The CCTPx cross-chain-transfer payload.
 * @param adapter - The EVM adapter responsible for chain context and transaction preparation.
 * @param context - The resolved operation context providing chain and address information.
 * @returns A promise that resolves to a prepared chain request for the `crossChainTransfer` call.
 * @throws KitError if `fromChain` is not EVM-compatible.
 * @throws KitError if `destinationAddress`, `claim.signedQuote`, or `hookData`
 *   is not a valid 0x-prefixed hex string ("Must be a valid hex string
 *   starting with 0x"). `hookData` is exempt when it is the empty literal
 *   `'0x'`; for the other two, `'0x'` is rejected because a Solidity `bytes`
 *   recipient or quote must carry at least one byte.
 * @throws KitError if `destinationAddress`, `claim.signedQuote`, or `hookData`
 *   has an odd number of hex digits ("Must cover whole bytes"). All three are
 *   Solidity `bytes`, whose width is unconstrained but must be a whole number
 *   of bytes; viem would otherwise right-pad an odd nibble count silently.
 * @throws KitError if `tokenId` or `destinationCaller` (when provided) is not a
 *   32-byte hex string — `0x` followed by exactly 64 hex characters.
 * @throws KitError if `serviceAddress` or `claim.refundAddress` (when provided)
 *   is not a valid EVM address — `0x` followed by exactly 40 hex characters
 *   ("Invalid EVM address").
 *
 * @example
 * ```typescript
 ...
 * await prepared.execute()
 * ```
 */ const prepareCrossChainTransfer = async (params, adapter, context)=>{
    if (params.fromChain.type !== 'evm') {
        throw createInvalidChainError(params.fromChain.name, `Expected EVM chain but received chain type: ${params.fromChain.type}`);
    }
    // destinationCaller is optional; the zero hash allows permissionless relay
    // (the default for auto-relayed CCTPx transfers).
    const destinationCaller = params.destinationCaller ?? ZERO_HASH;
    // tokenId and destinationCaller are Solidity bytes32 — enforce the exact
    // 32-byte length so short values are not silently right-padded by ethers/viem.
    assertBytes32('tokenId', params.tokenId);
    assertBytes32('destinationCaller', destinationCaller);
    // `destinationAddress` is a Solidity `bytes`, not an `address` or `bytes32`,
    // so its width is deliberately unconstrained here: this helper is given
    // `destinationDomain` (a number) and never the destination chain's
    // definition, so it cannot know which recipient encoding that domain
    // expects. Whole bytes is the one requirement that holds for every
    // destination.
    //
    // The width rule lives on the destination chain instead. An EVM destination
    // requires exactly 20 bytes — `EncodingUtils.bytesToAddress` reverts
    // `InvalidEVMAddressLength` for any other non-zero length — but that runs at
    // delivery, after the source leg has moved the tokens and paid the fee. Do
    // not narrow this to `assertEvmAddress`: it would reject a valid non-EVM
    // recipient this helper has no way to recognise.
    assertBytes('destinationAddress', params.destinationAddress);
    // `signedQuote` and `hookData` are Solidity `bytes` too, so they need the
    // same whole-bytes rule. viem does not enforce it: it right-pads an odd
    // nibble count silently (`0x123` encodes as `0x1230`), which would submit a
    // different quote than the one IRIS signed. ethers rejects it instead, so
    // without this guard the two adapters disagree.
    assertBytes('claim.signedQuote', params.claim.signedQuote);
    // hookData is a Solidity `bytes` arg, which accepts the empty literal '0x' for
    // basic transfers; only validate when non-empty (`assertBytes` requires at
    // least one byte).
    if (params.hookData !== '0x') {
        assertBytes('hookData', params.hookData);
    }
    assertEvmAddress(params.serviceAddress);
    // refundAddress is optional; the contract accepts the zero address (refunds
    // disabled). Validate as an EVM address only when the caller supplied one.
    const refundAddress = params.claim.refundAddress ?? ZERO_ADDRESS;
    if (params.claim.refundAddress !== undefined) {
        assertEvmAddress(params.claim.refundAddress);
    }
    return adapter.prepare({
        type: 'evm',
        abi: crossChainTokenServiceAbi,
        ...
        args: [
            ...
            destinationCaller,
            params.minFinalityThreshold,
            // IFeeManager.QuoteClaim tuple: [signedQuote, refundAddress].
            // Array form (not object form) is what viem/ethers expect for
            // positional tuple encoding.
            [
                params.claim.signedQuote,
                refundAddress
            ],
            params.autoExecuteHookData,
            params.hookData
        ],
        value: params.nativeFeeAmount
    }, context);
};

/**
 * Prepare an EVM-compatible `crossChainTransfer` transaction for CCTPx.
 *
 * Initiates a service-level CCTPx transfer through the source chain's
 * `CrossChainTokenService` (CCTS) contract for a registered token (Circle-issued or otherwise). The native fee
 * amount (from the IRIS-signed quote) is passed as the transaction `value`; Circle's
 * Orbit relayer auto-executes the destination mint via the FORWARD fee component
 * embedded in the signed quote.
 *
 * @param params - The action payload containing:
 *   - `tokenId`: CCTPx tokenId (`bytes32` hex).
 *   - `amount`: Amount to transfer in the token's smallest units.
 *   - `destinationDomain`: CCTP domain of the destination chain.
 *   - `destinationAddress`: Recipient address on the destination, bytes-encoded.
 *   - `destinationCaller`: `bytes32` (zero hash for permissionless relay).
 *   - `minFinalityThreshold`: 1000 (FAST) or 2000 (SLOW).
 *   - `claim`: `IFeeManager.QuoteClaim` tuple — `{ signedQuote, refundAddress? }`.
 *     `refundAddress` defaults to the zero address (refunds disabled).
 *   - `autoExecuteHookData`: Hook-execution flag (false for basic transfers).
 *   - `hookData`: Hook payload bytes (`'0x'` for basic transfers).
 *   - `serviceAddress`: CCTS proxy address on the source chain.
 *   - `nativeFeeAmount`: Native fee amount used as `msg.value`.
 *   - `fromChain`: Source chain definition.
 * @param adapter - The EVM adapter responsible for chain context and transaction preparation.
 * @param context - The resolved operation context providing chain and address information.
 * @returns A promise that resolves to a prepared chain request for the `crossChainTransfer` call.
 * @throws KitError if `fromChain` is not EVM-compatible or any hex-typed input is malformed.
 *
 * @example
 * ```typescript
 * const prepared = await crossChainTransfer(
 *   {
 *     tokenId: '0xabc...',
 *     amount: 1_000_000n,
 *     destinationDomain: 1,
 ...
 * await prepared.execute()
 * ```
 */ const crossChainTransfer = async (params, adapter, context)=>{
    return prepareCrossChainTransfer(params, adapter, context);
};
...
const earnExecuteParamsSchema$1 = z.object({
...
function validateEarnExecuteParams(params) {
    const result = earnExecuteParamsSchema$1.safeParse(params);
    if (!result.success) {
 ...
 */ const isEVMSwapParams = (params)=>{
    return 'executeParams' in params && params.executeParams !== undefined && 'tokenInputs' in params && params.tokenInputs !== undefined && 'inputAmount' in params && params.inputAmount !== undefined && 'tokenInAddress' in params && params.tokenInAddress !== undefined;
};
/**
 ...
 */ const executeSwap = async (params, adapter, context)=>{
    ...
    // Stargate V2) charge a LayerZero messaging fee in native ETH regardless of
    // the input token, so this must be forwarded even for ERC20 swaps.
    const instructionValueSum = params.executeParams.instructions.reduce((acc, instruction)=>acc + instruction.value, 0n);
    // For native-input swaps, the service usually encodes the native swap value
    // in the instruction list itself. Use the larger value so we do not
    // double-count same-chain native swaps, while still preserving the older
    // fallback where instruction values were absent.
    let totalValue = instructionValueSum;
    if (isNativeSwap && params.inputAmount > totalValue) {
        totalValue = params.inputAmount;
    }
    if (totalValue > 0n) {
        const hasNativeResidualRecipient = params.executeParams.tokens.some(({ token, beneficiary })=>token.toLowerCase() === NATIVE_TOKEN_ADDRESS.toLowerCase() && beneficiary.toLowerCase() === context.address.toLowerCase());
        if (!hasNativeResidualRecipient) {
            throw createValidationFailedError('executeParams.tokens', params.executeParams.tokens, 'Native-funded swaps must include a NATIVE_TOKEN residual recipient for the sender');
        }
    }
    ...
    return adapter.prepare({
        ...
        // Only attach value when something actually needs native funding. The
        // EIP-712 signature covers instructions/tokens/execId/deadline/metadata
        // — not msg.value — so the Adapter can safely pull a larger value from
        // the caller at runtime. Any over-fund (wei dust or bridge refund) is
        // swept back via the NATIVE_TOKEN residual recipient appended by the
        // service.
        ...totalValue > 0n && {
            value: totalValue
        }
 ...
 * await prepared.execute()
 * ```
 */ const approve$1 = async (params, adapter, context)=>{
    const chain = context.chain;
    if (chain.type !== 'evm') {
        throw createInvalidChainError(chain.name, `Expected EVM chain definition, but received chain type: ${chain.type}`);
    }
...
/**
 * Prepares an EVM-compatible `approve` transaction for USDC.
 *
 * Auto-resolves the USDC contract address from the chain definition and
 * delegates to the generic ERC-20 `approve` action. Use this instead of
 * `usdc.increaseAllowance` for chains running FiatToken v2.2 or later,
 * which removed `increaseAllowance`.
 *
 * @param params - The action payload containing:
 *   - `delegate`: The address to grant the USDC allowance to (spender).
 *   - `amount`: The allowance amount in USDC minor units.
 * @param adapter - The EVM adapter responsible for chain context and
 *   transaction preparation.
 * @param context - The resolved operation context providing chain and
 *   address information.
 * @returns A promise that resolves to a prepared chain request for the
 *   USDC `approve` call.
 * @throws Error if the USDC address is missing on the chain.
 *
 * @example
 * ```typescript
 * const prepared = await approve(
 *   { delegate: '0xabc...', amount: 1_000_000n },
 *   adapter,
 *   context,
 * )
 * await prepared.execute()
 ...
 */ const approve = async (params, adapter, context)=>{
    ...
    if (tokenAddress == null) {
        throw createValidationFailedError('chain', chain.name, `USDC address not configured for chain ${chain.name}`);
    }
    return approve$1({
 ...
 */ const getHandlers = (adapter)=>{
     ...
     */ 'token.approve': async (params, context)=>{
            return approve$1(params, adapter, context);
        },
        /**
     ...
     */ 'usdc.balanceOf': async (params, context)=>{
        ...
        /**
     * Handler for USDC approve operations on EVM chains.
     *
     * Sets the allowance of the USDC contract to the delegate address.
     * Prefer this over `usdc.increaseAllowance` on chains running FiatToken
     * v2.2 or later, which removed the `increaseAllowance` function.
     */ 'usdc.approve': async (params, context)=>{
     ...
     */ 'cctp.v2.customBurn': async (params, context)=>{
        ...
        /**
     * Handler for CCTP-X crossChainTransfer operations on EVM chains.
     *
     * Submits a service-level transfer through the source chain's
     * `CrossChainTokenService` (CCTS) contract for a registered 3P token. The
     * IRIS-signed fee quote is passed verbatim and the native fee amount is sent
     * as `msg.value`; Circle's Orbit relayer auto-executes the destination mint.
     */ 'cctpx.crossChainTransfer': async (params, context)=>{
            return crossChainTransfer(params, adapter, context);
        },
        /**
 ...
 * )
 * ```
 */ async function resolveOperationContext(meta, options) {
    // Validate meta input
 ...
 */ async function resolveContexts(operation, adapterCtx, options) {
    ...
    const operationCtx = await resolveOperationContext(operation, {
...
const gatewayBurnInputSchema = z.object({
    calldataBytes: hexStringSchema$1,
    signature: hexStringSchema$1
});
const gatewayMintInputSchema = z.object({
    attestation: hexStringSchema$1,
    signature: hexStringSchema$1
});
...
/** Complete a fund withdrawal after delay. */ defineAction({
...
/** Read the pending withdrawal balance. */ defineAction({
...
const hexBytesSchema = z.string().regex(/^0x([0-9a-fA-F]{2})*$/, 'Expected a 0x-prefixed even-length hex string');
...
const hexStringSchema = z.string().regex(/^0x[0-9a-fA-F]*$/, 'Expected a 0x-prefixed hex string');
 ...
 */ async function resolveEVMTokenDecimalsFromContract(readContract, tokenAddress, chain, options) {
    ...
    if (!Number.isInteger(decimals) || decimals < 0 || decimals > 255) {
        throw (createDefaultInvalidDecimalsError)(rawDecimals);
    }
 ...
 */ const getViemChainByEnum = (chain)=>{
    ...
    switch(definition.chain){
        ...
        case Blockchain.Cronos:
            return cronos;
        case Blockchain.Cronos_Testnet:
        ...
        case Blockchain.Plasma:
            return plasma;
        case Blockchain.Plasma_Testnet:
     ...
     */ case Blockchain.Arc:
        ...
        case Blockchain.X_Layer:
        case Blockchain.X_Layer_Testnet:
            return {
 ...
 */ class ViemAdapter extends EvmAdapter {
   ...
   */ async switchToChain(chain) {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            throw new Error(`Failed to switch to chain ${chain.name} (ID: ${String(chain.chainId)}): ${errorMessage}. ` + 'Ensure the chain is supported by your wallet or provider.');
        }
    }
    /**
   ...
   */ prepareStateChangingFunction(publicClient, contractCall, targetChain, resolvedAddress) {
       ...
       */ estimate: async (overrides, fallback)=>{
                ...
                try {
                    ...
                    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
                    if (fallback && errorMessage.toLowerCase().includes('execution reverted')) {
                        return fallback;
                    }
       ...
       */ execute: async (overrides)=>{
                ...
                for(let attempt = 1; attempt <= MAX_SIMULATION_ATTEMPTS; attempt++){
                    try {
                        ...
                        const isAllowanceError = lowerCaseError.includes('exceeds allowance') || lowerCaseError.includes('insufficient allowance');
   ...
   */ async getPublicClient(chainDef) {
        ...
        const publicClient = this.options.getPublicClient({
            chain: viemChain
        });
   ...
   */ async readContract(params, chain) {
    ...
    /**
   * Read a contract view/pure function and return its decoded result.
   *
   * @remarks
   * `@core/adapter`'s `executeAdapterContractRead` calls this generic read
   * seam when present, instead of falling back to `prepare().execute()`.
   * That legacy path stringifies every result to satisfy
   * `execute(): Promise<string>` (the transaction-hash contract used for
   * writes), which collapses a `boolean` like Morpho's `isAuthorized()`
   * into the literal string `"false"` and breaks callers that check
   * `typeof value === 'boolean'`. This method returns the raw, decoded
   * value instead.
   *
   ...
   * @param meta - The chain the read targets.
   * @returns The raw, decoded contract read result.
   * @throws {@link KitError} With `INPUT_VALIDATION_FAILED` when `input` is
   *   not a well-formed EVM contract-read request.
   ...
   * )
   * ```
   */ async read(input, meta) {
        assertEvmPreparedChainRequestParams(input);
        const targetChain = resolveChainIdentifier(meta.chain);
        if (targetChain.type !== 'evm') {
            throw new Error(`Invalid chain type '${String(targetChain.type)}' for ViemAdapter. Expected 'evm' chain type.`);
        }
        const abi = this.parseAndValidateAbi(input.abi, input.functionName);
        const publicClient = await this.getPublicClient(targetChain);
        const readContractFn = publicClient.readContract;
   ...
   *
   * Call the `decimals()` function on the ERC-20 token contract and normalize
   * a `number` or `bigint` response to a validated number.
   *
   * @param tokenAddress - The ERC-20 token contract address
   ...
   * returns a value outside the uint8 range.
   *
   * @remarks
   * - Accepts a `number` or `bigint` response and normalizes it to `number`
   ...
   */ async getTokenDecimals(tokenAddress, chain) {
        return resolveEVMTokenDecimalsFromContract(async (params)=>{
            try {
                return await this.readContract(params, chain);
            } catch (error) {
                if (error instanceof KitError) throw error;
                const operation = 'getTokenDecimals';
                throw parseBlockchainError({
   ...
   */ async supportsAtomicBatch(chain) {
        ...
        try {
            ...
            if (typeof getCapabilities !== 'function') {
                this.cachedCapabilities.set(chain.chainId, {
                    atomic: false
                });
                return false;
            }
   ...
   */ async batchExecute(calls, chain, options) {
        ...
        const { id: batchId } = await sendCalls({
            ...
            chain: viemChain,
            calls: mappedCalls,
            // EIP-5792 itself defaults to non-atomic execution. Default this adapter
            // to atomic so capability-gated approve + execute flows cannot partially
            // land; callers may opt out only when partial execution is acceptable.
            forceAtomic: options?.atomicRequired !== false
        });
   ...
   */ async pollBatchStatus(getCallsStatus, batchId, callCount, timeout, pollingInterval) {
        ...
        throw new KitError({
            ...
            recoverability: 'RETRYABLE',
            message: `Batched call timeout: wallet_getCallsStatus did not return ` + `a terminal status within ${String(timeout)}ms (batchId: ${batchId}).`
        });
    }
    /**
