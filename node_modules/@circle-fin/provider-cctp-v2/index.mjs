 ...
 */ function buildForwardingHookDataWithPayload(version, payload) {
    if (!Number.isInteger(version) || version < 0 || version > MAX_UINT32) {
        throw createValidationFailedError('version', version, 'Expected an integer in the uint32 range [0, 4294967295]');
    }
    if (!isHexString(payload)) {
        throw createValidationFailedError('payload', payload, 'Expected a 0x-prefixed hex string');
    }
    ...
    if (payload.length % 2 !== 0) {
        throw createValidationFailedError('payload', payload, 'Expected an even-length (whole-byte) hex string');
    }
 ...
 */ function buildSolanaAtaForwardingHookData(ataOwner) {
    if (!(ataOwner instanceof Uint8Array) || ataOwner.length !== SOLANA_PUBKEY_LENGTH) {
        throw createValidationFailedError('ataOwner', ataOwner, `Expected a ${String(SOLANA_PUBKEY_LENGTH)}-byte Solana owner public key`);
    }
    if (ataOwner.every((byte)=>byte === 0)) {
        throw createValidationFailedError('ataOwner', ataOwner, 'Expected a non-zero Solana owner public key; the all-zero key is reserved as "absent owner"');
    }
 ...
 */ function padAddressToBytes32(address) {
    if (!isAddress(address)) {
        throw createValidationFailedError('address', address, 'Expected a valid 20-byte EVM address');
    }
 ...
 */ function buildDepositForGenericExecutorPayload(options) {
    ...
    if (options === null || typeof options !== 'object') {
        throw createValidationFailedError('options', options, 'Expected an options object');
    }
    ...
    if (config === undefined) {
        throw createValidationFailedError('dappId', dappId, `Unknown dApp. Known dApps: ${Object.keys(registry).join(', ')}`);
    }
    if (!Array.isArray(config.deployments)) {
        throw createValidationFailedError('deployments', config.deployments, `Expected an array of deployments for dApp '${dappId}'`);
    }
    ...
    if (deployment === undefined) {
        throw createValidationFailedError('domainId', domainId, `No '${dappId}' deployment for domain ${String(domainId)}`);
    }
    if (!Array.isArray(dappParams)) {
        throw createValidationFailedError('params', dappParams, 'Expected an array of ABI-ordered parameters');
    }
    if (!isHexString(options.recoveryAddress, 32)) {
        throw createValidationFailedError('recoveryAddress', options.recoveryAddress, 'Expected a 0x-prefixed 32-byte (bytes32) hex string');
    }
    ...
    if (!isAddress(handler)) {
        throw createValidationFailedError('handler', handler, 'Expected a valid EVM address');
    }
    ...
    try {
        ...
        throw createValidationFailedError('function', config.function, 'Expected a valid Solidity function signature');
    }
    ...
    if (dappParams.length !== fragment.inputs.length) {
        throw createValidationFailedError('params', dappParams, `'${config.function}' expects ${String(fragment.inputs.length)} params, got ${String(dappParams.length)}`);
    }
    let depositCalldata;
    try {
        depositCalldata = dappInterface.encodeFunctionData(fragment, dappParams);
    } catch  {
        throw createValidationFailedError('params', dappParams, 'ABI encoding failed — check that each param matches the expected Solidity type');
    }
    if (!Array.isArray(config.dynamicAmountIndices)) {
        throw createValidationFailedError('dynamicAmountIndices', config.dynamicAmountIndices, `Expected an array of amount indices for dApp '${dappId}'`);
    }
    // Array.isArray narrows to `any[]`; re-assert the concrete type.
    ...
    const amountIndices = dynamicAmountIndices.map((paramIndex)=>{
        if (!Number.isInteger(paramIndex) || paramIndex < 0 || paramIndex >= fragment.inputs.length) {
            throw createValidationFailedError('dynamicAmountIndices', paramIndex, `Index out of range for '${config.function}' (${String(fragment.inputs.length)} params)`);
        }
 ...
 */ function resolveDomainId(options) {
    ...
    if (chain !== undefined) {
        ...
        if (chainDomain !== undefined) {
            if (options.domainId !== undefined && options.domainId !== chainDomain) {
                throw createValidationFailedError('domainId', options.domainId, `does not match destinationChain '${chain.name}' CCTP domain ` + String(chainDomain));
            }
            return chainDomain;
        }
    }
    ...
    throw createValidationFailedError('domainId', options.domainId, "Provide 'domainId', or a 'destinationChain' with a CCTP domain");
}
/**
 ...
 */ function resolveDepositContract(deployment, options, dappId) {
    ...
    if (dappId !== GATEWAY_DEPOSIT_DAPP_ID) {
        throw createValidationFailedError('depositContract', dappId, `dApp '${dappId}' must declare a 'depositContract'; only the built-in ` + `'${GATEWAY_DEPOSIT_DAPP_ID}' resolves its address from the ` + "destination chain's GatewayWallet");
    }
    const chain = options.destinationChain;
    if (chain === undefined || !isGatewayV1Supported(chain)) {
        throw createValidationFailedError('depositContract', dappId, `'${GATEWAY_DEPOSIT_DAPP_ID}' needs a 'destinationChain' with Gateway v1 ` + 'support to resolve its GatewayWallet, or an explicit deployment address');
    }
    return chain.gateway.contracts.v1.wallet;
}
/**
 ...
 */ function resolveDepositForHandler(options, domainId) {
    ...
    const chain = options.destinationChain;
    if (chain === undefined) {
        throw createValidationFailedError('handler', domainId, "No 'handler' supplied; pass 'handler' explicitly, or a " + "'destinationChain' whose Gateway config registers a DepositForHandler");
    }
    const registered = isGatewayV1Supported(chain) ? chain.gateway.contracts.v1.depositForHandler : undefined;
    if (registered === undefined) {
        throw createValidationFailedError('handler', domainId, `No DepositForHandler registered for domain ${String(domainId)} on ` + `chain '${chain.name}'; pass 'handler' explicitly`);
    }
    return registered;
}
/**
 ...
 */ function assertAddress(address, field) {
    if (!isAddress(address)) {
        throw createValidationFailedError(field, address, 'Expected a valid EVM address');
    }
 ...
 */ function assertBurnWithFeesParams(params) {
    ...
    if (!hasExecutor && !hasDirectDestination) {
        throw createValidationFailedError('destination', undefined, 'Provide executor, or both mintRecipient and destinationCaller');
    }
    if (hasExecutor && hasDirectDestination) {
        throw createValidationFailedError('destination', undefined, 'Provide executor or direct destination fields, not both');
    }
    // amount
...
export { CCTPV2BridgingProvider, PREPAID_FORWARD_REVERT_NAMES, assertForwardHookData, getBlocksUntilExpiry, getForwarderHookData, getMintRecipientAccount, hasForwardHook, isAttestationExpired, isMintFailureRelatedToAttestation, mapPrepaidForwardError, resolveFeePayment, validateAndConvertFee };
