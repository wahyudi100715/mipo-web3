...
- Add `estimateDepositBurn` and `prepareDepositForBurn` static methods to support fast cross-chain deposit flows.
...
- `estimate()` now reports the gas each bridge step **reserves** — the amount a
  wallet must hold to send it — instead of a flat average that under-reported it.

  **What this means for you:** the `gas` and `fee` values in `gasFees` are larger
  (~2.5x for burn, ~1.7x for mint; approve now reflects its reserved limit and
  always returns a value). This does **not** change what a bridge costs — unused
  gas is refunded. If you use these values to fund a wallet, funding now succeeds
  where it previously failed with "insufficient funds." If you display them as an
  expected cost, the shown number will rise. Non-EVM estimates are unchanged.

  **Approve error contract:** previously, if gas estimation failed for the approve
  step, `gasFees` included an entry with `fees: null` and an `error`. It now
  always returns a value (the reserved floor), matching the execute path's
  fallback behaviour.
...
- Fix bridge failures on Sei and Edge, where the approval step reverted with
  `ONCHAIN_SIMULATION_FAILED: Transaction reverted`. Both chains cost more gas for
  `increaseAllowance` than the hardcoded 100,000 limit the approval was submitted
  with, so it ran out of gas. This affected mainnet and testnet on both chains, and
  failed every time the allowance had been spent down to zero — which is the normal
  state, since each bridge consumes the full allowance.

  Gas limits for the approve, burn, and mint steps are now sized from the chain's
  own `eth_estimateGas` result plus a 25% buffer, with the previous constants
  acting as a floor rather than a fixed cap. Chains that cost more than the floor
  are covered by their own estimate, and chains whose estimator under-reports are
  covered by the floor. No application changes are needed.

  Two behaviour changes worth noting if you pre-check native balances before
  bridging: the approve floor rises from 100,000 to 150,000 and the burn floor from
  300,000 to 500,000, which raises the native balance required to _submit_ those
  steps (`balance >= gasLimit * maxFeePerGas`) though not the gas actually spent.
  Each bridge also makes one additional `eth_estimateGas` call per step.
