...
- Align README examples with the current public API.

## 1.5.0

### Minor Changes

- Completed swaps are now included in SwapKit's usage analytics. Turn this off
  by passing `disableAnalytics: true` when constructing `SwapKit`.
...
- Fix a missing runtime dependency (`@coral-xyz/anchor` and `bn.js`) that could
  cause `Could not find package` errors under strict module resolvers (Deno,
  Supabase Edge Runtime, pnpm).
