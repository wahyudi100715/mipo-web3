...
Pass a Circle API key as `config.apiKey` to authenticate service-backed
operations. Omit it to use the permissionless (keyless) path.
...
const params = {
  ...
  amountIn: '100',
  config: {
    // API key for authentication (optional — omit for permissionless mode)
    apiKey: process.env.CIRCLE_API_KEY,
