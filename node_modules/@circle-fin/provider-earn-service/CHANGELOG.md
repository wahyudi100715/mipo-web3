...
- Add a decoded earn-transaction preview delivered through EarnKit's `onBeforeSign` callback. Exposes the decoded-preview types `DecodedEarnTx`, `DecodedEarnInstruction`, `DecodedTokenAmount`, `DecodedRewardClaim`, and `EarnActionKind` so applications can show what a deposit or withdrawal will do — input token and amount, target vault, receiver, Circle fee, and any claimed rewards and their recipients — before signing. Additive and opt-in.
...
- Report same-chain Earn approve, deposit, and withdraw transaction outcomes back to the Earn Service, improving the accuracy of future gas-fee estimates.
...
- Earn Service requests now include the SDK version in a request header, letting
  Circle track which SDK version each client is running. Additive — no change to
...
- Run cross-chain deposit pre-sign validation inside the `sign` phase. Previously, when a prepared bundle failed validation (value tampering, fee-token mismatch, or an invalid signing-validity window), the rejection threw between the `prepare` and `sign` phases: `crossChainDeposit` consumers observed `prepare: pending → success` followed by an unannounced throw, and the failure was not captured in the error trace. A rejection now dispatches a `sign` error step event and is recorded in the error trace, while the authorization is still never signed when validation fails.
...
- `pollApiWithValidation` gains an opt-in `backoff` retry strategy. The default
  (`'fixed'`) keeps the existing constant `retryDelay` between retries, so every
  current caller's retry timing is unchanged. Passing `backoff: 'exponential'`
  instead grows the wait off `retryDelay` (the Nth retry seeds at
  `retryDelay * 2 ** (N - 1)`, capped at the new optional `maxRetryDelayMs`,
  default 2000ms) and randomizes it with full jitter to `[0, cappedDelay]` —
  spreading out retries so rate-limited (429) bursts back off gracefully instead
  of retrying in lockstep and exhausting the token bucket.

  The Earn Service provider opts into `'exponential'` for all its API calls.
  Other kits keep the fixed-delay behavior unless they opt in.

## 1.1.0

### Minor Changes

- The `EarningProvider` interface gains `actionDispatcher`, `registerDispatcher(dispatcher)`, `supportsRetry(error)`, and `retry(error)`. Custom providers must implement these members; a no-op `registerDispatcher` and a `supportsRetry` that returns `false` are sufficient for providers that do not participate in events or resume. `EarnServiceProvider` now emits step-level events and supports resuming a `deposit`, `withdraw`, or `claimRewards` operation that failed after a prior phase (such as a token approval) had already succeeded; failures surface as a `KitError` carrying step progress and resume context. See `@circle-fin/earn-kit` 1.1.0 for the corresponding `kit.on()` / `kit.off()` / `kit.retry(error)` user API.

## 1.0.0

### Major Changes

- Initial Earn service provider release backing EarnKit with Circle Earn service APIs, typed validation, quote and execution helpers, and Earn-specific API error parsing.
