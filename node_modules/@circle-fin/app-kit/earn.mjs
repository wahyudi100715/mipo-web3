 ...
 * ```
 */ const earnBridgeSourceChainIdentifierSchema = z.union([
 ...
 * ```
 */ const earnBridgeDestinationChainIdentifierSchema = z.union([
 ...
 * }
 * ```
 */ const supportedSwapTokenSchema = z.string().transform((value)=>value.toUpperCase()).pipe(swapTokenEnumSchema);
 ...
 */ const allowanceStrategySchema$1 = z.enum([
    'permit',
    'approve'
], {
 ...
 */ const serviceSwapCustomFeeSchema = z.object({
 ...
 */ const formattedFeeAmountSchema = z.string({
 ...
 */ z.object({
    ...
    protocolFee: z.bigint().optional().refine((v)=>v === undefined || v >= 0n, 'protocolFee cannot be negative')
});
 ...
 */ const solanaChainIdentifierSchema = z.union([
    ...
    z.nativeEnum(Blockchain).refine((val)=>val === Blockchain.Solana || val === Blockchain.Solana_Devnet, 'Must be a Solana blockchain enum value (Blockchain.Solana or Blockchain.Solana_Devnet)'),
    z.object({
 ...
 * ```
 */ const solanaWalletProviderSchema = z.object({
    ...
    signAllTransactions: z.function().returns(z.promise(z.array(z.unknown()))).optional(),
    signMessage: z.function().returns(z.promise(z.object({
...
const swapChainIdentifierField = z.custom((value)=>swapChainIdentifierSchema.safeParse(value).success, {
 ...
 */ const allowanceStrategySchema = z.enum([
    'permit',
    'approve'
]);
/**
 ...
 */ const swapCustomFeeSchema = z.object({
    /**
 ...
 */ const swapConfigSchema = z.object({
    ...
    stopLimit: z.string().min(1, 'Required').pipe(createDecimalStringValidator({
        allowZero: true,
 ...
 */ let _symbolResult;
const swapTokenSchema = z.string().superRefine((value, ctx)=>{
...
const amountInSchema = z.string().min(1, 'Required').pipe(createDecimalStringValidator({
    allowZero: false,
    regexMessage: AMOUNT_FORMAT_ERROR_MESSAGE,
    attributeName: 'amountIn',
    maxDecimals: 18
})(z.string())).describe('The amount of the input token to swap, expressed as a human-readable ' + "decimal string in token units (e.g., '0.05' for 0.05 USDC or 0.05 ETH).");
/**
...
const sourceAdapterContextSchema = z.object({
    adapter: adapterSchema,
 ...
 */ const getVaultsParamsSchema = z.object({
    vaults: z.array(vaultQuerySchema).min(1, 'at least one vault query is required').max(20, 'maximum 20 vault queries per request'),
 ...
 * })
 * ```
 */ const exploreVaultsParamsSchema = z.object({
    chain: earnChainIdentifierSchema,
    protocol: z.string().min(1, 'protocol must not be empty').optional(),
    asset: z.string().min(1, 'asset must not be empty').optional(),
    ...
    page: z.number().int('page must be an integer').min(1, 'page must be at least 1').max(MAX_EXPLORE_PAGE, `page must be at most ${MAX_EXPLORE_PAGE.toString()}`).optional(),
    pageSize: z.number().int('pageSize must be an integer').min(100, 'pageSize must be at least 100').max(500, 'pageSize must be at most 500').optional(),
    config: earnConfigSchema.optional()
});
/**
 ...
 */ const sameChainDepositParamsSchema = z.object({
    from: adapterContextSchema,
...
const crossChainDepositDestinationSchema = z.object({
 ...
 *
 * @internal Exported for branch-targeted validation in the deposit operation.
 */ const crossChainDepositParamsSchema = z.object({
 ...
 * })
 * ```
 */ const getCrossChainDepositStatusParamsSchema = z.object({
 ...
 * })
 * ```
 */ const waitForCrossChainDepositParamsSchema = z.object({
    execId: z.string().uuid(),
    config: earnConfigSchema.optional(),
...
const sameChainGetDepositQuoteParamsSchema = z.object({
    from: adapterContextSchema,
    vaultAddress: vaultAddressSchema,
    amount: amountSchema,
    config: earnConfigSchema.optional()
...
const getDepositQuoteParamsPreflightSchema = z.object({
    chain: z.unknown().optional(),
    address: z.unknown().optional(),
    transferSpeed: z.unknown().optional()
