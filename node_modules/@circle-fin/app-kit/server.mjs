...
/**
 * Identity helper for declaring a kit's slot map.
 *
 * @remarks
 * `defineSlots()` is a compile-time-only helper: it returns its input
 * untouched at runtime, but its `const`-generic signature preserves the
 * literal `slotKey` types that a bare type annotation would widen away.
 * Those literals are what {@link DefaultsFor} keys off when narrowing a
 * kit's defaults map, so every kit declares its slots through this
 * helper rather than annotating the object directly.
 *
 * @packageDocumentation
 */ /**
 * Identity helper for declaring a kit's slot map with full inference.
 *
 * @remarks
 * Using `defineSlots()` instead of a bare type annotation preserves
 * literal `slotKey` types so the {@link DefaultsFor} narrowing works.
 *
 * @typeParam T - The slot-map shape being declared. The constraint
 *   keeps every entry shaped like a {@link KitSlotDefinition} while
 *   preserving literal `slotKey` types.
 * @param slots - The slot-map literal.
 * @returns The same object, unchanged at runtime, but with its literal
 *   types preserved so `DefaultsFor` can narrow correctly.
 *
 * @example
 * ```ts
 * const BridgeKitSlots = defineSlots({
 *   cctpV2: {
 *     provider: createCctpV2Provider({}),
 *     defaults: {} as CctpV2Defaults,
 *   },
 * })
 * type BridgeKitSlotMap = typeof BridgeKitSlots
 * ```
...
/**
 * Assemble a {@link KitContext} from built-in slots, user-supplied
 * providers, and the narrowed defaults map.
 *
 * @remarks
 * This is the assembly step every kit factory calls. The runtime body
 * is a thin merge; the value of this function is the type signature,
 * which threads `TSlots` and `TProviders` end-to-end so downstream
 * operations inherit the narrowed shape.
 *
 * Runtime behaviour:
 *
 * - User providers whose `slotKey` matches a built-in slot replace
 *   that slot's provider; their entry is otherwise dropped.
 * - User providers with unknown `slotKey`s are collected onto
 *   `providers.extras`.
 * - `config` flows through verbatim and is `undefined` when omitted.
 *
 * @typeParam TSlots - The kit's slot map.
 * @typeParam TProviders - The user's `providers: [...]` tuple.
 * @param options - {@link CreateKitContextOptions}.
 * @returns Frozen {@link KitContext}.
 * @throws TypeError - When a built-in slot has no provider defined and
 *   no user override either (developer-time bug).
 *
 * @example
 * ```ts
 * import { createKitContext, defineSlots } from '@core/kit-base'
 *
 * const slots = defineSlots({})
 * const context = createKitContext({
 *   identity: { name: 'my-kit', version: '0.0.1' },
 *   builtInSlots: slots,
 *   providers: [] as const,
 *   defaults: {},
 * })
 * ```
 */ function createKitContext(options) {
    const providersTuple = options.providers ?? [];
    // Reject duplicate `slotKey`s up front. Without this, two user
    // providers sharing a slotKey would silently collapse to the first
    // match (`find` below / the `extras` filter), dropping the rest with
    // no signal — a developer wiring bug. Surface it at construction,
    // mirroring the "Missing built-in kit provider slot" throw.
    const seenSlotKeys = new Set();
    for (const provider of providersTuple){
        if (seenSlotKeys.has(provider.slotKey)) {
            throw new TypeError(`Duplicate kit provider slot: ${provider.slotKey}. Each slotKey may appear at most once in \`providers\`.`);
        }
        seenSlotKeys.add(provider.slotKey);
    }
    const slotKeys = Object.keys(options.builtInSlots);
    /**
   * Resolve each built-in slot to its effective provider: the user
   * override when present, otherwise the built-in default. A slot with
   * neither is a developer-time wiring bug, hence the throw rather than
   * a silent skip.
   */ const filledProviders = {};
    for (const key of slotKeys){
        const userOverride = providersTuple.find((p)=>p.slotKey === key);
        const builtInSlot = options.builtInSlots[key];
        if (builtInSlot === undefined) {
            throw new TypeError(`Missing built-in kit provider slot: ${key}`);
        }
        const provider = userOverride ?? builtInSlot.provider;
        filledProviders[key] = provider;
    }
    /**
   * Everything the user passed that neither overrode a known slot nor
   * matches a built-in key — surfaced verbatim on `providers.extras` so
   * kits can layer optional, kit-specific providers without a slot.
   */ // `Array.prototype.filter` cannot narrow a `readonly KitProvider[]` down
    // to the specific `extras` tuple type that `ProviderRegistry<TSlots, TProviders>`
    // carries — TypeScript's filter overloads don't model the predicate deeply enough
    // to produce the conditional tuple. The cast is safe: at runtime `extras` is
    // exactly the non-built-in providers in their original order.
    const extras = providersTuple.filter((p)=>!slotKeys.includes(p.slotKey));
    const providerRegistry = {
        ...filledProviders,
        extras
    };
    // The kit owns one runtime for its whole call tree. Default to a
    // fresh `createRuntime()` so a single `EventBus` backs the kit's
    // `on` / `off` and every operation's emitted events.
    const runtime = options.runtime ?? createRuntime();
    // The kit owns one token registry for its whole call tree. Default to
    // `createTokenRegistry()` (seeded with the built-in DEFAULT_TOKENS) so
    // token-less kits stay on the standard `InvocationContext` shape without
    // crafting a fork.
    ...
    return {
        ...
        runtime,
        tokens,
        // Omit optional keys entirely when unset so `exactOptionalPropertyTypes`
        // stays satisfied — the key is absent rather than `undefined`.
        ...options.config === undefined ? {} : {
            config: options.config
        },
        ...options.normalizeError === undefined ? {} : {
...
/**
 * Build a {@link KitOperationFactory} from a declarative operation spec.
 *
 * @remarks
 * Generic preservation is the whole point of this factory. Parameters
 * `TParams` and `TResult` are inferred from {@link CreateKitOperationOptions}
 * and threaded onto the produced {@link KitOperation} so the kit's method
 * surface inherits them at the call site without explicit annotations.
 *
 * The returned value is a factory `(ctx) => KitOperation`; {@link createKit}
 * calls it with the assembled {@link KitContext} to bind the operation to
 * the kit's providers, defaults, config, and runtime.
 *
 * Behaviour at call time (per the `@core/runtime` adapter standard):
 *
 * 1. Validate `params` with the optional Zod schema. A failure throws an
 *    `INPUT_VALIDATION_FAILED` `KitError` via `@core/errors`'s
 *    `createValidationErrorFromZod` (the formatted issues and raw Zod
 *    issues are attached as `cause.trace` for downstream branching).
 * 2. Resolve the raw `InvocationMeta` into a frozen `InvocationContext`
 *    via `@core/runtime`'s `resolveInvocationContext`, defaulting
 *    `runtime` / `tokens` to the kit's own so one `EventBus` collects
 *    the whole call tree and tokens resolve against one registry.
 * 3. Run the user-supplied `run` inside the runtime middleware stack
 *    (`createMiddlewareStack` + `runOperation`) so lifecycle events,
 *    retries, and error normalization apply uniformly. `run` receives
 *    the validated params and the resolved {@link KitOperationContext}.
 *
 * Kit identity stamping is performed by {@link createKit} at the kit
 * boundary, not here — that way the operation factory does not need to
 * know which kit it belongs to.
 *
 * @typeParam TParams - Operation parameter shape.
 * @typeParam TResult - Operation result shape.
 *
 * @param options - Declarative operation spec (name, optional schema, run fn).
 * @returns A {@link KitOperationFactory} that binds to a {@link KitContext}.
 * @throws KitError - `INPUT_VALIDATION_FAILED` (from
 *   `createValidationErrorFromZod`) when `options.schema` rejects the
 *   provided params. The formatted + raw Zod issues are exposed via
 *   `cause.trace` for callers that need the per-field issue list.
 *
 * @example
 * ```ts
 * import { createKit, createKitContext, createKitOperation, defineSlots } from '@core/kit-base'
 * import { z } from 'zod'
 *
 * const greet = createKitOperation({
 *   name: 'greet',
 *   schema: z.object({ name: z.string().min(1) }),
 *   run: async ({ name }, ctx) => {
 *     ctx.runtime.events.emit({ name: 'greet.done', data: { name } })
 *     return `hello ${name} (trace=${ctx.invocation.traceId})`
 *   },
 * })
 *
 * const kit = createKit({
 *   context: createKitContext({
 *     identity: { name: 'greeter', version: '0.0.1' },
 *     builtInSlots: defineSlots({}),
 *     defaults: {},
 *   }),
 *   operations: { greet },
 * })
 *
 * const message = await kit.greet({ name: 'world' })
 * ```
 */ function createKitOperation(options) {
    return (context)=>{
        const operation = async (params, meta)=>{
            // Validate first so `run` only ever sees well-formed params. The
            // schema is optional: kits with no untrusted input skip it and pass
            // params through verbatim.
            let validated;
            if (options.schema) {
                const parsed = options.schema.safeParse(params);
                if (!parsed.success) {
                    throw createValidationErrorFromZod(parsed.error, `input to ${options.name}`);
                }
                // `parsed.data` carries any schema-applied coercions/defaults.
                validated = parsed.data;
            } else {
                validated = params;
            }
            // Resolve raw meta into the shared `@core/runtime` InvocationContext.
            // `runtime` / `tokens` default to the kit's own so callers that omit
            // per-call overrides still emit onto the kit's single bus and resolve
            // against the kit's token registry. `signal` (when supplied) is
            // threaded through verbatim for cooperative cancellation.
            const invocation = resolveInvocationContext(meta, {
                runtime: context.runtime,
                tokens: context.tokens
            });
            // The resolved (per-call) runtime is the effective one — surface it
            // on the operation context so `run` emits where the middleware does.
            ...
            // Build the observability surface for the middleware from the same
            // resolved invocation the adapter layer uses.
            ...
            const middleware = createMiddlewareStack({
                ...
                ...context.normalizeError === undefined ? {} : {
                    normalizeError: context.normalizeError
                }
            });
            return runOperation(execContext, middleware, async ()=>options.run(validated, operationContext));
        };
        // Stamp the function's `name` so stack traces and debuggers show the
        // operation name instead of the anonymous async wrapper. Defined as a
        // non-writable own property to mirror a normal named function.
...
/**
 * Create a {@link KitEventSubscription} backed by a runtime's event bus.
 *
 * @remarks
 * Wraps the runtime's untyped `EventBus` with a
 * `TypedEventBus<KitEventMap>` and tracks handler → unsubscribe mappings
 * so `off(handler)` removes every subscription registered for that
 * handler reference.
 *
 * @typeParam M - The event map (defaults to {@link KitEventMap}).
 * @param runtime - The runtime whose event bus backs the subscription.
 * @returns A {@link KitEventSubscription} with `on` and `off`.
 *
 * @example
 * ```ts
 * import { createKitEvents } from '@core/kit-base'
 *
 * const events = createKitEvents(context.runtime)
 * const kit = { ...operations, ...events }
 * ```
 */ function createKitEvents(runtime) {
...
/**
 * Caller-stamping helper shared by {@link createKit} and
 * {@link createCompositeKit}.
 *
 * @remarks
 * Lives in its own module so both kit factories use the exact same
 * wrapper. The wrapper is append-only and idempotent — operations and
 * providers further down the stack continue to append their own
 * identities, which is what end-to-end telemetry attribution relies
 * on.
 *
 * @packageDocumentation
 */ /**
 * Wrap an operation so every invocation appends the given caller to
 * the invocation's call chain before reaching the operation body.
 *
 * @typeParam TParams - Parameter type of the wrapped operation.
 * @typeParam TResult - Result type of the wrapped operation.
 * @param op - Operation to wrap.
 * @param caller - Caller identity to append.
 * @returns A new operation with the same `.name` whose callers chain
 *   is augmented by `caller`.
 *
 * @remarks
 * The signature decomposes the operation's `TParams` / `TResult`
 * explicitly rather than using `TOp extends KitOperation` so that the
 * wrapper survives `exactOptionalPropertyTypes: true` and the rest of
 * the strict settings the spec tsconfig turns on. Constraining via
 * the operation's interface directly is brittle there because the
 * default arguments `unknown, unknown` widen contravariantly.
 *
 * @internal
 */ function stampCallerOnOperation(op, caller) {
    const wrapped = async (params, meta)=>{
        const enrichedMeta = {
            ...meta,
            callers: [
                ...meta?.callers ?? [],
...
/**
 * Assemble a kit from a {@link KitContext} and a registry of operation
 * factories.
 *
 * @remarks
 * Each entry in `options.operations` is a {@link KitOperationFactory}
 * (as returned by {@link createKitOperation}). `createKit` invokes each
 * factory with the assembled context to bind it to the kit's providers,
 * defaults, config, and runtime — mirroring how adapter actions bind to
 * an `AdapterContext`.
 *
 * The returned kit is a frozen object: one method per operation, plus
 * the {@link KitEventSubscription} `on`/`off` pair backed by the kit's
 * runtime `EventBus`. No mutable setters; runtime reconfiguration is
 * intentionally not supported.
 *
 * Generic preservation: `TFactories` is propagated through the return
 * type so each method retains its operation's parameter and result
 * generics. This is what keeps `AddressField` narrowing alive at the
 * call site.
 *
 * Operation names cannot collide with the reserved `on` / `off`
 * subscription methods; collisions throw a `TypeError` at construction.
 *
 * @typeParam TSlots - The kit's slot map.
 * @typeParam TProviders - The user's `providers: [...]` tuple.
 * @typeParam TFactories - Map from operation name to operation factory.
 * @param options - Kit context and operation-factory registry to expose.
 * @returns Frozen kit instance with operation methods and event surface.
 * @throws KitError - When an operation entry is `undefined`, or when
 *   an operation name shadows the reserved `on` / `off` methods.
 *
 * @example
 * ```ts
 * import { createKit, createKitContext, createKitOperation, defineSlots } from '@core/kit-base'
 *
 * const ping = createKitOperation({
 *   name: 'ping',
 *   run: async () => 'pong',
 * })
 *
 * const context = createKitContext({
 *   identity: { name: 'demo-kit', version: '0.0.1' },
 *   builtInSlots: defineSlots({}),
 *   defaults: {},
 * })
 *
 * const kit = createKit({ context, operations: { ping } })
 * const result = await kit.ping(undefined)
 * ```
 */ function createKit(options) {
    // The kit's public events wrap the runtime's single EventBus — the same
    // bus the operation middleware and any downstream providers emit on.
    const events = createKitEvents(options.context.runtime);
    // Stamp the kit's identity into every invocation's caller chain at the
    // kit boundary. This frees individual operations from knowing which
    // kit they live inside and gives downstream telemetry a stable handle
    // for attribution. Stamping is append-only and idempotent — operations
    // and providers further down the stack continue to append their own
    // identities.
    const kitCaller = {
        type: 'kit',
        name: options.context.identity.name,
        version: options.context.identity.version
    };
    const boundOperations = {};
    for (const key of Object.keys(options.operations)){
        if (key === 'on' || key === 'off') {
            throw new KitError({
                ...InputError.RESERVED_OPERATION_NAME,
                recoverability: 'FATAL',
                message: `Kit operation "${key}" shadows the reserved event subscription method.`
            });
        }
        const factory = options.operations[key];
        if (factory === undefined) {
            throw new KitError({
                ...InputError.MISSING_OPERATION,
                recoverability: 'FATAL',
                message: `Missing kit operation: ${key}`
            });
        }
        // Bind the operation to the kit's context, then stamp the kit caller
        // so every invocation carries the kit identity in its call chain.
        // The cast bridges the specific `KitContext<TSlots, TProviders>` to
        // the loose `KitContext` the factory declares: the `providers.extras`
        // field is invariant, so the specific context is not structurally
        // assignable even though it is a safe superset at runtime.
        const op = factory(options.context);
        boundOperations[key] = stampCallerOnOperation(op, kitCaller);
    }
    // Operations live last in the spread so any future event-surface entry
    // can never accidentally shadow a registered kit operation.
...
/**
 * Default base URL for the Circle wallets-api facade.
 *
 * @remarks
 * `wallets-api` exposes the `POST /v1/stablecoinKits/sessions` endpoint
 * that exchanges a long-lived `apiKey` for a short-lived
 * `sessionToken`. The same facade routes onramp traffic to the
 * appropriate downstream services, so a single `baseUrl` covers every
 * server-side call.
 *
 * Production traffic targets `https://api.circle.com`. Override via
 * `createOnrampServerKit({ baseUrl })` to point at staging. No public
 * sandbox exists today, so production is the default.
...
/**
 * Path on `wallets-api` for the API-key → session-token exchange.
 *
 * @remarks
 * Authenticated by the API-key exchange middleware on `wallets-api`. The
 * middleware expects the `Authorization` header to
 * carry the console-issued credential verbatim as a bearer token
 * (`Authorization: Bearer <apiKey>`); the kit adds only the `Bearer `
 * scheme.
 */ const SESSIONS_PATH = '/v1/stablecoinKits/sessions';
/**
 * Header name advertising the SDK that minted the request, primarily
 * for server-side observability dashboards on `wallets-api`.
 */ const SDK_USER_AGENT_HEADER = 'x-circle-sdk-user-agent';
/**
 * Default time budget (in ms) for an API-key → session-token exchange
 * before the request is considered failed.
 *
 * @remarks
 * `wallets-api` typically responds in under 500 ms; we set the budget
 * at 15 s to absorb cold-start variance and transient network issues
 * while still failing fast for the end user. Override per call via
 * `createOnrampServerKit({ requestTimeoutMs })`.
 ...
 * `OnrampServerKit`).
 */ const ONRAMP_SERVER_KIT_IDENTITY = {
...
/**
 * Assert that a usable `fetch` implementation was resolved.
 *
 * @param fetchImpl - The resolved fetch (caller-provided or global).
 *
 * @throws KitError - `INPUT_FETCH_UNAVAILABLE` (code `1902`) when no
 *   callable `fetch` is available (e.g. Node older than 18 without a
 *   polyfill).
 */ function assertFetchImplementation(fetchImpl) {
    if (typeof fetchImpl !== 'function') {
        throw new KitError({
            ...
            type: 'INPUT',
            recoverability: 'FATAL',
            message: 'No `fetch` implementation is available. Provide one explicitly via `createOnrampServerKit({ fetch })` or run on Node 18+ where `globalThis.fetch` is built in.'
        });
    }
}
/**
 * Assert that a non-empty API key was supplied.
 *
 * @param apiKey - The caller-supplied credential.
 *
 * @throws KitError - `INPUT_MISSING_API_KEY` (code `1900`) when the
 *   API key is missing or empty.
 */ function assertApiKey(apiKey) {
    if (typeof apiKey !== 'string' || apiKey.length === 0) {
        throw new KitError({
            code: 1900,
            name: 'INPUT_MISSING_API_KEY',
            type: 'INPUT',
            recoverability: 'FATAL',
            message: '`apiKey` is required to mint onramp sessions.'
        });
    }
}

/**
 * Compose an absolute wallets-api URL from a base URL and a path.
 *
 * @param baseUrl - The configured wallets-api base URL.
 * @param path - Request path. Must begin with `/`.
 * @returns The fully-qualified request URL.
 *
 * @throws KitError - `INPUT_INVALID_PATH` (code `1903`) when `path`
 *   does not start with `/`, or when the inputs cannot form a valid
 *   URL.
 *
 * @remarks
 * Enforce the leading-`/` convention so a relative `path` cannot be
 * resolved against the base's pathname segment (and so the helper
 * stays grep-able). The WHATWG `URL(input, base)` constructor handles
 * trailing-slash normalisation correctly once the path is anchored.
 */ function composeUrl(baseUrl, path) {
    if (!path.startsWith('/')) {
        throw new KitError({
            code: 1903,
            name: 'INPUT_INVALID_PATH',
            type: 'INPUT',
            recoverability: 'FATAL',
            message: `Onramp API path must begin with "/" (received: ${path}).`
        });
    }
    try {
        ...
        throw new KitError({
            ...
            type: 'INPUT',
            recoverability: 'FATAL',
            message: `Could not compose URL from baseUrl=${baseUrl} and path=${path}.`,
            cause: {
...
/**
 * A linked request signal plus a disposer that detaches any listener
 * registered on the upstream signal.
 */ /** Shared no-op disposer for the "nothing to detach" branches. */ const noop = ()=>undefined;
/**
 * Link an upstream caller signal to a request-scoped controller so the
 * request aborts when either the caller cancels or the timeout fires.
 *
 * @param upstream - Optional caller-provided `AbortSignal`.
 * @param controller - The request-scoped controller (also driven by the
 *   timeout).
 * @returns A {@link LinkedSignal} carrying the controller's signal and a
 *   `dispose` that removes the upstream subscription.
 *
 * @remarks
 * Forwards an already-aborted upstream signal immediately, and otherwise
 * subscribes once so the request is cancelled cooperatively. The caller
 * MUST invoke `dispose()` when the request settles: a reused or long-
 * lived upstream signal (e.g. one per-server-lifetime cancellation
 * token) would otherwise accumulate one `abort` listener per request,
 * leaking memory and tripping Node's `MaxListenersExceededWarning`.
 */ function linkSignal(upstream, controller) {
    if (!upstream) {
        return {
...
/**
 * Map a thrown `fetch` rejection to a structured {@link KitError}.
 *
 * @param err - The value thrown by `fetch`.
 * @returns `NETWORK_TIMEOUT` for abort/timeout errors, otherwise
 *   `NETWORK_CONNECTION_FAILED`. Both are `RETRYABLE`.
 */ function mapFetchError(err) {
    if (isAbortLikeError(err)) {
        return new KitError({
            ...NetworkError.TIMEOUT,
            recoverability: 'RETRYABLE',
            message: 'The Onramp API request timed out or was aborted.',
            cause: {
                trace: serializeError(err)
            }
        });
    ...
    return new KitError({
        ...NetworkError.CONNECTION_FAILED,
        recoverability: 'RETRYABLE',
        message: 'Could not reach the Onramp API.',
        cause: {
            trace: serializeError(err)
        }
    });
...
/**
 * Map a non-2xx wallets-api response to a structured {@link KitError}.
 *
 * @param status - The HTTP status code.
 * @param rawText - The raw response body. Only its length is recorded;
 *   the body itself is never embedded.
 * @returns A {@link KitError} whose `code`/`name`/`recoverability`
 *   reflect the failure class:
 *
 * | Status         | KitError                              |
 * | -------------- | ------------------------------------- |
 * | 401 / 403      | `INPUT_INVALID_API_KEY` (FATAL)       |
 * | 429            | `RATE_LIMIT_EXCEEDED` (RETRYABLE)     |
 * | 5xx            | `SERVICE_INTERNAL_ERROR` (RETRYABLE)  |
 * | other 4xx      | `INPUT_SESSION_REJECTED` (FATAL)      |
 *
 * @remarks
 * The verbatim upstream body is never embedded — not even a truncated
 * slice — because wallets-api error pages can echo a submitted email or
 * address or carry internal identifiers, and `cause.trace` reaches
 * server logs and the `onError` hook un-redacted. Only `statusCode` and
 * `bodyLength` are recorded, which is enough to correlate with upstream
 * logs without leaking payload contents. This mirrors the client half
 * (`fetchOnrampSession`), which also reads error bodies length-only.
 */ function mapHttpError(status, rawText) {
    const trace = {
        statusCode: status,
    ...
    if (status === 401 || status === 403) {
        return new KitError({
            code: 1907,
            name: 'INPUT_INVALID_API_KEY',
            type: 'INPUT',
            recoverability: 'FATAL',
            message: 'The API rejected your API key. Verify ONRAMP_API_KEY is the full value from the Circle console (<ENV>_API_KEY:<keyId>:<keySecret>) and that it matches the environment your configured baseUrl targets.',
            cause: {
                trace
            }
        });
    ...
    if (status === 429) {
        return new KitError({
            ...RateLimitError.RATE_LIMIT_EXCEEDED,
            recoverability: 'RETRYABLE',
            message: 'The Onramp API rate-limited the session request. Retry after a short delay.',
            cause: {
                trace
            }
        });
    ...
    if (status >= 500) {
        return new KitError({
            ...ServiceError.INTERNAL_ERROR,
            recoverability: 'RETRYABLE',
            message: `The Onramp API returned a server error (HTTP ${String(status)}) while creating the session. This is usually transient — retry shortly.`,
            cause: {
                trace
            }
        });
    ...
    return new KitError({
        code: 1901,
        name: 'INPUT_SESSION_REJECTED',
        type: 'INPUT',
        recoverability: 'FATAL',
        message: `The Onramp API rejected the session request (HTTP ${String(status)}). Check the request parameters.`,
        cause: {
            trace
        }
    });
...
/**
 * Headers we ignore from `OnrampServerKitOptions.extraHeaders` so the
 * wire format stays predictable.
 */ const RESERVED_HEADERS = new Set([
    'authorization',
    'content-type',
    'accept',
    SDK_USER_AGENT_HEADER.toLowerCase()
]);
/**
 * Filter caller-supplied extra headers down to the allowed set.
 *
 * @param headers - Caller-supplied extra headers (untrusted shape).
 * @returns A plain object of allowed headers; reserved and non-string
 *   entries are dropped.
 *
 * @remarks
 * Reserved headers (`Authorization`, `Content-Type`, `Accept`, and the
 * SDK user-agent header) are managed by the client and cannot be
 * overridden, so the bearer credential and content negotiation stay
 * deterministic.
 ...
 * @returns The body text, or an empty string if the body could not be
 *   read (already consumed, network reset mid-stream, etc.).
...
/**
 * Build the embedded wallets-api client used by the server kit's
 * operations.
 *
 * @param options - {@link CreateOnrampApiClientOptions}.
 * @returns A stateless {@link OnrampApiClient} safe to share across
 *   operations.
 *
 * @throws KitError - `INPUT_FETCH_UNAVAILABLE` (code `1902`) when no
 *   `fetch` is available, or `INPUT_MISSING_API_KEY` (code `1900`)
 *   when the API key is empty.
 *
 * @remarks
 * The factory pre-resolves defaults so each call inside the hot path
 * is a straight property lookup.
 *
 * @example
 * ```ts
 * const client = createOnrampApiClient({ apiKey })
 * const session = await client.postJson('/v1/stablecoinKits/sessions', body)
 * ```
 */ function createOnrampApiClient(options) {
    const baseUrl = options.baseUrl ?? DEFAULT_API_BASE_URL;
    const requestTimeoutMs = options.requestTimeoutMs ?? DEFAULT_REQUEST_TIMEOUT_MS;
    const fetchImpl = options.fetch ?? globalThis.fetch;
    const extraHeaders = pickAllowedHeaders(options.extraHeaders);
    assertFetchImplementation(fetchImpl);
    assertApiKey(options.apiKey);
    return {
        async postJson (path, body, signal) {
            const url = composeUrl(baseUrl, path);
            const controller = new AbortController();
            const { signal: linkedSignal, dispose } = linkSignal(signal, controller);
            const timeoutHandle = requestTimeoutMs > 0 ? setTimeout(()=>{
                controller.abort();
            ...
            try {
                ...
                try {
                    response = await fetchImpl(url, {
                        method: 'POST',
                        headers: {
                            // Bearer auth. The API key issued by the Circle console is
                            // used verbatim as the bearer credential (it already carries
                            // its own `<ENV>_API_KEY:` scheme prefix); we only add the
                            // `Bearer ` scheme.
                            Authorization: `Bearer ${options.apiKey}`,
                            'Content-Type': 'application/json',
                            Accept: 'application/json',
                            [SDK_USER_AGENT_HEADER]: `${ONRAMP_KIT_NAME}/${ONRAMP_KIT_VERSION}`,
                            ...extraHeaders
                        },
                ...
                try {
                    ...
                    throw new KitError({
                        ...
                        recoverability: 'RETRYABLE',
                        message: `The Onramp API returned an unexpected (non-JSON) response (HTTP ${String(response.status)}).`,
                        cause: {
                            trace: {
                                bodyLength: rawText.length,
                ...
                // Always release the per-request timer and the upstream-signal
                // subscription so a reused/long-lived caller signal cannot
...
/**
 * Serialize an {@link OnrampAssetSelection} into the widget's launch-URL
 * query params.
 *
 * @packageDocumentation
 */ /**
 * Convert a host-app asset selection into the `chains` / `tokens` / `pairs`
 * query params the hosted widget parses.
 *
 * @remarks
 * This is the wire contract shared with the widget (onramp-web): comma-
 * separated lists, and each pair encoded as `token:chain`. Empty or omitted
 * fields are left out entirely so the widget falls back to its full catalog.
 * Values are passed through verbatim (the widget normalizes case and maps
 * chain labels/ids), keeping the SDK forward-compatible with catalog growth.
 *
 * @param assets - The selection to serialize, or `undefined`.
 * @returns A sparse record of launch-URL params (only non-empty fields).
 *
 * @example
 * ```ts
 * serializeAssetSelection({ tokens: ['USDC'], chains: ['arc'] })
 * // => { tokens: 'USDC', chains: 'arc' }
 *
 * serializeAssetSelection({ pairs: [{ token: 'USDC', chain: 'base' }] })
 * // => { pairs: 'USDC:base' }
 * ```
 */ function serializeAssetSelection(assets) {
    if (assets === undefined) {
        return {};
    }
    const params = {};
    if (assets.tokens !== undefined && assets.tokens.length > 0) {
        params['tokens'] = assets.tokens.join(',');
    }
    if (assets.chains !== undefined && assets.chains.length > 0) {
        params['chains'] = assets.chains.join(',');
    }
    if (assets.pairs !== undefined && assets.pairs.length > 0) {
        params['pairs'] = assets.pairs.map((pair)=>`${pair.token}:${pair.chain}`).join(',');
    }
    return params;
}
...
/**
 * Build the `createSession` operation factory.
 *
 * @remarks
 * Two layers of "factory" stack here, by design:
 *
 * 1. This function closes over the kit-level HTTP client and widget
 *    origin — collaborators the empty-slot kit context cannot carry.
 * 2. {@link createKitOperation} returns a {@link KitOperationFactory}
 *    `(ctx) => KitOperation`; {@link createKit} binds it to the kit's
 *    runtime/config so the operation runs inside the runtime middleware
 *    stack (lifecycle/retry/error) and emits on the kit's single bus.
 *
 * The `run` reads `ctx.invocation` for the resolved `traceId` and
 * `signal`, forwarding the signal to the API client so callers can
 * cancel an in-flight mint.
 *
 * @param options - Operation collaborators.
 * @returns A {@link KitOperationFactory} for the `createSession` op.
 */ function createSessionOperation(options) {
    const { api, widgetBaseUrl, referrerDomain } = options;
    return createKitOperation({
        name: 'createSession',
        schema: onrampSessionRequestSchema,
        run: async (params, ctx)=>{
            const raw = await api.postJson(SESSIONS_PATH, toRequestBody(params, ctx.invocation.traceId, referrerDomain), ctx.invocation.signal);
            // No response-shape validation on purpose: the Onramp API owns its
            // response contract, and asserting a fixed schema here would couple
            // the kit to every additive change (a new field or a renamed one
            // would break minting outright). We unwrap the `{ data }` envelope
            // when present and read only the fields the client kit relies on.
            // Launch-time safety (token presence, expiry, widget-URL
            // scheme/origin) is still enforced in `getOnrampUrl`.
            //
            // A non-object response degrades to an empty record rather than
            // throwing — downstream normalisation and the launch-time checks
            // surface any genuinely unusable session.
            const unwrapped = unwrapDataEnvelope(raw);
            const session = unwrapped !== null && typeof unwrapped === 'object' ? unwrapped : {};
            const sessionToken = typeof session['sessionToken'] === 'string' ? session['sessionToken'] : undefined;
            const existingWidgetUrl = typeof session['widgetUrl'] === 'string' && session['widgetUrl'].length > 0 ? session['widgetUrl'] : undefined;
            // The widget reads the destination wallet from the launch URL
            // (`destinationWallet` query param), NOT from the session token —
            // the `/v1/stablecoinKits/sessions` body is anonymous. We must
            // therefore bake it into the composed URL or the widget reports
            // "Destination wallet is required." mid-flow. Host-app asset scoping
            // (`assets`) rides the same launch URL as `chains`/`tokens`/`pairs`.
            const assetParams = serializeAssetSelection(params.assets);
            const composedWidgetUrl = sessionToken === undefined ? undefined : composeWidgetUrl(widgetBaseUrl, sessionToken, ONRAMP_WIDGET_PATH, {
                destinationWallet: params.destinationAddress,
                ...assetParams
            });
            // When wallets-api returns a `widgetUrl`, asset scoping must still be
            // applied — `assets` is a display-only widget concern and is never
            // forwarded upstream, so we merge the params directly into the returned
            // URL here rather than letting it win verbatim.
            const widgetUrl = mergeAssetParams(existingWidgetUrl, composedWidgetUrl, assetParams);
            const traceId = typeof session['traceId'] === 'string' ? session['traceId'] : ctx.invocation.traceId;
            // Outbound allow-list. We deliberately rebuild the result from the
            // exact fields the client kit consumes rather than spreading
            // `...session`. Inbound parsing stays schema-free for forward-compat
            // (an additive upstream field must never break minting), but the
            // server→browser hop is a trust boundary: any internal identifier,
            // PII, or debug field the Onramp API adds now or later must NOT
            // auto-forward to the client. Unmodeled fields are dropped here.
            const normalised = {
                destinationWallet: params.destinationAddress,
                traceId
            };
            if (sessionToken !== undefined) {
                normalised['sessionToken'] = sessionToken;
            }
            if (typeof session['sessionId'] === 'string') {
                normalised['sessionId'] = session['sessionId'];
            }
            if (typeof session['expiresAt'] === 'string') {
                normalised['expiresAt'] = session['expiresAt'];
            }
            if (widgetUrl === undefined) {
                // `widgetUrl` is required on `MintedOnrampSession` (guaranteed
                // present). If neither the API returned one nor a `sessionToken`
                // was present to compose it, the session is unusable — throw
                // rather than return a structurally invalid object under a
                // guaranteed-present type (which would cause a silent TypeError
                // at widget-mount time).
                throw new KitError({
                    code: 8925,
                    name: 'SERVICE_SESSION_MISSING_WIDGET_URL',
                    type: 'SERVICE',
                    recoverability: 'RETRYABLE',
                    message: 'Onramp API response did not include a `widgetUrl` or a `sessionToken` from which one can be composed.'
                });
            }
            normalised['widgetUrl'] = widgetUrl;
            return normalised;
        }
    });
}
/**
 * Merge asset-scoping query params into the resolved widget URL.
 *
 * @internal
 */ function mergeAssetParams(existingWidgetUrl, composedWidgetUrl, assetParams) {
    if (existingWidgetUrl === undefined) {
        return composedWidgetUrl;
    }
    const url = new URL(existingWidgetUrl);
...
/**
 * Translate the SDK parameter shape into the wallets-api request body.
 *
 * @remarks
 * Centralising the mapping keeps the SDK's public field names friendly
 * (`destinationAddress`, `appUserId`, …) regardless of wallets-api's
 * internal naming. In particular, the session endpoint
 * (`POST /v1/stablecoinKits/sessions`) validates a **required
 * `walletAddress`** string, so the public `destinationAddress` param is
 * mapped to `walletAddress` on the wire here. Keeping the wire name in
 * one place avoids the field-name drift that would otherwise surface
 * only as a runtime 400 from the API.
 *
 * @internal
 */ function toRequestBody(params, traceId, referrerDomain) {
    const body = {
    ...
    if (params.destinationChain !== undefined) {
        body['destinationChain'] = params.destinationChain;
    }
    ...
    // Server-derived, not from `params`: `referrerDomain` is a kit-construction
    // option so a client-submitted session request can never widen the
    // frame-ancestor allowlist.
    if (referrerDomain !== undefined) {
        body['referrerDomain'] = referrerDomain;
    }
...
/**
 * Assert that `options` is a usable {@link OnrampServerKitOptions}
 * object.
 *
 * @param options - The value supplied to `createOnrampServerKit`.
 *
 * @throws KitError - When `options` is not an object, or any of
 *   `baseUrl` / `widgetBaseUrl` / `requestTimeoutMs` / `referrerDomain`
 *   is present but malformed. URL fields must be absolute `https://` URLs
 *   (with a `http://localhost` carve-out for fixtures); `requestTimeoutMs`
 *   must be a non-negative integer; `referrerDomain` must be a bare
 *   hostname (no scheme, port, path, wildcard, or whitespace).
 *
 * @remarks
 * Runtime-validated even though strict TypeScript catches most
 * mistakes — plain-JS consumers and bundlers that strip generics
 * need the same guarantees. `apiKey` itself is validated downstream
 * by the API client so the wire error stays authoritative.
 *
 * @example
 * ```ts
 * assertOnrampServerKitOptions({ apiKey: '<ENV>_API_KEY:keyId:keySecret' })
 * ```
 *
 * @internal
 */ function assertOnrampServerKitOptions(options) {
    if (options === null || typeof options !== 'object') {
        throw new KitError({
            code: 1915,
            name: 'INPUT_INVALID_SERVER_OPTIONS',
            type: 'INPUT',
            recoverability: 'FATAL',
            message: '`createOnrampServerKit` requires an options object with an `apiKey`.'
        });
    }
    const candidate = options;
    assertOptionalAbsoluteUrl(candidate.baseUrl, 'baseUrl');
    assertOptionalAbsoluteUrl(candidate.widgetBaseUrl, 'widgetBaseUrl');
    assertOptionalNonNegativeInteger(candidate.requestTimeoutMs, 'requestTimeoutMs');
    assertOptionalReferrerDomain(candidate.referrerDomain);
}
// Bare-hostname matcher: one or more dot-separated labels of
// alphanumerics and internal hyphens. Deliberately rejects a scheme
// (`https://`), port (`:443`), path (`/buy`), wildcard (`*.arc.io`), and
// any whitespace — everything the `referrerDomain` JSDoc forbids.
const HOSTNAME_PATTERN = /^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)*$/i;
function assertOptionalReferrerDomain(value) {
    if (value === undefined) return;
    // Present-but-invalid fails fast (like `baseUrl`) rather than reaching
    // wallets-api as a blank/malformed `referrerDomain` claim that only
    // surfaces later as an iframe `frame-ancestors` failure. Empty and
    // whitespace-only are treated as invalid — omit the option instead.
    if (typeof value !== 'string' || !HOSTNAME_PATTERN.test(value)) {
        throw new KitError({
            code: 1918,
            name: 'INPUT_INVALID_REFERRER_DOMAIN',
            type: 'INPUT',
            recoverability: 'FATAL',
            message: '`referrerDomain` must be a bare hostname when provided (e.g. `portal.arc.io`) — no scheme, port, path, wildcard, or whitespace.'
        });
    }
}
function assertOptionalAbsoluteUrl(value, fieldName) {
    if (value === undefined) return;
    if (typeof value !== 'string' || value.length === 0) {
        throw new KitError({
            code: 1916,
            name: 'INPUT_INVALID_SERVER_URL',
            type: 'INPUT',
            recoverability: 'FATAL',
    ...
    try {
        ...
        throw new KitError({
            ...
            type: 'INPUT',
            recoverability: 'FATAL',
            message: `\`${fieldName}\` must be a valid absolute URL (received: ${value}).`
        });
    }
    // Refuse anything that isn't https:// (with a localhost http://
    // carve-out for local fixtures). Defends both against `javascript:`
    // / `data:` injection into the iframe `src` (widgetBaseUrl) and
    // against accidentally routing API-key traffic over plaintext
    // (baseUrl).
...
function assertOptionalNonNegativeInteger(value, fieldName) {
    ...
    if (typeof value !== 'number' || !Number.isInteger(value) || !Number.isFinite(value) || value < 0) {
        throw new KitError({
            code: 1917,
            name: 'INPUT_INVALID_REQUEST_TIMEOUT',
            type: 'INPUT',
            recoverability: 'FATAL',
...
/**
 * Assert that `serverKit` is a usable {@link OnrampServerKit}.
 *
 * @param serverKit - The first argument to `createSessionRouteHandler`.
 *
 * @throws KitError - When `serverKit` is not an object exposing a
 *   `createSession` function.
 *
 * @internal
 */ function assertOnrampServerKit(serverKit) {
    if (serverKit === null || typeof serverKit !== 'object' || typeof serverKit.createSession !== 'function') {
        throw new KitError({
            code: 1932,
            name: 'INPUT_INVALID_SERVER_KIT',
            type: 'INPUT',
            recoverability: 'FATAL',
            message: '`createSessionRouteHandler` requires an `OnrampServerKit` instance.'
        });
    }
}
/**
 * Assert that the optional `options` bag is an object when provided.
 *
 * @param options - The second argument to `createSessionRouteHandler`.
 *
 * @throws KitError - When `options` is supplied as a non-object.
 *
 * @internal
 */ function assertRouteHandlerOptions(options) {
    if (options === null || typeof options !== 'object') {
        throw new KitError({
            code: 1929,
            name: 'INPUT_INVALID_ROUTE_HANDLER_OPTIONS',
            type: 'INPUT',
            recoverability: 'FATAL',
            message: '`createSessionRouteHandler` options must be an object when provided.'
        });
    }
}

/**
 * Empty slot map — the onramp server kit owns no provider slots
 * because it talks to exactly one upstream service (wallets-api).
 *
 * @remarks
 * Kept as a module-level constant so the `KitContext` reference is
 * stable across factory invocations, which makes the kit cheap to
 * construct per-request if the host wants to (it does not have to —
 * the factory is safe to memoise).
 */ const ONRAMP_SERVER_KIT_SLOTS = defineSlots({});
/**
 * Build the onramp server kit.
 *
 * @remarks
 * The factory composes:
 *
 * 1. An embedded `fetch`-based wallets-api client.
 * 2. A `createSession` operation closed over that client.
 * 3. A `KitContext` (empty slot map; identity-only).
 * 4. A `Kit` instance via {@link createKit} so caller stamping, the
 *    runtime middleware stack, and the runtime-backed event surface come
 *    from one source of truth.
 *
 * Lifecycle: the returned kit is stateless beyond closures, safe to
 * cache as a process-singleton or re-create per request. The
 * underlying HTTP client is also stateless.
 *
 * @param options - Kit options. `apiKey` is required.
 * @returns Fully-assembled onramp server kit.
 *
 * @throws KitError - When `apiKey` is missing or empty, `fetch` is
 *   unavailable, or other constructor-time inputs are invalid.
 *
 * @example
 * ```ts
 * import { createOnrampServerKit } from '@circle-fin/onramp-kit/server'
 *
 * const onramp = createOnrampServerKit({ apiKey: process.env.ONRAMP_API_KEY! })
 *
 ...
 * res.json(session)
 * ```
 */ function createOnrampServerKit(options) {
    assertOnrampServerKitOptions(options);
    const api = createOnrampApiClient({
    ...
    const context = createKitContext({
        identity: ONRAMP_SERVER_KIT_IDENTITY,
        builtInSlots: ONRAMP_SERVER_KIT_SLOTS,
        providers: [],
        defaults: {}
    });
    const createSession = createSessionOperation({
        api,
        widgetBaseUrl,
 ...
 * })
 * ```
 */ function createSessionRouteHandler(serverKit, options = {}) {
    assertOnrampServerKit(serverKit);
    assertRouteHandlerOptions(options);
    return async function handle(request) {
        try {
            const method = request.method.toUpperCase();
            if (method !== 'POST') {
                // RFC 7231 §6.5.5 requires a 405 to advertise the permitted
                // methods via `Allow`.
                return jsonError(405, {
                    code: 0,
                    name: 'METHOD_NOT_ALLOWED',
                    type: 'INPUT',
                    message: `Only POST is allowed, got ${method}.`
                }, {
                    Allow: 'POST'
                });
            }
            if (options.authorize) {
                ...
                // Fail-closed: only `true` or `undefined` are accepted as
                // explicit "allow". Anything else (false, null, 0, "", an
                // unexpected object from a plain-JS caller) short-circuits as
                // unauthorized so that a buggy host hook cannot accidentally
                // mint sessions.
            ...
            try {
                ...
                return jsonError(400, {
                    code: 1930,
                    name: 'INPUT_BODY_NOT_JSON',
                    type: 'INPUT',
                    message: 'Request body must be valid JSON.'
                });
            }
            // Reuse the shared protocol parser so the request wire contract
            // (and its `INPUT_INVALID_SESSION_REQUEST` error) is single-
            // sourced. It throws a `KitError(INPUT)` on a shape mismatch,
            // which the outer catch maps to a 400 via `mapErrorToResponse`.
            const params = parseOnrampSessionRequest(body);
            const session = await serverKit.createSession(params);
            ...
            if (options.onError) {
                try {
                    await options.onError(err, request);
                } catch  {
                // Swallow logging-side failures — the response path must
                // stay deterministic.
                }
            }
...
function mapErrorToResponse(err) {
    if (err instanceof KitError) {
        return jsonError(kitErrorToStatus(err.type), {
            code: err.code,
            name: err.name,
            type: err.type,
            message: err.message
    ...
    return jsonError(500, {
        ...
        name: 'UNKNOWN',
        type: 'UNKNOWN',
        message: 'Unexpected error while minting onramp session.'
    });
}
function kitErrorToStatus(type) {
    switch(type){
        case 'INPUT':
            return 400;
        case 'RATE_LIMIT':
...
/**
 * Assert that the supplied value is a usable {@link AppServerKitConfig}.
 *
 * @remarks
 * Validates only the shape AppKit owns — that `config` is an object
 * and `config.onramp` is present. Field-level validation of
 * `onramp` (apiKey shape, URL formats, timeouts) is delegated to
 * `createOnrampServerKit` so we have a single source of truth for
 * the underlying onramp options.
 *
 * @param config - The first argument to `createAppServerKit`.
 *
 * @throws KitError - When `config` is not an object, or
 *   `config.onramp` is missing / non-object.
 *
 * @internal
 */ function assertAppServerKitConfig(config) {
    if (config === null || typeof config !== 'object') {
        throw new KitError({
            code: 1950,
            name: 'INPUT_INVALID_APP_SERVER_KIT_CONFIG',
            type: 'INPUT',
            recoverability: 'FATAL',
            message: '`createAppServerKit` requires a configuration object.'
        });
    }
    const onramp = config.onramp;
    if (onramp === undefined || onramp === null || typeof onramp !== 'object') {
        throw new KitError({
            code: 1951,
            name: 'INPUT_MISSING_APP_SERVER_KIT_ONRAMP',
            type: 'INPUT',
            recoverability: 'FATAL',
            message: '`createAppServerKit` requires an `onramp` configuration object.'
        });
    }
}
 ...
 * ```
 */ function createAppServerKit(config) {
    assertAppServerKitConfig(config);
    return Object.freeze({
        onramp: createOnrampServerKit(config.onramp)
    });
}
