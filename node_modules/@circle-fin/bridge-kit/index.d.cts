...
import { CCTPXRouteToken, CCTPXBridgingProvider } from '@circle-fin/provider-cctpx';
export { CCTPXBridgingProvider, CCTPXBridgingProviderConfig, CCTPXRouteToken, CCTPXTokenId, KNOWN_TOKEN_SYMBOLS, KnownTokenSymbol } from '@circle-fin/provider-cctpx';
...
/**
 * Chain definition with CCTPx configuration.
 *
 * @alias ChainDefinitionWithCCTPX
 * @extends ChainDefinition
 * @category Types
 *
 * @description Represents a chain definition that includes CCTPx configuration.
 * Use this type to narrow `ChainDefinition` to chains where the CCTS contract is
 * deployed, allowing safe access to `chain.cctpx.serviceAddress` without optional
 * chaining.
 *
 * @example
 * ```typescript
 * import type { ChainDefinitionWithCCTPX } from '@core/chains'
 * import { isCCTPXSupported } from '@core/chains'
 *
 * function getServiceAddress(chain: ChainDefinition): string | null {
 *   if (isCCTPXSupported(chain)) {
 ...
 *
 * @see {@link CCTPXChainConfig} for the structure of CCTPx configuration.
 * @see {@link isCCTPXSupported} for the type guard.
 */
type ChainDefinitionWithCCTPX = ChainDefinition & {
    cctpx: CCTPXChainConfig;
};
/**
...
declare const Chains_Arc: typeof Arc;
...
declare const InputError: {
    ...
    /** Wallet is on a different chain than the operation targets */
    readonly CHAIN_MISMATCH: {
        readonly code: 1010;
        readonly name: "INPUT_CHAIN_MISMATCH";
        readonly type: ErrorType;
    };
    /** User rejected a chain-switch or add-chain prompt */
    readonly CHAIN_SWITCH_REJECTED: {
        readonly code: 1011;
        readonly name: "INPUT_CHAIN_SWITCH_REJECTED";
        readonly type: ErrorType;
    };
    /** Wallet does not recognise the target chain and cannot add it */
    readonly UNRECOGNIZED_CHAIN: {
        readonly code: 1012;
        readonly name: "INPUT_UNRECOGNIZED_CHAIN";
        readonly type: ErrorType;
    };
    /** Swap amount is outside the upstream provider's accepted bounds */
    readonly AMOUNT_OUT_OF_RANGE: {
        readonly code: 1013;
        readonly name: "INPUT_AMOUNT_OUT_OF_RANGE";
        readonly type: ErrorType;
    };
    /**
     ...
     */
    readonly UNSUPPORTED_FEE_TOKEN: {
        readonly code: 1014;
        readonly name: "INPUT_UNSUPPORTED_FEE_TOKEN";
        readonly type: ErrorType;
    };
    /** Retry / resume is not supported by this provider for the given result */
    readonly RETRY_NOT_SUPPORTED: {
        readonly code: 1015;
        readonly name: "INPUT_RETRY_NOT_SUPPORTED";
        readonly type: ErrorType;
    };
    /** Bridge-step analysis for retry is not supported by this provider */
    readonly STEP_ANALYSIS_NOT_SUPPORTED: {
        readonly code: 1016;
        readonly name: "INPUT_STEP_ANALYSIS_NOT_SUPPORTED";
        readonly type: ErrorType;
    };
    /**
     ...
     */
    readonly RESERVED_OPERATION_NAME: {
        readonly code: 1000;
        readonly name: "INPUT_RESERVED_OPERATION_NAME";
        readonly type: ErrorType;
    };
    /** A kit was constructed with a missing / undefined operation factory */
    readonly MISSING_OPERATION: {
        readonly code: 1017;
        readonly name: "INPUT_MISSING_OPERATION";
        readonly type: ErrorType;
    };
...
declare const ServiceError: {
     ...
     */
    readonly ROUTE_CHECK_UNAVAILABLE: {
        readonly code: 8003;
        readonly name: "SERVICE_ROUTE_CHECK_UNAVAILABLE";
        readonly type: ErrorType;
    };
};
 ...
 * ```
 */
declare const SPEED_DOWNGRADED_WARNING_CODE: "SPEED_DOWNGRADED";
/**
 ...
 * ```
 */
declare const QUOTE_NOT_REUSED_WARNING_CODE: "QUOTE_NOT_REUSED";
/**
...
declare abstract class BridgingProvider<TProviderActions = Record<string, unknown>, TToken extends string = 'USDC', TFeeToken extends string = 'USDC'> {
     ...
     * ```
     */
    abstract bridge<TFromAdapterCapabilities extends AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities>(params: BridgeParams$1<TFromAdapterCapabilities, TToAdapterCapabilities, ChainDefinition, TToken>): Promise<BridgeResult<TToken>>;
    /**
     ...
     * ```
     */
    abstract estimate<TFromAdapterCapabilities extends AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities>(params: BridgeParams$1<TFromAdapterCapabilities, TToAdapterCapabilities, ChainDefinition, TToken>): Promise<EstimateResult<TToken, TFeeToken>>;
    /**
 ...
 *
 * @remarks
 * The plain-estimate arm leaves `quote` as `unknown`, because the kit routes
 * across providers and each issues its own quote shape. Narrow to a provider's
 * own estimate type to read it, or take the receive-exact arm, whose `quote` is
 ...
 * @since 1.14.0
 */
type BridgeEstimateResult = EstimateResult<string, string> | ReceiveExactEstimateResult;
 ...
 */
declare const getDefaultProviders: (config?: DefaultProvidersConfig) => readonly [CCTPV2BridgingProvider, CCTPXBridgingProvider];
...
interface BridgeKitConfig<TExtraProviders extends FlexibleBridgingProvider[] = []> {
    ...
    /**
     * Custom HTTP headers forwarded with the default providers' attestation
     * (Iris) API requests — both `CCTPV2BridgingProvider` (USDC routes) and
     * `CCTPXBridgingProvider` (non-USDC routes).
     *
     * @remarks
...
/**
 * Keys present on any member of a union of object types.
 *
 * `keyof (A | B)` resolves to only the keys common to every member (the
 * intersection). Distributing over the union with a conditional collects
 * each member's keys instead, yielding the true union of keys.
 *
 * @typeParam T - A union of object types.
 */
...
type MergeActions<Ps extends readonly FlexibleBridgingProvider[]> = {
    [K in KeysOfUnion<Ps[number]['actions']>]: Extract<Ps[number]['actions'], Record<K, unknown>>[K];
};
/**
 ...
 *
 * This type merges the actions from both the default providers (CCTPv2 and
 * CCTPx) and any additional providers passed to the BridgeKit constructor. It
 * ensures that all available actions from all providers are accessible through
 * the event system.
 *
 ...
 */
type FlexibleBridgingProvider = BridgingProvider<any, string, string>;
/**
...
declare class BridgeKit<TExtraProviders extends FlexibleBridgingProvider[] = []> {
     ...
     * @throws {KitError} With `INPUT_UNSUPPORTED_ROUTE` (1003, `FATAL`) when no
     *   registered provider supports the route.
     * @throws {KitError} With `SERVICE_ROUTE_CHECK_UNAVAILABLE` (8003,
     *   `RETRYABLE`) when a provider's route check failed transiently — for
     *   example an unreachable token registry. Support could not be determined,
     *   so retry rather than treating the route as unsupported.
     *
     * @example
     * ```typescript
     * import { BridgeKit } from '@circle-fin/bridge-kit'
     ...
     * ```
     */
    bridge<TFromAdapterCapabilities extends AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities>(params: Omit<BridgeParams<TFromAdapterCapabilities, TToAdapterCapabilities>, 'token'> & {
        token?: 'USDC';
    }): Promise<BridgeResult>;
    bridge<TFromAdapterCapabilities extends AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities>(params: BridgeParams<TFromAdapterCapabilities, TToAdapterCapabilities>): Promise<BridgeResult<string>>;
    /**
     ...
     * ```
     */
    retry<TFromAdapterCapabilities extends AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities, TToken extends string = string>(result: BridgeResult<TToken>, context: RetryContext<TFromAdapterCapabilities, TToAdapterCapabilities>, invocationMeta?: InvocationMeta): Promise<BridgeResult<TToken>>;
    /**
     ...
     * ```
     */
    estimate<TFromAdapterCapabilities extends AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities>(params: Omit<BridgeParams<TFromAdapterCapabilities, TToAdapterCapabilities>, 'token'> & {
        token?: 'USDC';
        config: {
            feePayment: 'source';
        };
    ...
    estimate<TFromAdapterCapabilities extends AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities>(params: BridgeParams<TFromAdapterCapabilities, TToAdapterCapabilities>): Promise<EstimateResult<string, string>>;
    /**
...
declare const bridgeParamsWithChainIdentifierSchema: z.ZodObject<{
    ...
    token: z.ZodOptional<z.ZodUnion<[z.ZodEnum<["USDC", "cirBTC", "wETH", "EURC"]>, z.ZodString]>>;
    config: z.ZodOptional<z.ZodObject<{
        transferSpeed: z.ZodOptional<z.ZodNativeEnum<typeof TransferSpeed>>;
        feePayment: z.ZodOptional<z.ZodEnum<["source", "destination"]>>;
    ...
    quote: z.ZodOptional<z.ZodUnknown>;
}, "strip", z.ZodTypeAny, {
    from: {
        adapter: {
            prepare: (...args: unknown[]) => unknown;
