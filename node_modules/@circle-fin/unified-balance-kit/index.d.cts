...
/**
 * Per-kit identity passed to all telemetry helpers.
 *
 * @example
 * ```typescript
...
/**
 * Internal options bag for the spend operation.
 *
 * Carries callbacks that exist solely to support kit-level telemetry — they
 * are not part of the public `SpendParams` contract and external consumers
 * are not expected to populate them. Kept on a separate type (rather than
 * mixed into `SpendParams`) so the published `SpendParams` declaration stays
 * free of implementation-detail fields.
 *
 * @internal
 */
interface SpendOptions {
    /**
     * Callback invoked exactly once with the destination-chain mint
     * transaction hash as soon as the mint broadcast succeeds, before
     * any post-broadcast logic that might throw. Lets the caller observe
     * the hash even when a later step fails, so error telemetry can
     * include it.
     *
     * Per-call closures keep concurrent invocations isolated. The provider
     * defends against a throwing callback so user operations are never
     * stranded after a successful on-chain mint — a callback that throws
     * is swallowed and surfaced as a `console.warn` prefixed with
     * `[stablecoin-kits provider-gateway-v1] callback threw and was
     * swallowed:` (the raw `Error` is passed as the second `console.warn`
     * argument so the stack survives).
     *
     * @internal
     */
...
interface IGatewayProvider<TProviderActions = GatewayV1Actions> {
     ...
     * @param options - Internal options for kit-level telemetry; external
     *   consumers should not populate this and the field may change
     *   without notice. See {@link SpendOptions}.
     ...
     * ```
     */
    spend<TFromAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(params: SpendParams$1<TFromAdapterCapabilities, TToAdapterCapabilities>, options?: SpendOptions): Promise<SpendResult$1>;
    /**
...
interface UnifiedBalanceKitContext<TProviders extends readonly FlexibleGatewayProvider[] = readonly FlexibleGatewayProvider[]> {
    ...
    /**
     * Optional declarative fee recipient map, keyed by chain type. When
     * set, takes priority over `customFeePolicy.resolveFeeRecipientAddress`
     * for resolving the fee recipient address.
     */
    feeRecipients?: FeeRecipientsConfig;
    /**
     * Analytics telemetry config for success-path event emissions.
     * Set by the class-based API; absent in functional API usage.
     *
     * @internal
     */
    analyticsConfig?: TelemetryConfig;
}
/**
 ...
 *
 * For same-chain (STANDARD) deposits: validates params, resolves defaults
 * (token → `'USDC'`, allowanceStrategy → `'authorize'`), finds a provider,
 * and delegates to the provider's deposit operation.
 *
 * For cross-chain (FAST) deposits (when `to` is set): fetches or validates a
 * fee quote, runs a preflight affordability check, submits the prepaid-FORWARD
 * burn on the source chain, and waits for Circle's relay to confirm the mint
 * on the destination. The caller's Gateway account is credited on the
 * destination chain.
 *
 * @param context - The kit context containing providers.
 * @param params - The deposit details including the depositor context, amount,
 *   optional token type, optional allowance strategy, optional destination
 *   chain (`to`), optional transfer config, and optional quote.
 * @returns Promise resolving to the deposit confirmation.
 * @throws KitError If validation or preflight fails, or no provider supports
 *   the route.
 * @throws KitError If the destination chain is not a Gateway fast-finality
 ...
 * })
 *
 * // Cross-chain fast deposit (from estimate)
 * const estimate = await estimateDeposit(context, {
 *   from: { adapter, chain: 'Ethereum_Sepolia' },
 *   amount: '100',
 *   to: { chain: 'Arc_Testnet' },
 *   config: { transferSpeed: 'FAST' },
 * })
 * const crossResult = await deposit(context, { ...estimate, from: { adapter, chain: 'Ethereum_Sepolia' } })
 * ```
 */
 ...
 *
 * For same-chain (STANDARD) deposits: validates params, resolves defaults
 * (token → `'USDC'`, allowanceStrategy → `'approve'`), finds a provider,
 * and delegates to the provider's depositFor operation.
 *
 * For cross-chain (FAST) deposits (when `to` is set): fetches or validates a
 * fee quote, runs a preflight affordability check, submits the prepaid-FORWARD
 * burn on the source chain, and waits for Circle's relay to confirm the mint.
 * The `depositAccount` is credited on the destination chain.
 *
 * @param context - The kit context containing providers.
 * @param params - The deposit details including the depositor context, amount,
 *   the account address to credit, optional destination chain, and optional
 *   quote.
 * @returns Promise resolving to the deposit confirmation.
 * @throws KitError If validation or preflight fails, or no provider supports
 *   the route.
 ...
 * })
 *
 * // Cross-chain fast deposit-for (from estimate)
 * const estimate = await estimateDeposit(context, {
 *   from: { adapter, chain: 'Ethereum_Sepolia' },
 *   amount: '100',
 *   to: { chain: 'Arc_Testnet' },
 ...
 *   config: { transferSpeed: 'FAST' },
 * })
 * const crossResult = await depositFor(context, {
 *   ...estimate,
 *   from: { adapter, chain: 'Ethereum_Sepolia' },
 *   depositAccount: '0x1234…abcd',
 * })
...
/**
 * Estimate fees for a cross-chain deposit without executing the transaction.
 *
 * Resolves both chains, fetches a fee quote for FAST transfers, estimates
 * source-chain gas, and returns an itemized fee breakdown plus a quote
 * needed to commit to the price.
 *
 * The returned value is plain, serializable data — pass it directly into
 * {@link deposit} or {@link depositFor} (re-attaching only `from`):
 *
 * ```typescript
 * const estimate = await kit.estimateDeposit({ from, amount, to })
 * const result   = await kit.deposit({ ...estimate, from })
 * ```
 *
 * @param context - Kit context (used for analytics telemetry emissions).
 * @param params - The estimate parameters: source context, amount, destination
 *   chain, optional token, transfer speed, and allowance strategy.
 * @returns Promise resolving to an itemized fee estimate with an optional
 *   quote for FAST cross-chain transfers.
 * @throws KitError If the wallet address cannot be resolved — wallet not connected
 *   for user-controlled adapters, or `from.address` absent for
 *   developer-controlled adapters.
 * @throws KitError If the source chain does not support Gateway v1 (STANDARD path).
 * @throws KitError If the source chain is not EVM (FAST path).
 * @throws KitError If the source chain does not support CCTP v2 (FAST path).
 * @throws KitError If the destination chain does not support CCTP v2 (FAST path).
 * @throws KitError If the destination chain is not a Gateway fast-finality
 *   destination (FAST path).
 * @throws KitError If the fee quote fetch fails (FAST path).
 *
 * @example
 * ```typescript
 * import {
 ...
 *   config: { transferSpeed: 'FAST' },
 * })
 * // estimate.fees → [{ type: 'gasFee', token: 'ETH', amount: '0.0002' }, ...]
 * // estimate.quote → the quote returned by estimateDeposit for committing to this price
 * const result = await deposit(context, { ...estimate, from: { adapter, chain: 'Ethereum_Sepolia' } })
 * ```
 */
declare function estimateDeposit<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: EstimateDepositParams<TAdapterCapabilities>): Promise<EstimateDepositResult>;
 ...
 * @param onBroadcast - Internal callback invoked once with the destination-chain
 *   mint transaction hash immediately after the provider broadcast succeeds.
 *   Lets the kit class observe the hash even when a later step fails, so error
 *   telemetry can include it. Not part of the public surface — external
 *   callers using the functional `spend` should leave this unset.
 * @returns Promise resolving to the spend confirmation.
 ...
 * ```
 */
declare function spend<TFromAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: SpendParams<TFromAdapterCapabilities, TToAdapterCapabilities>, 
/** @internal */
onBroadcast?: (txHash: string) => void): Promise<SpendResult>;
/**
 * Estimate the fees for a spend operation without executing it.
 *
 * @param context - The kit context containing providers.
...
/**
 * Configuration forwarded to the default Gateway v1 provider.
 */
interface DefaultProvidersConfig {
    /**
     * Custom HTTP headers forwarded with the default Gateway provider's API
     * requests. See {@link UnifiedBalanceKitConfig.headers}.
     */
    headers?: Record<string, string>;
}
/**
 ...
 * ```
 */
declare const getDefaultProviders: (config?: DefaultProvidersConfig) => readonly [IGatewayProvider<GatewayV1Actions>];
/**
 ...
 * ```
 */
declare function createUnifiedBalanceKitContext<TExtraProviders extends FlexibleGatewayProvider[] = []>(config: UnifiedBalanceKitConfig<TExtraProviders> & {
    excludeDefaultProviders: true;
}): UnifiedBalanceKitContext<[...TExtraProviders]>;
...
declare class UnifiedBalanceKit<TExtraProviders extends FlexibleGatewayProvider[] = []> implements UntypedEventEmitter {
    ...
    /**
     * Estimate fees for a cross-chain deposit without executing it.
     *
     * Returns plain, serializable data that can be spread directly into
     * {@link UnifiedBalanceKit.deposit} or {@link UnifiedBalanceKit.depositFor}:
     *
     * ```typescript
     * const estimate = await kit.estimateDeposit({ from, amount, to })
     * const result   = await kit.deposit({ ...estimate, from })
     * ```
     *
     * @param params - The estimate parameters including source context, amount,
     *   destination chain, optional transfer speed, and optional allowance strategy.
     * @returns Promise resolving to an itemised fee breakdown.
     *
     * @see UnifiedBalanceKit.deposit to execute after estimating.
     */
    estimateDeposit<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(params: EstimateDepositParams<TAdapterCapabilities>): Promise<EstimateDepositResult>;
    /**
     * Spend (mint) USDC on a destination chain by pulling funds from one
     * or more account sources.
     *
...
export { BalanceError, Blockchain, InputError, KitError, NetworkError, OnchainError, RpcError, SUPPORTED_TOKENS, UnifiedBalanceChain, UnifiedBalanceKit, addDelegate, createUnifiedBalanceKitContext, deposit, depositFor, estimateDeposit, estimateSpend, getBalances, getDelegateStatus, getErrorCode, getErrorMessage, getSupportedChains, initiateRemoveFund, isBalanceError, isFatalError, isInputError, isKitError, isNetworkError, isOnchainError, isRetryableError, isRpcError, removeDelegate, removeFund, resolveChainIdentifier, setExternalPrefix, spend };
export type { ActionHandler, AdapterContext, Allocation, AllocationResult, AllowanceStrategy, BalanceSource, BalanceWithPendingBreakdown, BridgeChainIdentifier, ChainBalanceBreakdown, ChainDefinition, ChainIdentifier, ConfirmedBalanceEntry, CustomFeeConfig, CustomFeePolicy, DelegateStatus, DepositConfig, DepositForParams, DepositParams, DepositProgress, DepositResult, ErrorDetails, EstimateDepositParams, EstimateDepositResult, EstimateSpendResult, FeeAllocation, FeeEntry, FeeQuoteExchangeRates, FeeRecipientChainType, FeeRecipientsConfig, FeeType, ForwarderSpendDestination, GetBalancesParams, GetBalancesResult, GetDelegateStatusParams, GetSupportedChainsOptions, InitiateRemoveFundParams, InitiateRemoveFundResult, NetworkType, PendingBalanceTransaction, Recoverability, RemoveFundParams, RemoveFundResult, ResolvedSpendParams, RetryMintConfig, SpendConfig, SpendDestination, SpendDestinationUnion, SpendFeeFunction, SpendFeeRecipientFunction, SpendParams, SpendResult, SpendSource, SpendStep, SupportedToken, SupportedTokenInput, UnifiedBalanceChainIdentifier, UnifiedBalanceKitConfig, UnifiedBalanceKitContext, UntypedEventEmitter, UpdateDelegateParams, UpdateDelegateResult };
