...
_Move USDC across chains with simple method calls—no manual bridging, no liquidity pre-positioning, no on-chain delays for normal spend flows_
...
The Unified Balance Kit provides a high-level abstraction for cross-chain USDC operations via Circle's Gateway protocol. Deposit, spend (mint), and query balances across multiple chains with simple method calls—no manual bridging steps, no liquidity pre-positioning, and no on-chain delays for normal spend flows.

Use `spend` for normal movement out of a Unified Balance. `removeFund` is a trustless recovery path for situations where the normal spend flow is unavailable, and it requires a 7-day withdrawal delay before funds can be removed.
...
### Fee recipients across chain types

`spend()` can pull funds from multiple source chains (e.g. combining an EVM
and a Solana source), but every fee payout in a spend mints to the **same
single destination chain** — the one you pass as `to`. So `destinationChain`
above is always the spend's destination, never a source chain, and exactly
one recipient address (valid on that destination) is resolved per spend.

If you spend to different destination chain types across calls (some spends
to an EVM chain, others to Solana), configure `setFeeRecipients` once with an
address for each chain type instead of writing chain-branching logic
yourself. **`setFeeRecipients` only affects which recipient address is used
— you still need `setCustomFeePolicy`'s `computeFee` to determine the fee
amount.** Calling `setFeeRecipients` without ever calling
`setCustomFeePolicy` throws at spend time (there is no `computeFee` to
determine an amount):

```typescript
kit.setCustomFeePolicy({
  computeFee: (params) => (parseFloat(params.amount) * 0.01).toFixed(6), // 1%
})
kit.setFeeRecipients({
  evm: '0xEvmAddress...',
  solana: 'SolanaAddressBase58...',
})
```

At spend time the kit maps the resolved destination chain to its chain type
(`'evm'` or `'solana'` today) and looks up the matching entry.
`setFeeRecipients` takes priority over `resolveFeeRecipientAddress` for any
chain type it has an entry for — the callback is never invoked for that
type. If the map is missing an entry for the resolved destination's chain
type and a `resolveFeeRecipientAddress` callback is also configured, the
kit falls back to that callback for the missing type (a warning is logged
so the fallback isn't a silent surprise). The spend throws before any fee
collection is attempted only if neither the map nor the callback can
resolve an address for the destination's chain type. Existing integrations
using only `resolveFeeRecipientAddress` or a single-string `recipientAddress`
...
| `getSupportedChains(context, token?, options?)` | Get chains supported by configured providers                       |
...
| `initiateRemoveFund(context, params)`           | Start recovery-only fund removal and the 7-day delay               |
| `removeFund(context, params)`                   | Complete recovery-only fund removal after the 7-day delay          |

Use `spend` for normal movement out of a Unified Balance. Use `removeFund`
only as a trustless recovery path when the normal spend flow is unavailable.
