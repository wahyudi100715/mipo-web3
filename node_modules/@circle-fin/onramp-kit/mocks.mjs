...
const DEFAULT_MOCK_WIDGET_URL = 'https://onramp.example.test/launch?sessionToken=mock-session-token';
function normaliseToMinted(session) {
    return {
        ...session,
        widgetUrl: session.widgetUrl ?? DEFAULT_MOCK_WIDGET_URL,
        traceId: session.traceId ?? 'mock-trace'
    };
}
/**
 * Build a server kit whose `createSession()` does not perform any
 * network I/O.
 *
 * @remarks
 * The mock kit still goes through `createKit` / `createKitOperation`,
 * so caller stamping, schema validation, the runtime middleware stack,
 * and the event surface behave exactly like production. The only thing
 * that's swapped is the HTTP round-trip.
 *
 * @example
 * ```ts
 * import { createMockOnrampServerKit } from '@circle-fin/onramp-kit/mocks'
 *
 * const onramp = createMockOnrampServerKit({
 *   session: (params) => ({
 *     sessionId: `mock-${params.appUserId}`,
 *     sessionToken: 'mock-token',
 *     widgetUrl: 'https://onramp.example.test/launch?sessionToken=mock-token',
 *     expiresAt: new Date(Date.now() + 60_000).toISOString(),
 *   }),
 * })
 *
 * const session = await onramp.createSession({ appUserId: 'u1', destinationAddress: '0x1' })
 * ```
 */ function createMockOnrampServerKit(options = {}) {
    const fallback = {
        sessionId: 'mock-session',
        sessionToken: 'mock-session-token',
        widgetUrl: DEFAULT_MOCK_WIDGET_URL,
        expiresAt: new Date(Date.now() + 60 * 60_000).toISOString(),
        traceId: 'mock-trace'
    };
    const configured = options.session;
    const sessionFactory = typeof configured === 'function' ? configured : ()=>configured ?? fallback;
    const context = createKitContext({
    ...
    const createSession = createKitOperation({
        name: 'createSession',
        schema: onrampSessionRequestSchema,
        // eslint-disable-next-line @typescript-eslint/require-await -- mock body is synchronous; async keeps the contract aligned with the production op.
        run: async (params)=>normaliseToMinted(sessionFactory(params))
    });
    return createKit({
        context,
        operations: {
...
/**
 * Build a {@link MockOnrampWidget} suitable for unit tests.
 *
 * @param options - Optional {@link CreateMockOnrampWidgetOptions}.
 *   Defaults to deterministic placeholder ids.
 * @returns A fresh in-memory mock widget. The returned object reuses
 *   the production lifecycle state machine, so tests against it stay
 *   faithful to production behaviour.
 *
 * @example
 * ```ts
 * import { createMockOnrampWidget } from '@circle-fin/onramp-kit/mocks'
 * import { ONRAMP_EVENT_CODES, ONRAMP_EVENT_TYPES } from '@circle-fin/onramp-kit'
 *
 * const widget = createMockOnrampWidget({ sessionId: 'mock' })
 *
 * const settled: unknown[] = []
 * widget.on('DEPOSIT_SETTLED', (e) => settled.push(e.payload))
 *
 * widget.emit({
 *   event: ONRAMP_EVENT_TYPES.DEPOSIT_SETTLED,
 *   code: ONRAMP_EVENT_CODES.DEPOSIT_SETTLED,
 *   payload: {
 *     amount: 100,
 *     tokenSymbol: 'USDC',
 *     paymentMethod: 'Debit',
 ...
 * ```
 */ function createMockOnrampWidget(options = {}) {
    ...
    return {
        ...
        on (event, handler) {
            ...
            const set = typedHandlers.get(event) ?? new Set();
            set.add(handler);
            typedHandlers.set(event, set);
            return ()=>{
                set.delete(handler);
            };
        },
        off (_event, handler) {
            // Removal is by handler reference, matching the real widget
            // (`createControllerEvents`): the event name is accepted for
            // symmetry with `on` but every subscription for this handler is
            // removed, regardless of the name(s) it was registered under. A
            // handler registered on two events and removed via one of them
            // must not survive on the other.
            anyHandlers.delete(handler);
            for (const set of typedHandlers.values()){
                set.delete(handler);
            }
        },
        close () {
        ...
        emit (envelope) {
            ...
            const set = typedHandlers.get(envelope.event);
            if (set) {
                for (const handler of set)handler(envelope);
            }
            for (const handler of anyHandlers)handler(envelope);
        }
    };
}

/**
 * Build an in-memory mock implementation of {@link OnrampKit} for unit
 * tests.
 *
 * @param options - Behaviour overrides for the mock; defaults are
 *   suitable for the typical "drive the widget" test.
 * @returns A {@link MockOnrampKit} that records every call.
 *
 * @example
 * ```ts
 * import { createMockOnrampKit } from '@circle-fin/onramp-kit/mocks'
 *
 * const onrampKit = createMockOnrampKit()
 * const widget = onrampKit.mountIframe({
 *   session: { sessionToken: 'tok-1', sessionId: 's-1', expiresAt: '2099-01-01T00:00:00Z' },
 *   container: document.createElement('div'),
 * })
 *
 * expect(onrampKit.iframeCalls).toHaveLength(1)
 * await widget.close()
 * ```
 */ function createMockOnrampKit(options = {}) {
    const iframeCalls = [];
    const windowCalls = [];
    const buildWidget = (opts)=>{
        if (options.widget) return options.widget(opts);
        return createMockOnrampWidget({
            sessionId: opts.session.sessionId,
            traceId: opts.session.traceId
        });
    };
    let lastWidget;
    const mountIframe = (opts)=>{
        const widget = buildWidget(opts);
        iframeCalls.push({
            options: opts,
            widget
        });
    ...
    const openWindow = (opts)=>{
        if (options.popupBlocked === true) {
            const blocked = {
                status: 'blocked',
                reason: 'popup_blocked',
                errorMessage: 'Mock popup blocker is active.'
            };
            windowCalls.push({
                options: opts,
                result: blocked
...
export { createMockOnrampKit, createMockOnrampServerKit, createMockOnrampWidget };
