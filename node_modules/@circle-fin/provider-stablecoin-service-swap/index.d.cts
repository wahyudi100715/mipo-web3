...
/**
 * Source-leg transaction information reported by the service.
 *
 * Unlike {@link SwapDestinationLeg}, the Stablecoin Service status endpoint
 * does not report source-leg token details, so only `txHash` is exposed.
 */
 ...
 * `amount` is in base units (stringified bigint) at provider level;
 * SwapKit formats it to a human-readable decimal.
 */
 ...
 *
 * Note: Authentication must be provided inline per operation via `config.apiKey`.
 *
 * @example
 * ```typescript
 ...
 */
declare const CREDENTIAL_CONFIG_FIELDS: readonly ["apiKey", "kitKey"];

export { CREDENTIAL_CONFIG_FIELDS, StablecoinServiceSwapProvider };
export type { AllowanceStrategy, EstimateResult, ExecutedTransaction, ServiceSwapConfig, ServiceSwapFee, ServiceSwapParams, StablecoinServiceSwapProviderConfig, SwapContext, SwapDestinationLeg, SwapProgress, SwapResult, SwapSourceLeg, SwapStatus, SwapTerminalStatus, SwappingProvider, TokenAmount };
