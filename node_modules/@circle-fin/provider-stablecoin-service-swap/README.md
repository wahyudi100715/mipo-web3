...
| `customFee.amount`           | `string`                | -          | Absolute fee in base units (mutually exclusive with percentageBps) |
| `customFee.recipientAddress` | `string`                | -          | Address that receives the fee (required if customFee is set) |
| `apiKey`                     | `string`                | -          | Optional Circle API key (bearer credential); omit for permissionless mode |
| `kitKey`                     | `string`                | -          | Deprecated alias for `apiKey`; still honored                 |
...
- **Authentication**: Optional API key via Authorization header (permissionless when omitted)
...
Authentication with Circle's Stablecoin Service is optional. When an API key is provided it is sent as a bearer credential; when omitted, the provider operates in permissionless mode (no `Authorization` header).

The credential is supplied per operation via `config.apiKey`, not on the
constructor — `StablecoinServiceSwapProviderConfig` carries only `customFee`.
...
> **Legacy `kitKey`:** `config.kitKey` is deprecated in favor of
> `config.apiKey`. It keeps working, and legacy `KIT_KEY:<keyId>:<keySecret>`
> values remain accepted by either field. When both fields are set, `apiKey`
> wins.
